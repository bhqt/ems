...
import com.ruoyi.system.domain.*;
import com.ruoyi.system.domain.bo.EnergyStatisticsBo;
import com.ruoyi.system.domain.bo.ItemTopologyBo;
import com.ruoyi.system.domain.enums.EnergyType;
import com.ruoyi.system.domain.vo.*;
import com.ruoyi.system.mapper.*;
import com.ruoyi.system.domain.vo.*;
import com.ruoyi.system.service.IEnergyService;
import com.ruoyi.system.service.IItemTopologyService;
import com.ruoyi.system.service.ISysConfigService;
import com.ruoyi.system.domain.bo.EnergyBo;
import com.ruoyi.system.mapper.*;
...
public class EnergyServiceImpl implements IEnergyService {
    ...
    public List<ChainDataVo> getChainByDevice(String energyType, String dateType, String date, String areaIds) {
        ...
        if (sns.size() > 0) {
            ...
            if (dateType.equals("date")) {
                ...
                List<EnergyStatistics> nowData = energyStatisticsMapper.selectList(buildStatisticQuery(sns, energyType).between(EnergyStatistics::getTime, date + " 00:00:00", date + " 23:59:59"));
                List<EnergyStatistics> lastData = energyStatisticsMapper.selectList(
                    buildStatisticQuery(sns, energyType).between(EnergyStatistics::getTime, DateUtils.dateTime(last) + " 00:00:00", DateUtils.dateTime(last) + " 23:59:59"));
                ...
                List<EnergyStatistics> nowData = energyStatisticsMapper.selectList(
                    buildStatisticQuery(sns, energyType).between(EnergyStatistics::getTime, getFirstDayOfWeek(year, week) + " 00:00:00", getLastDayOfWeek(year, week) + " 23:59:59"));
                List<EnergyStatistics> lastData = energyStatisticsMapper.selectList(
                    buildStatisticQuery(sns, energyType).between(EnergyStatistics::getTime, getFirstDayOfWeek(year, week - 1) + " 00:00:00", getLastDayOfWeek(year, week - 1) + " 23:59:59"));
                chainDataVos = getStatisticData(nowData, lastData, itemTopologies);
                ...
                List<EnergyStatistics> nowData = energyStatisticsMapper.selectList(
                    buildStatisticQuery(sns, energyType).between(EnergyStatistics::getTime, date.substring(0, 7) + "-01 00:00:00", date.substring(0, 7) + "-" + day + " 23:59:59"));
                List<EnergyStatistics> lastData = energyStatisticsMapper.selectList(
                    buildStatisticQuery(sns, energyType).between(EnergyStatistics::getTime, DateUtils.parseDateToStr("yyyy-MM", last) + "-01 00:00:00",
                        DateUtils.parseDateToStr("yyyy-MM", last) + "-" + lastDay + " 23:59:59"));
                chainDataVos = getStatisticData(nowData, lastData, itemTopologies);
            } else if (dateType.equals("year")) {
                Date now = DateUtils.parseDate(date.substring(0, 4) + "-01-01");
                Date last = DateUtils.addYears(now, -1);
                List<EnergyStatistics> nowData =
                    energyStatisticsMapper.selectList(buildStatisticQuery(sns, energyType).between(EnergyStatistics::getTime, now, DateUtils.parseDate(date.substring(0, 4) + "-12-31 23:59:59")));
                List<EnergyStatistics> lastData = energyStatisticsMapper.selectList(
                    buildStatisticQuery(sns, energyType).between(EnergyStatistics::getTime, last, DateUtils.parseDate(new SimpleDateFormat("yyyy").format(last) + "-12-31 23:59:59")));
                chainDataVos = getStatisticData(nowData, lastData, itemTopologies);
            }
        }
        return chainDataVos;
    ...
    private List<ChainDataVo> getStatisticData(List<EnergyStatistics> nowData, List<EnergyStatistics> lastData, List<ItemTopology> itemTopologies) {
        ...
        //根据拓扑逐层整理数据
        for (ItemTopology itemTopology : itemTopologies) {
            List<EnergyStatistics> nowList = nowData.stream().filter(e -> DeviceIdMatcher.containsDevice(itemTopology.getDeviceId(), e.getEquipmentSn())).collect(Collectors.toList());
            List<EnergyStatistics> lastList = lastData.stream().filter(e -> DeviceIdMatcher.containsDevice(itemTopology.getDeviceId(), e.getEquipmentSn())).collect(Collectors.toList());
    ...
    private LambdaQueryWrapper<Energy> buildQuery(List<String> deviceId, String tableName) {
        return new LambdaQueryWrapper<Energy>().in(Energy::getClientId, deviceId).eq(Energy::getType, tableName).orderByDesc(Energy::getTs);
    }

    private LambdaQueryWrapper<EnergyStatistics> buildStatisticQuery(List<String> deviceId, String type) {
        return new LambdaQueryWrapper<EnergyStatistics>().in(EnergyStatistics::getEquipmentSn, deviceId).eq(EnergyStatistics::getEnergyType, type).orderByDesc(EnergyStatistics::getTime);
    }
    ...
    public List<YearAnalysisVo> yearAnalysis(Long areaId, String year, String energyType) {
        ...
        for (int i = 1; i < 13; i++) {
            ...
            List<EnergyStatistics> current = energyStatisticsMapper.selectList(
                new LambdaQueryWrapper<EnergyStatistics>().between(EnergyStatistics::getTime, startCurrent, endCurrent).in(EnergyStatistics::getEquipmentSn, deviceId).eq(
                    EnergyStatistics::getEnergyType, energyType));
            ...
            List<EnergyStatistics> last = energyStatisticsMapper.selectList(
                new LambdaQueryWrapper<EnergyStatistics>().between(EnergyStatistics::getTime, startLast, endLast).in(EnergyStatistics::getEquipmentSn, deviceId).eq(EnergyStatistics::getEnergyType,
                    energyType));
            ...
            if (!(yearAnalysisVo.getCorrespondingPeriod().equals("--")) && !(yearAnalysisVo.getCurrentPeriod().equals("--"))) {
                yearAnalysisVo.setYoy(
                    (BigDecimal.valueOf(Float.parseFloat(yearAnalysisVo.getCurrentPeriod())).subtract(BigDecimal.valueOf(Float.parseFloat(yearAnalysisVo.getCorrespondingPeriod())))).multiply(
                        BigDecimal.valueOf(100)).divide(BigDecimal.valueOf(Float.parseFloat(yearAnalysisVo.getCorrespondingPeriod())), 2, RoundingMode.HALF_UP));
            }
    ...
    public List<Tree<Long>> lossAnalysis(EnergyBo bo) {
        ...
        List<ItemTopologyVo> topologyVoList = itemTopologyMapper.selectVoList(new LambdaQueryWrapper<ItemTopology>().eq(ItemTopology::getItemType, "building"));
    ...
    public List<Tree<Long>> buildTreeSelect(List<ItemTopologyVo> voList, EnergyBo bo) {
        ...
        //整理出所有的sn
        List<String> sns = equipmentInfos.stream().map(EquipmentInfo::getSn).collect(Collectors.toList());
        List<EnergyStatistics> energyStatisticsList = new ArrayList<>();
        if (sns.size() > 0) {
            //筛选计算表中的所有相关数据
            energyStatisticsList = energyStatisticsMapper.selectList(
                new LambdaQueryWrapper<EnergyStatistics>().in(EnergyStatistics::getEquipmentSn, sns).eq(StringUtils.isNotBlank(bo.getEnergyType()), EnergyStatistics::getEnergyType,
                    bo.getEnergyType()).between(StringUtils.isNotBlank(bo.getStartTime()) && StringUtils.isNotBlank(bo.getEndTime()), EnergyStatistics::getTime, bo.getStartTime(), bo.getEndTime()));
        }
        //根据拓扑逐层计算能耗
    ...
    public Map<String, Object> getChain(Long areaId, String energyType) {
        ...
        if (!(result.get("last").toString().equals("--")) && !(result.get("last").toString().equals("0.00"))) {
            result.put("nowTrend",
                BigDecimal.valueOf(Float.parseFloat(result.get("now").toString())).subtract(BigDecimal.valueOf(Float.parseFloat(result.get("last").toString()))).setScale(2, RoundingMode.HALF_UP));
            result.put("nowPer",
                BigDecimal.valueOf(Float.parseFloat(result.get("nowTrend").toString())).divide(BigDecimal.valueOf(Float.parseFloat(result.get("last").toString())), 4, RoundingMode.HALF_UP));
        }
        ...
        if (!(result.get("lastMonth").toString().equals("--")) && !(result.get("lastMonth").toString().equals("0.00"))) {
            result.put("monthTrend",
                BigDecimal.valueOf(Float.parseFloat(result.get("nowMonth").toString())).subtract(BigDecimal.valueOf(Float.parseFloat(result.get("lastMonth").toString()))).setScale(2,
                    RoundingMode.HALF_UP));
            result.put("monthPer",
                BigDecimal.valueOf(Float.parseFloat(result.get("monthTrend").toString())).divide(BigDecimal.valueOf(Float.parseFloat(result.get("lastMonth").toString())), 4, RoundingMode.HALF_UP));
        }
        //        if (!(BigDecimal.valueOf(Float.parseFloat(result.get("lastYear").toString())).stripTrailingZeros()).equals(BigDecimal.ZERO)) {
        if (!(result.get("nowYear").toString().equals("--")) && !(result.get("lastYear").toString().equals("--")) && !(result.get("lastYear").toString().equals("0.00"))) {
            result.put("yearTrend", BigDecimal.valueOf(Float.parseFloat(result.get("nowYear").toString())).subtract(BigDecimal.valueOf(Float.parseFloat(result.get("lastYear").toString()))).setScale(2,
                RoundingMode.HALF_UP));
            result.put("yearPer",
                BigDecimal.valueOf(Float.parseFloat(result.get("yearTrend").toString())).divide(BigDecimal.valueOf(Float.parseFloat(result.get("lastYear").toString())), 4, RoundingMode.HALF_UP));
        }
    ...
    public Map<String, Object> getDailyP(Long areaId, String energyType) {
        ...
        List<PowerStatistics> powerStatistics = powerStatisticsMapper.selectList(
            new LambdaQueryWrapper<PowerStatistics>().in(PowerStatistics::getEquipmentSn, deviceId).between(PowerStatistics::getTime, energyBo.getStartTime(), energyBo.getEndTime()));

        //        今日电功率峰值
        Date finalTodayStart1 = todayStart;
        List<PowerStatistics> todayVos = powerStatistics.stream().filter(e -> e.getCreateTime().after(new Timestamp(finalTodayStart1.getTime())) && e.getCreateTime().before(nowTime)).sorted(
            Comparator.comparing(PowerStatistics::getMax).reversed()).collect(Collectors.toList());
        ...
        List<PowerStatistics> yesterdayVos =
            powerStatistics.stream().filter(e -> e.getCreateTime().after(new Timestamp(finalYesterday1.getTime())) && e.getCreateTime().before(finalTodayStart1)).sorted(
                Comparator.comparing(PowerStatistics::getMax).reversed()).collect(Collectors.toList());
        ...
        for (int i = 0; i < 24; i++) {
            ...
            List<PowerStatistics> voList = powerStatistics.stream().filter(e -> e.getCreateTime().after(new Timestamp(finalYesterday.getTime())) && e.getCreateTime().before(yesterdatEnd)).sorted(
                Comparator.comparing(PowerStatistics::getMax).reversed()).collect(Collectors.toList());
            ...
            List<PowerStatistics> todayList = powerStatistics.stream().filter(e -> e.getCreateTime().after(new Timestamp(finalTodayStart.getTime())) && e.getCreateTime().before(todayEnd)).sorted(
                Comparator.comparing(PowerStatistics::getMax).reversed()).collect(Collectors.toList());
    ...
    private List<Object> getWDayByJob(Date startTime, Date endTime, Long areaId, String energyType) {
        ...
        for (int i = 0; i < 24; i++) {
            ...
            List<EnergyStatistics> list = energyStatisticsList.stream().filter(e -> e.getTime().before(endHour) && DateUtil.compare(e.getTime(), finalStartTime) >= 0).collect(Collectors.toList());
            ...
            if (ObjectUtil.isNotEmpty(list) && !list.stream().map(EnergyStatistics::getStatistics).reduce(BigDecimal.ZERO, BigDecimal::add).stripTrailingZeros().equals(BigDecimal.ZERO)) {
                result.add(list.stream().map(EnergyStatistics::getStatistics).reduce(BigDecimal.ZERO, BigDecimal::add).setScale(2, RoundingMode.HALF_UP));
            } else {
                result.add("--");
            }
    ...
    private List<Object> getWMonthByJob(Date startTime, Date endTime, Long areaId, String energyType) {
        ...
        for (int i = 0; i < day; i++) {
            ...
            List<EnergyStatistics> list = energyStatisticsList.stream().filter(e -> e.getTime().before(endDate) && e.getTime().after(finalStartDate)).collect(Collectors.toList());
    ...
    private List<Object> getWYearByJob(Date startTime, Date endTime, Long areaId, String energyType) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            Date finalStartTime = startTime;
            List<EnergyStatistics> list = energyStatisticsList.stream().filter(e -> e.getTime().before(endDate) && DateUtil.compare(e.getTime(), finalStartTime) >= 0).collect(Collectors.toList());

            if (ObjectUtil.isNotEmpty(list)) {
                result.add(list.stream().map(EnergyStatistics::getStatistics).reduce(BigDecimal.ZERO.stripTrailingZeros(), BigDecimal::add).setScale(2, RoundingMode.HALF_UP));
            } else {
                result.add("--");
            }
    ...
    public Map<String, Object> getChain() {
        ...
        // 趋势
        result.put("yearTrend",
            BigDecimal.valueOf(Float.parseFloat(result.get("nowYear").toString())).subtract(BigDecimal.valueOf(Float.parseFloat(result.get("lastYear").toString()))).setScale(3, RoundingMode.HALF_UP));
        result.put("monthTrend", BigDecimal.valueOf(Float.parseFloat(result.get("nowMonth").toString())).subtract(BigDecimal.valueOf(Float.parseFloat(result.get("lastMonth").toString()))).setScale(3,
            RoundingMode.HALF_UP));
    ...
    public Map<String, Object> getReportChart(Long areaId, String startTime, String endTime, String energyType) {
        ...
        for (int i = 0; i < day + 1; i++) {
            ...
            List<EnergyStatisticsVo> voList = energyVos.stream().filter(e -> e.getTime().before(finish) && e.getTime().after(begin)).collect(Collectors.toList());
    ...
    public List<ReportTableVo> getReportTable(Long areaId, String startTime, String endTime) {
        ...
        if (!lastElectricity.get("total").equals(BigDecimal.ZERO)) {
            chainElectricity =
                (((BigDecimal) nowElectricity.get("total")).subtract((BigDecimal) lastElectricity.get("total"))).multiply(BigDecimal.valueOf(100)).divide((BigDecimal) lastElectricity.get("total"), 2,
                    RoundingMode.HALF_UP);
        }
        //同期用电量
        Map<String, Object> sameElectricity =
            getReportChart(areaId, DateUtils.parseDateToStr(DateUtils.YYYY_MM_DD_HH_MM_SS, sameStartTime), DateUtils.parseDateToStr(DateUtils.YYYY_MM_DD_HH_MM_SS, sameEndTime), "0");
        //同比
        BigDecimal yoyElectricity = BigDecimal.ZERO;
        if (!sameElectricity.get("total").equals(BigDecimal.ZERO)) {
            yoyElectricity =
                (((BigDecimal) nowElectricity.get("total")).subtract((BigDecimal) sameElectricity.get("total"))).multiply(BigDecimal.valueOf(100)).divide((BigDecimal) sameElectricity.get("total"), 2,
                    RoundingMode.HALF_UP);
        }
        ...
        if (!lastWater.get("total").equals(BigDecimal.ZERO)) {
            chainWater = (((BigDecimal) nowWater.get("total")).subtract((BigDecimal) lastWater.get("total"))).multiply(BigDecimal.valueOf(100)).divide((BigDecimal) lastWater.get("total"), 2,
                RoundingMode.HALF_UP);
        }
        //同期用水量
        Map<String, Object> sameWater =
            getReportChart(areaId, DateUtils.parseDateToStr(DateUtils.YYYY_MM_DD_HH_MM_SS, sameStartTime), DateUtils.parseDateToStr(DateUtils.YYYY_MM_DD_HH_MM_SS, sameEndTime), "1");
        //同比
        BigDecimal yoyWater = BigDecimal.ZERO;
        if (!sameWater.get("total").equals(BigDecimal.ZERO)) {
            yoyWater = (((BigDecimal) nowWater.get("total")).subtract((BigDecimal) sameWater.get("total"))).multiply(BigDecimal.valueOf(100)).divide((BigDecimal) sameWater.get("total"), 2,
                RoundingMode.HALF_UP);
        }

        //本期综合能耗
        BigDecimal nowEnergy = ((BigDecimal) nowElectricity.get("total")).divide(BigDecimal.valueOf(8.167), 2, RoundingMode.HALF_UP).add(
            ((BigDecimal) nowWater.get("total")).multiply(BigDecimal.valueOf(0.4857)).setScale(2, RoundingMode.HALF_UP));
        //上期综合能耗
        BigDecimal lastEnergy = ((BigDecimal) lastElectricity.get("total")).divide(BigDecimal.valueOf(8.167), 2, RoundingMode.HALF_UP).add(
            ((BigDecimal) lastWater.get("total")).multiply(BigDecimal.valueOf(0.4857)).setScale(2, RoundingMode.HALF_UP));
        //环比
        BigDecimal chainEnergy = BigDecimal.ZERO;
        if (lastEnergy.compareTo(BigDecimal.ZERO) != 0) {
            chainEnergy = (nowEnergy.subtract(lastEnergy)).multiply(BigDecimal.valueOf(100)).divide(lastEnergy, 2, RoundingMode.HALF_UP);
        }
        //同期综合能耗
        BigDecimal sameEnergy = ((BigDecimal) sameElectricity.get("total")).divide(BigDecimal.valueOf(8.167), 2, RoundingMode.HALF_UP).add(
            ((BigDecimal) sameWater.get("total")).multiply(BigDecimal.valueOf(0.4857)).setScale(2, RoundingMode.HALF_UP));
        //同比
        BigDecimal yoyEnergy = BigDecimal.ZERO;
        if (sameEnergy.compareTo(BigDecimal.ZERO) != 0) {
            yoyEnergy = (nowEnergy.subtract(sameEnergy)).multiply(BigDecimal.valueOf(100)).divide(sameEnergy, 2, RoundingMode.HALF_UP);
        }

        //本期碳排放量
        BigDecimal nowCarbon = ((BigDecimal) nowElectricity.get("total")).multiply(BigDecimal.valueOf(0.785)).setScale(2, RoundingMode.HALF_UP).add(
            ((BigDecimal) nowWater.get("total")).multiply(BigDecimal.valueOf(0.91)).setScale(2, RoundingMode.HALF_UP));
        //上期碳排放量
        BigDecimal lastCarbon = ((BigDecimal) lastElectricity.get("total")).multiply(BigDecimal.valueOf(0.785)).setScale(2, RoundingMode.HALF_UP).add(
            ((BigDecimal) lastWater.get("total")).multiply(BigDecimal.valueOf(0.91)).setScale(2, RoundingMode.HALF_UP));
        //环比
        BigDecimal chainCarbon = BigDecimal.ZERO;
        if (lastCarbon.compareTo(BigDecimal.ZERO) != 0) {
            chainCarbon = (nowCarbon.subtract(lastCarbon)).multiply(BigDecimal.valueOf(100)).divide(lastCarbon, 2, RoundingMode.HALF_UP);
        }
        //同期碳排放量
        BigDecimal sameCarbon = ((BigDecimal) sameElectricity.get("total")).multiply(BigDecimal.valueOf(0.785)).setScale(2, RoundingMode.HALF_UP).add(
            ((BigDecimal) sameWater.get("total")).multiply(BigDecimal.valueOf(0.91)).setScale(2, RoundingMode.HALF_UP));
        //环比
        BigDecimal yoyCarbon = BigDecimal.ZERO;
        if (sameCarbon.compareTo(BigDecimal.ZERO) != 0) {
            yoyCarbon = (nowCarbon.subtract(sameCarbon)).multiply(BigDecimal.valueOf(100)).divide(sameCarbon, 2, RoundingMode.HALF_UP);
        }
    ...
    public Boolean statisticsEnergyByHour(String energyType) {
        // 获取前一个小时的完整时间范围（0分0秒到59分59秒）
        Date now = new Date();
        Date lastHourStart = DateUtils.addHours(now, -1);
        String startTime = DateUtils.parseDateToStr("yyyy-MM-dd HH:00:00", lastHourStart);
        String endTime = DateUtils.parseDateToStr("yyyy-MM-dd HH:59:59.999", lastHourStart);

        // 查询所有设备
        List<EquipmentInfoVo> equipmentInfoVos = equipmentInfoMapper.selectVoList(new LambdaQueryWrapper<EquipmentInfo>().eq(StringUtils.isNotBlank(energyType), EquipmentInfo::getType, energyType));

        if (ObjectUtil.isEmpty(equipmentInfoVos)) {
            return Boolean.TRUE;
        }

        List<EnergyVo> nowElectricity = new ArrayList<>();
        // 能源类型
        String tableName = EnergyType.ELECTRICITY.getInfo();
        if (energyType.equals(EnergyType.ELECTRICITY.getCode())) {
            tableName = EnergyType.ELECTRICITY.getInfo();

            // 不启用TDengine数据库，直接走MySQL数据库
            if (!tdDbEnabled) {
                // 同mysql数据库获取数据
                List<ElectricityWVo> electricityWVoList =
                    electricityWMapper.selectVoList(new LambdaQueryWrapper<ElectricityW>().between(ElectricityW::getCreateTime, startTime, endTime).orderByDesc(ElectricityW::getCreateTime));
                for (ElectricityWVo vo : electricityWVoList) {
                    EnergyVo energyVo = new EnergyVo();
                    energyVo.setType(tableName);
                    energyVo.setClientId(vo.getClientId());
                    energyVo.setVal(Float.parseFloat(vo.getValue().toString()));
                    energyVo.setTs(DateUtil.date(vo.getCreateTime()).toTimestamp());
                    nowElectricity.add(energyVo);
                }
            }
            ...
            if (!tdDbEnabled) {
                // 同mysql数据库获取数据
                List<WaterConsumptionVo> WaterConsumptionVoList =
                    waterConsumptionMapper.selectVoList(
                        new LambdaQueryWrapper<WaterConsumption>().between(WaterConsumption::getCreateTime, startTime, endTime).orderByDesc(WaterConsumption::getCreateTime));
                for (WaterConsumptionVo vo : WaterConsumptionVoList) {
                    EnergyVo energyVo = new EnergyVo();
                    energyVo.setType(tableName);
                    energyVo.setClientId(vo.getClientId());
                    energyVo.setVal(Float.parseFloat(vo.getValue().toString()));
        ...
        // 启用TDengine数据库
        if (tdDbEnabled) {
            // 查询td表中当前一小时数据：ts 字段为时间戳，val 为累计值。
            // 注意 val 为累计值
            nowElectricity = energyMapper.selectVoList(new LambdaQueryWrapper<Energy>().eq(Energy::getType, tableName).between(Energy::getTs, startTime, endTime).orderByDesc(Energy::getTs));
        }

        // 查询统计表中当前一小时的数据
        List<EnergyStatistics> energyStatisticsList =
            energyStatisticsMapper.selectList(new LambdaQueryWrapper<EnergyStatistics>().eq(EnergyStatistics::getEnergyType, energyType).between(EnergyStatistics::getTime, startTime, endTime));

        // 新增、修改列表
        List<EnergyStatistics> updateList = new ArrayList<>();

        // 统计每台设备当前一小时能耗，能耗为val累加值计算获得，计算能耗差值：最后一条数据减最早一条数据
        // dongbei 2025-9-16  调整能耗差值的计算逻辑，当最后一条数据的值为 0 或小于第一条数据时，依次检查前面的数据，直到找到合适的值进行计算，避免出现负值
        for (EquipmentInfoVo equipmentInfoVo : equipmentInfoVos) {
            List<EnergyVo> energyVos = nowElectricity.stream()
                .filter(e -> e.getClientId().equals(equipmentInfoVo.getSn()))
                .sorted(Comparator.comparing(EnergyVo::getTs))
                .collect(Collectors.toList());

            BigDecimal statistics = BigDecimal.ZERO;
            if (ObjectUtil.isNotEmpty(energyVos) && energyVos.size() > 1) {
                // 获取第一条数据的值
                double firstVal = energyVos.get(0).getVal();
                if (firstVal < 0) {
                    firstVal = 0;
                }
                ...
                // 从最后一条数据开始向前查找有效的值
                for (int i = energyVos.size() - 1; i > 0; i--) {
                    lastVal = energyVos.get(i).getVal();
                    if (lastVal < 0) {
                        lastVal = 0;
                    }
                    // 找到第一个大于等于第一条数据的值，避免最后一条数据小于第一条数据出现计算结果小于0的情况
                    if (lastVal >= firstVal) {
                        foundValid = true;
                        break;
                    }
                ...
                // 如果找到有效数据则计算差值，否则保持为0
                if (foundValid) {
                    statistics = BigDecimal.valueOf(lastVal - firstVal);
                }
            }

            List<EnergyStatistics> statisticsList = energyStatisticsList.stream()
                .filter(e -> e.getEquipmentSn().equals(equipmentInfoVo.getSn()))
                .collect(Collectors.toList());
        ...
        //     }
        // }
        logger.info("[获取]-定时任务-统计当前一小时能耗：{}", JSONUtil.toJsonStr(updateList));
    ...
    public Boolean statisticsPowerByHour(String energyType) {
        ...
        if (!tdDbEnabled) {
            // 同mysql数据库获取数据
            List<ElectricityPVo> electricityList =
                electricityPMapper.selectVoList(new LambdaQueryWrapper<ElectricityP>().between(ElectricityP::getCreateTime, startTime, endTime).orderByDesc(ElectricityP::getCreateTime));
            for (ElectricityPVo vo : electricityList) {
                EnergyVo energyVo = new EnergyVo();
                energyVo.setType(tableName);
                energyVo.setClientId(vo.getClientId());
                energyVo.setVal(Float.parseFloat(vo.getValue().toString()));
        ...
        // 查询统计表中当前一小时的数据
        List<PowerStatistics> powerStatisticsList =
            powerStatisticsMapper.selectList(new LambdaQueryWrapper<PowerStatistics>().eq(PowerStatistics::getEnergyType, energyType).between(PowerStatistics::getTime, startTime, endTime));
        ...
        logger.info("[获取]-定时任务-统计当前一小时电功率峰谷值平均值：{}", JSONUtil.toJsonStr(updateList));
    ...
    public Boolean statisticsCurrentByHour(String energyType) {
        ...
        if (!tdDbEnabled) {
            // 同mysql数据库获取数据
            List<ElectricityIVo> electricityList =
                electricityIMapper.selectVoList(new LambdaQueryWrapper<ElectricityI>().between(ElectricityI::getCreateTime, startTime, endTime).orderByDesc(ElectricityI::getCreateTime));
            for (ElectricityIVo vo : electricityList) {
                EnergyVo energyVo = new EnergyVo();
                energyVo.setType(tableName);
                energyVo.setClientId(vo.getClientId());
                energyVo.setVal(Float.parseFloat(vo.getValue().toString()));
        ...
        // 查询统计表中当前一小时的数据
        List<CurrentStatistics> currentStatisticsList =
            currentStatisticsMapper.selectList(new LambdaQueryWrapper<CurrentStatistics>().eq(CurrentStatistics::getEnergyType, energyType).between(CurrentStatistics::getTime, startTime, endTime));
        ...
        logger.info("[获取]-定时任务-统计当前一小时电流峰谷值平均值：{}", JSONUtil.toJsonStr(updateList));
    ...
    public Boolean statisticsVoltageByHour(String energyType) {
        ...
        if (!tdDbEnabled) {
            // 同mysql数据库获取数据
            List<ElectricityUVo> electricityList =
                electricityUMapper.selectVoList(new LambdaQueryWrapper<ElectricityU>().between(ElectricityU::getCreateTime, startTime, endTime).orderByDesc(ElectricityU::getCreateTime));
            for (ElectricityUVo vo : electricityList) {
                EnergyVo energyVo = new EnergyVo();
                energyVo.setType(tableName);
                energyVo.setClientId(vo.getClientId());
                energyVo.setVal(Float.parseFloat(vo.getValue().toString()));
        ...
        // 查询统计表中当前一小时的数据
        List<VoltageStatistics> voltageStatisticsList =
            voltageStatisticsMapper.selectList(new LambdaQueryWrapper<VoltageStatistics>().eq(VoltageStatistics::getEnergyType, energyType).between(VoltageStatistics::getTime, startTime, endTime));
        ...
        logger.info("[获取]-定时任务-统计当前一小时电压峰谷值平均值-回写数据=[{}]", JSONUtil.toJsonStr(updateList));
    ...
    public BigDecimal getAccumulate(Long itemId, String quotaType, Date quotaTime) {
        ...
        List<EnergyStatistics> energyStatistics =
            energyStatisticsMapper.selectList(new LambdaQueryWrapper<EnergyStatistics>().between(EnergyStatistics::getTime, quotaTime, endTime).in(EnergyStatistics::getEquipmentSn, devices));
    ...
    public List<EnergyVo> queryPowerParameter(Long areaId, String startTime, String endTime, String energyType) {
        ...
        EquipmentInfo equipmentInfo =
            equipmentInfoMapper.selectOne(new LambdaQueryWrapper<EquipmentInfo>().in(EquipmentInfo::getSn, equip).eq(EquipmentInfo::getType, EnergyType.ELECTRICITY.getCode()).last("limit 1"));
        ...
        if (tdDbEnabled) {
            // 查询td表中相关设备数据
            nowElectricity = energyMapper.selectVoList(new LambdaQueryWrapper<Energy>()
        ...
        if (!tdDbEnabled) {
            if (energyType.equals(EnergyType.ELECTRICITYU.getInfo())) {
                List<ElectricityUVo> electricityList = electricityUMapper.selectVoList(new LambdaQueryWrapper<ElectricityU>()
                    .eq(ElectricityU::getClientId, equipmentInfo.getSn())
                    .between(ElectricityU::getCreateTime, startTime, endTime));
                for (ElectricityUVo vo : electricityList) {
                    EnergyVo energyVo = new EnergyVo();
                    energyVo.setType(energyType);
                    energyVo.setClientId(vo.getClientId());
                    energyVo.setVal(Float.parseFloat(vo.getValue().toString()));
                    energyVo.setTs(DateUtil.date(vo.getCreateTime()).toTimestamp());
                    nowElectricity.add(energyVo);
            ...
            if (energyType.equals(EnergyType.ELECTRICITYI.getInfo())) {
                List<ElectricityIVo> electricityList = electricityIMapper.selectVoList(new LambdaQueryWrapper<ElectricityI>()
                    .eq(ElectricityI::getClientId, equipmentInfo.getSn())
                    .between(ElectricityI::getCreateTime, startTime, endTime));
                for (ElectricityIVo vo : electricityList) {
                    EnergyVo energyVo = new EnergyVo();
                    energyVo.setType(energyType);
                    energyVo.setClientId(vo.getClientId());
            ...
            if (energyType.equals(EnergyType.ELECTRICITYP.getInfo())) {
                List<ElectricityPVo> electricityList = electricityPMapper.selectVoList(new LambdaQueryWrapper<ElectricityP>()
                    .eq(ElectricityP::getClientId, equipmentInfo.getSn())
                    .between(ElectricityP::getCreateTime, startTime, endTime));
                for (ElectricityPVo vo : electricityList) {
                    EnergyVo energyVo = new EnergyVo();
                    energyVo.setType(energyType);
                    energyVo.setClientId(vo.getClientId());
    ...
    public List<PowerStatisticsVo> queryDailyExtremum(Long areaId, String startTime, String endTime, String energyType) {
        ...
        if (energyType.equals(EnergyType.ELECTRICITYP.getInfo())) {
            // 筛选所选时间范围内该设备的中间计算结果
            List<PowerStatistics> all = powerStatisticsMapper.selectList(
                new LambdaQueryWrapper<PowerStatistics>().eq(PowerStatistics::getEquipmentSn, equipmentInfo.getSn()).between(PowerStatistics::getCreateTime, start, DateUtils.addDays(end, 1)));
            while (start.before(end) || start.equals(end)) {
                ...
                if (ObjectUtil.isNotEmpty(temp)) {
                    ...
                    //求平均值
                    average =
                        temp.stream().map(vo -> ObjectUtil.isEmpty(vo.getAve()) ? new BigDecimal(0) : vo.getAve()).reduce(BigDecimal.ZERO, BigDecimal::add).divide(BigDecimal.valueOf(temp.size()), 2,
        ...
        if (energyType.equals(EnergyType.ELECTRICITYI.getInfo())) {
            // 筛选所选时间范围内该设备的中间计算结果
            List<CurrentStatistics> all = currentStatisticsMapper.selectList(
                new LambdaQueryWrapper<CurrentStatistics>().eq(CurrentStatistics::getEquipmentSn, equipmentInfo.getSn()).between(CurrentStatistics::getCreateTime, start, DateUtils.addDays(end, 1)));
            while (start.before(end) || start.equals(end)) {
                Date tempEnd = DateUtils.addDays(start, 1);
                PowerStatisticsVo ps = new PowerStatisticsVo();
                Date finalStart = start;
        ...
        if (energyType.equals(EnergyType.ELECTRICITYU.getInfo())) {
            // 筛选所选时间范围内该设备的中间计算结果
            List<VoltageStatistics> all = voltageStatisticsMapper.selectList(
                new LambdaQueryWrapper<VoltageStatistics>().eq(VoltageStatistics::getEquipmentSn, equipmentInfo.getSn()).between(VoltageStatistics::getCreateTime, start, DateUtils.addDays(end, 1)));
            while (start.before(end) || start.equals(end)) {
                Date tempEnd = DateUtils.addDays(start, 1);
                PowerStatisticsVo ps = new PowerStatisticsVo();
                Date finalStart = start;
    ...
    public Map<String, Object> getReportChargingChart(Long areaId, String startTime, String endTime, String energyType) {
        ...
        //获取最新的计费规则
        ChargingVo chargingVo = chargingMapper.selectVoOne(
            new LambdaQueryWrapper<Charging>().eq(Charging::getStatus, "0").eq(Charging::getType, energyType).le(Charging::getStartDate, DateUtils.getNowDate()).ge(Charging::getEndDate,
                DateUtils.getNowDate()).last("limit 1"));
        ...
        //获取阶梯计费开启时所有的阶梯计费规则
        List<ChargingStep> chargingSteps =
            chargingStepMapper.selectList(new LambdaQueryWrapper<ChargingStep>().eq(ChargingStep::getChargingId, chargingVo.getChargingId()).orderByAsc(ChargingStep::getStartStep));
...
/*        //最大值
    ...
    private BigDecimal getStepCharging(BigDecimal total, List<ChargingStep> chargingSteps, BigDecimal unitPrice) {
        ...
        for (ChargingStep step : chargingSteps) {
            if (total.intValue() < step.getStartStep()) {
                price = price.add(total.multiply(unitPrice).setScale(2, RoundingMode.HALF_UP));
                break;
            }
            if (total.intValue() > step.getStartStep() && total.intValue() < step.getEndStep()) {
                price = price.add(total.subtract(BigDecimal.valueOf(step.getStartStep())).multiply(unitPrice.add(step.getPriceDifference())).setScale(2, RoundingMode.HALF_UP));
                break;
            }
    ...
    public List<ReportTableVo> getRecurringRate(Long areaId, String startTime, String endTime, String energyType) {
        ...
        if (!(lastSharp.equals(BigDecimal.ZERO))) {
            sharpReportTable.setChain(currentSharp.subtract(lastSharp).multiply(BigDecimal.valueOf(100)).divide(lastSharp, 2, RoundingMode.HALF_UP));
        }
        ...
        if (!(lastPeek.equals(BigDecimal.ZERO))) {
            peekReportTable.setChain(currentPeek.subtract(lastPeek).multiply(BigDecimal.valueOf(100)).divide(lastPeek, 2, RoundingMode.HALF_UP));
        }
        ...
        if (!(lastOrdinary.equals(BigDecimal.ZERO))) {
            ordinaryReportTable.setChain(currentOrdinary.subtract(lastOrdinary).multiply(BigDecimal.valueOf(100)).divide(lastOrdinary, 2, RoundingMode.HALF_UP));
        }
        ...
        if (!(lastValley.equals(BigDecimal.ZERO))) {
            valleyReportTable.setChain(currentValley.subtract(lastValley).multiply(BigDecimal.valueOf(100)).divide(lastValley, 2, RoundingMode.HALF_UP));
        }
    ...
    private BigDecimal getPeriodStatisticAll(String value, List<EnergyStatisticsVo> energyVos) {
        ...
        for (String temp : values) {
            ...
            List<EnergyStatisticsVo> voList = energyVos.stream().filter(e -> DateUtils.dateTime(DateUtils.HH_MM_SS, DateUtils.parseDateToStr(DateUtils.HH_MM_SS, e.getCreateTime())).after(
                DateUtils.dateTime(DateUtils.HH_MM_SS, dates.get(0))) && DateUtils.dateTime(DateUtils.HH_MM_SS, DateUtils.parseDateToStr(DateUtils.HH_MM_SS, e.getCreateTime())).before(
                DateUtils.dateTime(DateUtils.HH_MM_SS, dates.get(1)))).collect(Collectors.toList());
    ...
    public List<ExpenseReportVo> getConsumptionExpenseReport(Collection<Long> areaIds, String dateType, String startTime, String endTime, String energyType) {
        ...
        //获取阶梯计费开启时所有的阶梯计费规则
        List<ChargingStep> chargingSteps = new ArrayList<>();
        if (null != chargingVo) {
            chargingSteps = chargingStepMapper.selectList(new LambdaQueryWrapper<ChargingStep>().eq(ChargingStep::getChargingId, chargingVo.getChargingId()).orderByAsc(ChargingStep::getStartStep));
            ...
            if (dateType.equals("date")) {
                for (ItemTopology itemTopology : itemTopologys) {
                    ...
                    for (int i = 0; i < 24; i++) {
                        ...
                        List<EnergyStatisticsVo> voList =
                            energyVos.stream().filter(e -> DeviceIdMatcher.containsDevice(itemTopology.getDeviceId(), e.getEquipmentSn()) && DateUtils.dateTime(DateUtils.HH_MM_SS,
                                DateUtils.parseDateToStr(DateUtils.HH_MM_SS, e.getCreateTime())).after(startTemp) && DateUtils.dateTime(DateUtils.HH_MM_SS,
                                DateUtils.parseDateToStr(DateUtils.HH_MM_SS, e.getCreateTime())).before(endTemp)).collect(Collectors.toList());
            ...
            if (dateType.equals("week")) {
                for (ItemTopology itemTopology : itemTopologys) {
                    ...
                    for (int i = 0; i < 7; i++) {
                        ...
                        ExpenseReportVo reportTemp = new ExpenseReportVo();
                        List<EnergyStatisticsVo> voList =
                            energyVos.stream().filter(
                                e -> DeviceIdMatcher.containsDevice(itemTopology.getDeviceId(), e.getEquipmentSn()) && e.getCreateTime().after(startTemp) && e.getCreateTime().before(endTemp)).collect(
                                Collectors.toList());
    ...
    private BigDecimal getFinalCharging(ChargingVo chargingVo, List<ChargingStep> chargingSteps, List<EnergyStatisticsVo> voList, String sharp, String peek, String ordinary, String valley) {
        ...
        if (chargingVo.getPeriodSwitch().equals("0") && chargingVo.getStepSwitch().equals("1")) {
            if (ObjectUtil.isNotEmpty(voList)) {
                ...
                return (sharpValue.multiply(chargingVo.getSharpPrice())).add(peekValue.multiply(chargingVo.getPeekPrice())).add(ordinaryValue.multiply(chargingVo.getOrdinaryPrice())).add(
                    valleyValue.multiply(chargingVo.getValleyPrice())).setScale(2, RoundingMode.HALF_UP);
        ...
        if (chargingVo.getPeriodSwitch().equals("1") && chargingVo.getStepSwitch().equals("0")) {
            if (ObjectUtil.isNotEmpty(voList)) {
                BigDecimal total =
                    voList.stream().map(EnergyStatisticsVo::getStatistics).reduce(BigDecimal.ZERO, BigDecimal::add).multiply(chargingVo.getUnitPrice()).setScale(2, RoundingMode.HALF_UP);
        ...
        if (chargingVo.getPeriodSwitch().equals("0") && chargingVo.getStepSwitch().equals("0")) {
            if (ObjectUtil.isNotEmpty(voList)) {
                ...
                BigDecimal ordinaryValue = getPeriodStatisticAll(ordinary, voList);
                BigDecimal valleyValue = getPeriodStatisticAll(valley, voList);
                return getStepCharging(sharpValue, chargingSteps, chargingVo.getSharpPrice()).add(getStepCharging(peekValue, chargingSteps, chargingVo.getPeekPrice())).add(
                    getStepCharging(ordinaryValue, chargingSteps, chargingVo.getOrdinaryPrice())).add(getStepCharging(valleyValue, chargingSteps, chargingVo.getValleyPrice())).setScale(2,
                    RoundingMode.HALF_UP);
        ...
        for (int i = 0; i < 24; i++) {
            ...
            if (ObjectUtil.isNotEmpty(list)) {
                BigDecimal electricityTotal =
                    list.stream().filter(e -> e.getEnergyType().equals("0")).map(EnergyStatistics::getStatistics).reduce(BigDecimal.ZERO, BigDecimal::add).setScale(2, RoundingMode.HALF_UP);
                BigDecimal waterTotal =
                    list.stream().filter(e -> e.getEnergyType().equals("1")).map(EnergyStatistics::getStatistics).reduce(BigDecimal.ZERO, BigDecimal::add).setScale(2, RoundingMode.HALF_UP);
