// English Language Pack
export default {
  // Common
  common: {
    search: 'Search',
    reset: 'Reset',
    submit: 'Submit',
    cancel: 'Cancel',
    confirm: 'Confirm',
    delete: 'Delete',
    edit: 'Edit',
    add: 'Add',
    view: 'View',
    export: 'Export',
    import: 'Import',
    download: 'Download',
    upload: 'Upload',
    operate: 'Action',
    status: 'Status',
    enable: 'Enable',
    disable: 'Disable',
    yes: 'Yes',
    no: 'No',
    save: 'Save',
    close: 'Close',
    back: 'Back',
    refresh: 'Refresh',
    loading: 'Loading...',
    success: 'Operation successful',
    error: 'Operation failed',
    warning: 'Warning',
    info: 'Info',
    pleaseSelect: 'Please select',
    pleaseInput: 'Please input',
    startDate: 'Start Date',
    endDate: 'End Date',
    startTime: 'Start Time',
    endTime: 'End Time',
    createTime: 'Create Time',
    updateTime: 'Update Time',
    remark: 'Remark',
    action: 'Action',
    more: 'More',
    all: 'All',
    selectAll: 'Select All',
    empty: 'No data',
    total: 'Total {total} items',
    pageSize: 'items/page',
    goTo: 'Go to',
    page: 'page',
    index: 'Index',
    operator: 'Operator',
    creator: 'Creator',
    updater: 'Updater',
    deleter: 'Deleter',
    owner: 'Owner',
    department: 'Department',
    deleteFlag: 'Delete Flag',
    deleteTime: 'Delete Time',
    submitEdit: 'Submit Edit',
    clear: 'Clear',
    publishTime: 'Publish Time',
    systemBuiltIn: 'System Built-in',
    loginInfo: 'Login Info',
    operInfo: 'Operation Info',
    normal: 'Normal',
    failed: 'Failed',
    completed: 'Completed',
    inProgress: 'In Progress',
    standalone: 'Standalone',
    cluster: 'Cluster',
    refreshCache: 'Refresh Cache',
    selectDate: 'Select Date',
    systemTitle: 'EMS Platform',
    // Right Toolbar
    showSearch: 'Show Search',
    hideSearch: 'Hide Search',
    showHideColumns: 'Show/Hide Columns',
    showHideTitle: 'Show/Hide',
    show: 'Show',
    hide: 'Hide',
    // Cron Builder
    crontab: {
      second: 'Second',
      minute: 'Minute',
      hour: 'Hour',
      day: 'Day',
      month: 'Month',
      week: 'Week',
      year: 'Year',
      timeExpression: 'Time Expression',
      cronExpression: 'Cron Expression'
    },
    moreMenu: 'More Menus',
    // File Upload
    upload: {
      selectFile: 'Select File',
      pleaseUpload: 'Please upload',
      sizeLimit: 'Size not exceed',
      formatLimit: 'Format is',
      fileSuffix: 'file',
      uploading: 'Uploading file, please wait...',
      uploadFailed: 'Upload failed, please retry',
      fileTypeError: 'Invalid file format',
      fileSizeError: 'File size cannot exceed'
    }
  },

  // Login
  login: {
    title: 'Zhurong Energy Management System',
    username: 'Username',
    password: 'Password',
    captcha: 'Captcha',
    remember: 'Remember me',
    login: 'Login',
    logging: 'Logging in...',
    usernameRequired: 'Please input username',
    passwordRequired: 'Please input password',
    captchaRequired: 'Please input captcha',
    loginSuccess: 'Login successful',
    loginError: 'Login failed',
    logout: 'Logout',
    logoutConfirm: 'Are you sure to logout?'
  },

  // Menu
  menu: {
    system: {
      title: 'System Management',
      user: 'User Management',
      role: 'Role Management',
      menu: 'Menu Management',
      dept: 'Department Management',
      post: 'Post Management',
      dict: 'Dictionary Management',
      config: 'Config Management',
      notice: 'Notice',
      log: {
        title: 'Log Management',
        operlog: 'Operation Log',
        logininfor: 'Login Log'
      }
    },
    monitor: {
      title: 'System Monitor',
      online: 'Online Users',
      job: 'Scheduled Tasks',
      druid: 'Data Monitor',
      server: 'Server Monitor',
      cache: 'Cache Monitor',
      redis: 'Redis Monitor'
    },
    tool: {
      title: 'System Tools',
      build: 'Form Builder',
      gen: 'Code Generator',
      swagger: 'API Docs',
      icon: 'Icon Library'
    },
    energy: {
      title: 'Energy Management',
      overview: 'Energy Overview',
      analysis: 'Energy Analysis',
      report: 'Energy Report',
      quota: 'Energy Quota',
      plan: 'Energy Plan'
    },
    equipment: {
      title: 'Equipment Management',
      list: 'Equipment List',
      type: 'Equipment Type',
      status: 'Equipment Status',
      maintenance: 'Maintenance',
      repair: 'Repair Management'
    },
    alarm: {
      title: 'Alarm Management',
      current: 'Real-time Alarm',
      history: 'Alarm History',
      rule: 'Alarm Rules',
      level: {
        general: 'General',
        urgent: 'Urgent',
        serious: 'Serious'
      }
    },
    camera: {
      title: 'Video Monitoring',
      realtime: 'Real-time Video',
      playback: 'Video Playback',
      config: 'Camera Config'
    },
    charging: {
      title: 'Charging Station',
      station: 'Charging Station',
      pile: 'Charging Pile',
      order: 'Charging Order',
      price: 'Pricing Strategy'
    },
    inspection: {
      title: 'Inspection Management',
      plan: 'Inspection Plan',
      task: 'Inspection Task',
      record: 'Inspection Record',
      point: 'Inspection Point'
    },
    itemized: {
      title: 'Itemized Metering',
      topology: 'Topology Management',
      analysis: 'Itemized Analysis'
    },
    dataQuery: {
      title: 'Data Query',
      realtime: 'Real-time Data',
      history: 'Historical Data',
      statistics: 'Statistics'
    },
    profile: {
      title: 'Profile',
      info: 'Personal Info',
      password: 'Change Password'
    },
    control: {
      title: 'Centralized Control',
      control: 'Centralized Control'
    },
    metering: {
      title: 'Metering Management',
      metering: 'Metering Management'
    },
    carbon: {
      title: 'Carbon Asset Management',
      analysis: 'Carbon Analysis'
    },
    energyAnalysis: {
      title: 'Energy Analysis',
      energyOverview: 'Energy Overview',
      energyFlow: 'Energy Flow',
      energyTrend: 'Energy Trend',
      yoyAnalysis: 'YoY Analysis',
      monAnalysis: 'MoM Analysis',
      lossAnalysis: 'Loss Analysis',
      expenseBoard: 'Expense Board',
      expenseReport: 'Expense Report'
    },
    digitaltwin: {
      title: 'Digital Twin',
      digitaltwin: 'Digital Twin'
    },
    inventory: {
      title: 'Inventory Management',
      attachment: 'Attachment Management',
      purveyor: 'Supplier Management'
    },
    managementSystem: {
      title: 'Management System',
      prePlan: 'Pre-plan Management',
      process: 'Process Management',
      regulation: 'Regulation Management',
      standard: 'Standard Management'
    },
    maintenance: {
      title: 'Maintenance Management',
      duty: 'Duty Management',
      inspectionPlan: 'Inspection Plan',
      inspectionRecord: 'Inspection Record',
      repairOrder: 'Repair Order',
      schedule: 'Schedule Management',
      exampleReport: 'Example Report',
      myInspection: 'My Inspection',
      myRepairOrder: 'My Order',
      itemTopology: 'Equipment Topology'
    },
    quota: {
      title: 'Quota Management',
      analysis: 'Quota Analysis',
      config: 'Quota Config',
      monitor: 'Quota Monitor'
    },
    report: {
      title: 'Report Management',
      generate: 'Report Generate',
      template: 'Report Template'
    },
    newenergy: {
      title: 'New Energy',
      energyStorage: 'Energy Storage',
      microGrid: 'Micro Grid',
      pvStation: 'PV Station',
      storageBattery: 'Storage Battery'
    },
    analysisReport: {
      title: 'Analysis Report',
      analysisReport: 'Analysis Report'
    },
    dataQuery: {
      title: 'Data Query',
      electricParams: 'Electric Parameters'
    },
    dataBoard: {
      title: 'Data Board',
      dataBoard: 'Data Board'
    },
    dashboard: {
      title: 'Dashboard',
      dashboard: 'Dashboard'
    },
    visualization3D: {
      title: '3D Visualization',
      visualization3D: '3D Visualization'
    },
    business: {
      title: 'Business Management',
      business: 'Business Management'
    },
    item: {
      title: 'Item Management',
      item: 'Item Management'
    },
    operation: {
      title: 'Operation Management',
      operation: 'Operation Management'
    },
    alarmManage: {
      title: 'Alarm Management',
      realtime: 'Real-time Alarm',
      history: 'History Alarm',
      rule: 'Alarm Rules',
      analysis: 'Alarm Analysis'
    },
    newEnergyManage: {
      title: 'New Energy Management',
      energyStorage: 'Energy Storage',
      microGrid: 'Microgrid',
      pvStation: 'PV Station',
      storageBattery: 'Storage Battery'
    },
    managementSystem2: {
      title: 'Management System',
      prePlan: 'Pre-plan',
      process: 'Process',
      regulation: 'Regulation',
      standard: 'Standard'
    },
    carbonAnalysis: {
      title: 'Carbon Analysis',
      analysis: 'Carbon Emission Analysis'
    },
    powerParam: {
      title: 'Power Parameter Query',
      query: 'Parameter Query'
    },
    videoConfig: {
      title: 'Video Config',
      config: 'Camera Config'
    },
    realTimeView: {
      title: 'Real-time View',
      view: 'Real-time Video'
    },
    orgManage: {
      title: 'Organization Management',
      dept: 'Department Management'
    },
    paramSet: {
      title: 'Parameter Settings',
      config: 'System Parameters'
    },
    fileManage: {
      title: 'File Management',
      files: 'File List'
    },
    billingType: {
      title: 'Billing Type',
      type: 'Billing Type'
    },
    adminMonitor: {
      title: 'Admin Monitor',
      monitor: 'System Monitor'
    },
    jobScheduler: {
      title: 'Job Scheduler',
      job: 'Scheduled Tasks'
    },
    cacheList: {
      title: 'Cache List',
      list: 'Cache Monitor'
    },
    usageDetect: {
      title: 'Usage Detection',
      detect: 'Usage Monitor'
    },
    energyConsumeTrend: {
      title: 'Energy Trend',
      trend: 'Energy Trend'
    },
    energyStatus: {
      title: 'Energy Status',
      status: 'Energy Status'
    },
    usageMonitor: {
      title: 'Usage Monitor',
      monitor: 'Usage Monitor'
    },
    energyOverview: {
      title: 'Energy Overview',
      overview: 'Energy Overview'
    },
    energyFlow: {
      title: 'Energy Flow',
      flow: 'Energy Flow'
    },
    itemOverview: {
      title: 'Item Overview',
      overview: 'Itemized Overview'
    },
    deviceInfo: {
      title: 'Device Info',
      info: 'Device List'
    },
    gateway: {
      title: 'Gateway Management',
      manage: 'Gateway List'
    },
    projectTopology: {
      title: 'Project Topology',
      topology: 'Topology Management'
    },
    chargingManage: {
      title: 'Charging Management',
      station: 'Charging Station',
      pile: 'Charging Pile',
      order: 'Charging Order',
      price: 'Billing Strategy'
    },
    hazardous: {
      title: 'Hazardous Materials',
      goods: 'Hazardous Goods Info',
      info: 'Hazardous Goods Info Management',
      stockIn: 'Hazardous Goods Stock In Record',
      stockOut: 'Hazardous Goods Stock Out Record',
      inventory: 'Inventory Management',
      warning: 'Inventory Warning'
    },
    patrol: {
      title: 'Patrol Management',
      point: 'Patrol Points',
      route: 'Patrol Routes',
      plan: 'Patrol Plans',
      record: 'Patrol Records',
      alarm: 'Patrol Alarms',
      task: 'Patrol Tasks'
    },
    itemManage: {
      title: 'Item Management',
      info: 'Item Info',
      inventory: 'Item Inventory',
      stockIn: 'Item Stock In',
      stockOut: 'Item Stock Out'
    },
    chargingOperation: {
      title: 'Charging Operation',
      home: 'Charging Home',
      order: {
        title: 'Order Management',
        realtime: 'Real-time Orders',
        history: 'History Orders',
        abnormal: 'Abnormal Orders',
        occupied: 'Occupied Orders'
      },
      merchant: {
        title: 'Merchant Management',
        platform: 'Platform Merchants',
        interconnect: 'Interconnected Merchants'
      },
      station: {
        title: 'Station Management',
        direct: 'Direct Stations',
        interconnect: 'Interconnected Stations'
      },
      pile: {
        title: 'Pile Management',
        direct: 'Direct Piles',
        interconnect: 'Interconnected Piles'
      },
      brand: 'Brand & Model',
      price: 'Price & Fees',
      strategy: 'Charging Price Strategy'
    },
    pvManage: {
      title: 'PV Management',
      standard: 'Operation Standards',
      billing: 'Billing Scheme'
    }
  },

  // Navbar
  navbar: {
    home: 'Home',
    profile: 'Profile',
    layoutSetting: 'Layout Settings',
    logout: 'Logout',
    settings: 'Settings',
    fullscreen: 'Fullscreen',
    exitFullscreen: 'Exit Fullscreen',
    size: 'Layout Size',
    sizeDefault: 'Default',
    sizeMedium: 'Medium',
    sizeSmall: 'Small',
    sizeMini: 'Mini',
    theme: 'Theme Settings',
    themeLight: 'Light Theme',
    themeDark: 'Dark Theme',
    language: 'Switch Language',
    search: 'Menu Search'
  },

  // Tags View
  tagsView: {
    refreshPage: 'Refresh Page',
    closeCurrent: 'Close Current',
    closeOthers: 'Close Others',
    closeAll: 'Close All',
    closeLeft: 'Close Left',
    closeRight: 'Close Right'
  },

  // Settings
  settings: {
    title: 'System Config',
    platformName: 'Platform Name',
    platformNamePlaceholder: 'Please input platform name',
    platformLogo: 'Platform Logo',
    platformLogoPlaceholder: 'Please input platform logo',
    topNav: 'Enable TopNav',
    tagsView: 'Enable Tags-Views',
    fixedHeader: 'Fixed Header',
    sidebarLogo: 'Sidebar Logo'
  },

  // Placeholder
  placeholder: {
    username: 'Please input username',
    password: 'Please input password',
    captcha: 'Please input captcha',
    userName: 'Please input user name',
    nickName: 'Please input nickname',
    phonenumber: 'Please input phone number',
    email: 'Please input email',
    deptName: 'Please input department name',
    roleName: 'Please input role name',
    postName: 'Please input post name',
    dictType: 'Please input dictionary name',
    configName: 'Please input config name',
    menuName: 'Please input menu name',
    noticeTitle: 'Please input notice title',
    search: 'Please input keyword',
    startDate: 'Start date',
    endDate: 'End date',
    selectDept: 'Please select department',
    selectPost: 'Please select post',
    selectRole: 'Please select role',
    selectGender: 'Please select gender',
    selectStatus: 'Please select status',
    select: 'Please select',
    input: 'Please input'
  },

  // Form Validation
  validation: {
    required: '{field} is required',
    email: 'Please enter a valid email address',
    phone: 'Please enter a valid phone number',
    number: 'Please enter a number',
    minLength: 'Length cannot be less than {min} characters',
    maxLength: 'Length cannot exceed {max} characters'
  },

  // Button
  button: {
    search: 'Search',
    reset: 'Reset',
    add: 'Add',
    edit: 'Edit',
    delete: 'Delete',
    import: 'Import',
    export: 'Export',
    download: 'Download',
    upload: 'Upload',
    submit: 'Submit',
    cancel: 'Cancel',
    close: 'Close',
    save: 'Save',
    more: 'More',
    refresh: 'Refresh',
    expand: 'Expand',
    collapse: 'Collapse',
    back: 'Back',
    view: 'View',
    operate: 'Action',
    confirm: 'Confirm',
    select: 'Select',
    clear: 'Clear',
    generate: 'Generate',
    preview: 'Preview',
    copy: 'Copy',
    move: 'Move',
    enable: 'Enable',
    disable: 'Disable',
    start: 'Start',
    stop: 'Stop',
    run: 'Run',
    detail: 'Detail',
    log: 'Log',
    auth: 'Assign',
    resetPwd: 'Reset Password',
    authRole: 'Assign Role',
    authUser: 'Assign User'
  },

  // Table Column
  table: {
    userId: 'User ID',
    userName: 'User Name',
    nickName: 'Nickname',
    deptName: 'Department',
    phonenumber: 'Phone',
    status: 'Status',
    createTime: 'Create Time',
    updateTime: 'Update Time',
    operate: 'Action',
    roleId: 'Role ID',
    roleName: 'Role Name',
    roleKey: 'Role Key',
    roleSort: 'Role Sort',
    deptId: 'Dept ID',
    parentDept: 'Parent Dept',
    orderNum: 'Order',
    leader: 'Leader',
    postId: 'Post ID',
    postName: 'Post Name',
    postCode: 'Post Code',
    postSort: 'Post Sort',
    dictId: 'Dict ID',
    dictName: 'Dict Name',
    dictType: 'Dict Type',
    configId: 'Config ID',
    configName: 'Config Name',
    configKey: 'Config Key',
    configValue: 'Config Value',
    menuId: 'Menu ID',
    menuName: 'Menu Name',
    icon: 'Icon',
    path: 'Path',
    component: 'Component',
    perms: 'Permission',
    visible: 'Visible',
    isFrame: 'External',
    isCache: 'Cache',
    noticeId: 'Notice ID',
    noticeTitle: 'Notice Title',
    noticeType: 'Notice Type',
    noticeContent: 'Content',
    loginTime: 'Login Time',
    ipaddr: 'IP Address',
    loginLocation: 'Location',
    browser: 'Browser',
    os: 'OS',
    operId: 'Log ID',
    title: 'Module',
    businessType: 'Type',
    method: 'Method',
    requestMethod: 'Request',
    operName: 'Operator',
    operUrl: 'URL',
    operIp: 'IP',
    operParam: 'Params',
    jsonResult: 'Result',
    errorMsg: 'Error',
    operTime: 'Time',
    costTime: 'Cost(ms)'
  },

  // Form Label
  form: {
    userName: 'User Name',
    nickName: 'Nickname',
    dept: 'Department',
    phonenumber: 'Phone',
    email: 'Email',
    password: 'Password',
    gender: 'Gender',
    status: 'Status',
    post: 'Post',
    role: 'Role',
    remark: 'Remark',
    roleName: 'Role Name',
    roleKey: 'Role Key',
    roleSort: 'Role Sort',
    dataScope: 'Data Scope',
    menuPermissions: 'Menu Permissions',
    deptName: 'Department Name',
    parentDept: 'Parent Department',
    showOrder: 'Show Order',
    leader: 'Leader',
    contactPhone: 'Contact Phone',
    postName: 'Post Name',
    postCode: 'Post Code',
    postSort: 'Post Sort',
    dictName: 'Dict Name',
    dictType: 'Dict Type',
    dictLabel: 'Label',
    dictValue: 'Value',
    dictSort: 'Sort',
    isDefault: 'Default',
    listClass: 'Style',
    configName: 'Config Name',
    configKey: 'Config Key',
    configValue: 'Config Value',
    menuName: 'Menu Name',
    parentMenu: 'Parent Menu',
    menuType: 'Menu Type',
    menuIcon: 'Menu Icon',
    menuSort: 'Menu Sort',
    isFrame: 'External Link',
    isCache: 'Cache',
    visible: 'Visible',
    routePath: 'Route Path',
    componentPath: 'Component Path',
    perms: 'Permission',
    noticeTitle: 'Notice Title',
    noticeType: 'Notice Type',
    status: 'Status',
    noticeContent: 'Content',
    newPassword: 'New Password'
  },

  // Message
  message: {
    addSuccess: 'Added successfully',
    editSuccess: 'Modified successfully',
    deleteSuccess: 'Deleted successfully',
    saveSuccess: 'Saved successfully',
    submitSuccess: 'Submitted successfully',
    importSuccess: 'Imported successfully',
    exportSuccess: 'Exported successfully',
    uploadSuccess: 'Uploaded successfully',
    downloadSuccess: 'Downloaded successfully',
    copySuccess: 'Copied successfully',
    operationSuccess: 'Operation successful',
    addFailed: 'Add failed',
    editFailed: 'Edit failed',
    deleteFailed: 'Delete failed',
    saveFailed: 'Save failed',
    submitFailed: 'Submit failed',
    importFailed: 'Import failed',
    uploadFailed: 'Upload failed',
    operationFailed: 'Operation failed',
    confirmDelete: 'Are you sure to delete',
    confirmClear: 'Are you sure to clear',
    confirmLogout: 'Are you sure to logout',
    confirmExport: 'Confirm to export',
    selectOne: 'Please select one record',
    selectAtLeastOne: 'Please select at least one record',
    selectRecord: 'Please select record to operate',
    inputRequired: 'Please input',
    selectRequired: 'Please select',
    deleteConfirm: 'Cannot be recovered after deletion, confirm?',
    enableSuccess: 'Enabled successfully',
    disableSuccess: 'Disabled successfully',
    resetPwdSuccess: 'Password reset successful, new password is: ',
    noData: 'No data',
    loading: 'Loading...',
    clearingCache: 'Clearing settings cache and refreshing, please wait...',
    systemError: 'System error',
    networkError: 'Network error',
    timeout: 'Request timeout',
    invalidFileType: 'Invalid file type',
    fileTooLarge: 'File too large'
  },

  // Confirm
  confirm: {
    title: 'System Prompt',
    confirmButtonText: 'Confirm',
    cancelButtonText: 'Cancel',
    delete: 'Are you sure to delete selected records?',
    clear: 'Are you sure to clear all data?',
    logout: 'Are you sure to logout?',
    export: 'Confirm to export data?',
    import: 'Confirm to import data?',
    enable: 'Confirm to enable this record?',
    disable: 'Confirm to disable this record?',
    resetPwd: 'Please input new password for "{username}"',
    authRole: 'Confirm to assign roles?',
    authUser: 'Confirm to assign users?',
    runTask: 'Confirm to run this task immediately?',
    clean: 'Confirm to clear cache?',
    forceLogout: 'Confirm to force logout this user?'
  },

  // Validation Rules
  rules: {
    userNameRequired: 'User name is required',
    userNameLength: 'User name length must be between 2 and 20',
    nickNameRequired: 'Nickname is required',
    passwordRequired: 'Password is required',
    passwordLength: 'Password length must be between 5 and 20',
    emailInvalid: 'Please input valid email',
    phoneInvalid: 'Please input valid phone number',
    deptRequired: 'Department is required',
    roleNameRequired: 'Role name is required',
    roleKeyRequired: 'Role key is required',
    roleSortRequired: 'Role sort is required',
    deptNameRequired: 'Department name is required',
    postNameRequired: 'Post name is required',
    postCodeRequired: 'Post code is required',
    dictNameRequired: 'Dict name is required',
    dictTypeRequired: 'Dict type is required',
    configNameRequired: 'Config name is required',
    configKeyRequired: 'Config key is required',
    menuNameRequired: 'Menu name is required',
    pathRequired: 'Route path is required',
    noticeTitleRequired: 'Notice title is required',
    noticeContentRequired: 'Notice content is required'
  },

  // System Module
  system: {
    user: {
      title: 'User Management',
      importTitle: 'User Import',
      importTip: 'Drag file here or ',
      importClick: 'click to upload',
      importFormat: 'Only xls, xlsx files are allowed',
      updateSupport: 'Update existing user data',
      downloadTemplate: 'Download template',
      importResult: 'Import Result',
      resetPwdTitle: 'Prompt',
      resetPwdConfirm: 'Confirm',
      resetPwdCancel: 'Cancel',
      pwdLengthError: 'Password length must be between 5 and 20'
    },
    role: {
      title: 'Role Management',
      authUserTitle: 'Assign Users',
      selectUser: 'Select User',
      dataScope1: 'All data permissions',
      dataScope2: 'Custom data permissions',
      dataScope3: 'Department data permissions',
      dataScope4: 'Department and below permissions',
      dataScope5: 'Only own data permissions'
    },
    dept: {
      title: 'Department Management',
      expandAll: 'Expand/Collapse',
      selectParent: 'Select parent department'
    },
    post: {
      title: 'Post Management'
    },
    dict: {
      title: 'Dictionary Management',
      dataTitle: 'Dictionary Data',
      addData: 'Add Dictionary Data',
      editData: 'Edit Dictionary Data'
    },
    config: {
      title: 'Config Management'
    },
    menu: {
      title: 'Menu Management',
      expandAll: 'Expand/Collapse',
      selectIcon: 'Select Icon',
      directory: 'Directory',
      menu: 'Menu',
      button: 'Button',
      parentMenu: 'Main Category'
    },
    notice: {
      title: 'Notice',
      addTitle: 'Add Notice',
      editTitle: 'Edit Notice'
    },
    log: {
      operlog: {
        title: 'Operation Log',
        detailTitle: 'Operation Log Detail',
        operId: 'Operation ID',
        operModule: 'Module',
        operType: 'Type',
        operDesc: 'Description'
      },
      logininfor: {
        title: 'Login Log',
        unlock: 'Unlock',
        unlockSuccess: 'Unlock successful'
      }
    }
  },

  // Monitor Module
  monitor: {
    online: {
      title: 'Online Users',
      forceLogout: 'Force Logout',
      forceLogoutConfirm: 'Are you sure to force logout this user?',
      forceLogoutSuccess: 'Force logout successful',
      sessionId: 'Session ID',
      loginName: 'Login Name',
      deptName: 'Department',
      host: 'Host',
      loginTime: 'Login Time'
    },
    job: {
      title: 'Scheduled Tasks',
      addTitle: 'Add Task',
      editTitle: 'Edit Task',
      cronExpression: 'Cron Expression',
      jobName: 'Job Name',
      jobGroup: 'Job Group',
      invokeTarget: 'Invoke Target',
      misfirePolicy: 'Misfire Policy',
      concurrent: 'Concurrent',
      status: 'Status',
      runOnce: 'Run Once',
      log: 'Log',
      logTitle: 'Job Log',
      logDetail: 'Log Detail',
      jobLogId: 'Log ID',
      jobMessage: 'Message',
      isConcurrent: 'Is Concurrent',
      runOnceConfirm: 'Confirm to run this task once?',
      runSuccess: 'Run successful'
    },
    server: {
      title: 'Server Monitor',
      cpu: 'CPU',
      memory: 'Memory',
      disk: 'Disk',
      serverInfo: 'Server Info',
      jvmInfo: 'JVM Info',
      diskInfo: 'Disk Status',
      cpuUsage: 'CPU Usage',
      memoryUsage: 'Memory Usage',
      runTime: 'Run Time',
      startTime: 'Start Time'
    },
    cache: {
      title: 'Cache Monitor',
      cacheName: 'Cache Name',
      cacheKey: 'Cache Key',
      cacheValue: 'Cache Value',
      clear: 'Clear Cache',
      clearSuccess: 'Clear successful',
      list: 'Cache List',
      keyList: 'Key List'
    }
  },

  // Dashboard
  dashboard: {
    title: 'Dashboard',
    projectOverview: 'Project Overview',
    deviceStatus: 'Device Status',
    alarmInfo: 'Alarm Info',
    energyStatistic: 'Today Energy Statistics',
    energyTrend: 'Today Energy Trend',
    carbonEmissions: 'Today Carbon Emissions',
    projectTotal: 'Total Projects',
    meterTotal: 'Total Meters',
    alarmTotal: 'Alarm Records',
    normalDevice: 'Normal Device',
    alarmDevice: 'Alarm Device',
    offlineDevice: 'Offline Device',
    proportion: 'Proportion',
    electricity: 'Electricity(kW·h)',
    comprehensiveEnergy: 'Energy(kgce)',
    carbonEmission: 'Carbon(kg)',
    totalEnergy: 'Total Energy',
    water: 'Water',
    time: 'Time',
    more: 'More>>',
    chargingUser: 'Charging Users',
    chargingAmount: 'Charging Amount',
    chargingEnergy: 'Charging Energy',
    chargingCount: 'Charging Count',
    totalUser: 'Total Users',
    totalAmount: 'Total Amount',
    totalEnergyKwh: 'Total Energy',
    totalCount: 'Total Count',
    monthOverMonth: 'MoM',
    yearOverYear: 'YoY',
    unitUser: 'users',
    unitYuan: 'CNY',
    unitKwh: 'kW·h',
    unitCount: 'times',
    chargingTerminalStatus: 'Terminal Status',
    chargingOrderTrend: 'Order Trend',
    last7DaysCount: 'Last 7 Days Count',
    last7DaysDuration: 'Last 7 Days Duration',
    last7DaysEnergy: 'Last 7 Days Energy',
    last7DaysAmount: 'Last 7 Days Amount',
    duration: 'Duration',
    energy: 'Energy',
    amount: 'Amount',
    thisMonth: 'This Month',
    thisYear: 'This Year',
    day: 'Day',
    month: 'Month'
  },

  // Energy Management Module
  energyModule: {
    title: 'Energy Management',
    // Batch Management
    batch: {
      title: 'Batch Management',
      batchId: 'Batch ID',
      productName: 'Product Name',
      batchNumber: 'Batch Number',
      productionDate: 'Production Date',
      productionLine: 'Production Line',
      energyConsumption: 'Energy Consumption',
      unit: 'Unit',
      status: 'Status',
      createTime: 'Create Time',
      remark: 'Remark',
      placeholder: {
        searchBatch: 'Please input batch number/product name',
        selectDate: 'Please select production date'
      }
    },
    // Benchmark Management
    benchmark: {
      title: 'Benchmark Management',
      standardId: 'Standard ID',
      standardName: 'Standard Name',
      energyType: 'Energy Type',
      benchmarkValue: 'Benchmark Value',
      unit: 'Unit',
      industry: 'Industry',
      region: 'Region',
      effectiveDate: 'Effective Date',
      status: 'Status',
      remark: 'Remark',
      placeholder: {
        searchStandard: 'Please input standard name',
        selectType: 'Please select energy type'
      }
    },
    // Energy Balance Management
    balance: {
      title: 'Energy Balance Management',
      balanceId: 'Balance ID',
      totalSupply: 'Total Supply',
      totalConsumption: 'Total Consumption',
      balanceAmount: 'Balance Amount',
      balanceRate: 'Balance Rate',
      balanceDate: 'Balance Date',
      calculate: 'Calculate Energy Balance'
    },
    // Energy Quality Management
    quality: {
      title: 'Energy Quality Management',
      qualityId: 'Record ID',
      qualityIndex: 'Quality Index',
      standardValue: 'Standard Value',
      actualValue: 'Actual Value',
      deviation: 'Deviation',
      detectionTime: 'Detection Time'
    },
    // Energy Analysis
    analysis: {
      title: 'Energy Analysis',
      trend: 'Energy Trend',
      comparison: 'Energy Comparison',
      ranking: 'Energy Ranking',
      report: 'Analysis Report'
    }
  },

  // Charging Station Module
  chargingModule: {
    title: 'Charging Station Management',
    // Station Management
    station: {
      title: 'Station Management',
      stationId: 'Station ID',
      stationName: 'Station Name',
      stationCode: 'Station Code',
      stationType: 'Station Type',
      price: 'Station Price',
      merchant: 'Merchant',
      merchantName: 'Merchant Name',
      address: 'Station Address',
      status: 'Station Status',
      longitude: 'Longitude',
      latitude: 'Latitude',
      openTime: 'Open Time',
      parkFee: 'Parking Fee',
      serviceFee: 'Service Fee',
      contactName: 'Contact Person',
      contactPhone: 'Contact Phone',
      createTime: 'Create Time',
      updateTime: 'Update Time',
      remark: 'Remark',
      placeholder: {
        searchStation: 'Please input station name',
        searchMerchant: 'Please input merchant name',
        selectType: 'Please select station type',
        selectStatus: 'Please select station status',
        inputAddress: 'Please input station address'
      }
    },
    // Charging Pile Management
    pile: {
      title: 'Charging Pile Management',
      pileId: 'Pile ID',
      pileName: 'Pile Name',
      pileCode: 'Pile Code',
      pileType: 'Pile Type',
      pileModel: 'Pile Model',
      station: 'Station',
      stationName: 'Station Name',
      status: 'Pile Status',
      power: 'Power',
      voltage: 'Voltage',
      current: 'Current',
      connectorType: 'Connector Type',
      connectorCount: 'Connector Count',
      gunNumber: 'Gun Number',
      gunStatus: 'Gun Status',
      manufacturer: 'Manufacturer',
      productionDate: 'Production Date',
      installationDate: 'Installation Date',
      lastMaintainTime: 'Last Maintenance',
      createTime: 'Create Time',
      remark: 'Remark',
      encoding: 'Terminal Code',
      name: 'Terminal Name',
      merchantName: 'Merchant Name',
      workStatus: 'Work Status',
      batchEnable: 'Batch Enable',
      batchDisable: 'Batch Disable',
      selectBrandFirst: 'Please select brand first',
      addTitle: 'Add Charging Pile',
      editTitle: 'Edit Charging Pile',
      deleteConfirm: 'Are you sure to delete the charging pile with ID "{ids}"?',
      batchEnableConfirm: 'Are you sure to batch enable selected terminals?',
      batchDisableConfirm: 'Are you sure to batch disable selected terminals?',
      batchEnableSuccess: 'Batch enable successful',
      batchDisableSuccess: 'Batch disable successful',
      statusConfirm: 'Are you sure to {text} this charging pile?',
      placeholder: {
        searchPile: 'Please input pile name/code',
        selectStation: 'Please select station',
        selectType: 'Please select pile type',
        selectStatus: 'Please select pile status'
      }
    },
    // Brand Management
    brand: {
      title: 'Brand Management',
      id: 'Brand ID',
      brandName: 'Brand Name',
      status: 'Brand Status',
      addTitle: 'Add Brand Information',
      editTitle: 'Edit Brand Information',
      deleteConfirm: 'Are you sure to delete the brand information with ID "{ids}"?',
      statusConfirm: 'Are you sure to {text} this brand?',
      // Model Management
      modelId: 'Model ID',
      modelName: 'Model Name',
      modelStatus: 'Model Status',
      addModelTitle: 'Add Model Information',
      editModelTitle: 'Edit Model Information',
      deleteModelConfirm: 'Are you sure to delete the model information with ID "{ids}"?',
      modelStatusConfirm: 'Are you sure to {text} this model?'
    },
    // Merchant Management
    merchant: {
      title: 'Merchant Management',
      name: 'Merchant Name',
      type: 'Merchant Type',
      contact: 'Contact',
      address: 'Address',
      status: 'Merchant Status',
      managePile: 'Manage Piles',
      addTitle: 'Add Merchant Information',
      editTitle: 'Edit Merchant Information',
      deleteConfirm: 'Are you sure to delete the merchant information with ID "{ids}"?',
      statusConfirm: 'Are you sure to {text} this merchant?'
    },
    // Charging Order
    order: {
      title: 'Charging Order',
      orderNo: 'Order No',
      userName: 'User Name',
      phone: 'Phone',
      merchantName: 'Merchant Name',
      stationName: 'Station Name',
      pileName: 'Pile Name',
      carNo: 'License Plate',
      carVin: 'VIN Code',
      chargeMethod: 'Charge Method',
      orderSource: 'Order Source',
      settleType: 'Settlement Type',
      orderStatus: 'Order Status',
      createTime: 'Create Time',
      startTime: 'Start Time',
      endTime: 'End Time',
      settleTime: 'Settlement Time',
      settlePrice: 'Settlement Amount',
      paidPrice: 'Paid Amount',
      discountAmt: 'Discount Amount',
      elecAmt: 'Electricity Fee',
      serveAmt: 'Service Fee',
      chargeDuration: 'Charge Duration',
      energy: 'Total Energy',
      settleBalance: 'Account Balance',
      payType: 'Payment Method',
      abnoCause: 'Abnormal Cause',
      detail: 'Detail',
      process: 'Process',
      addTitle: 'Add Order Information',
      editTitle: 'Edit Order Information',
      deleteConfirm: 'Are you sure to delete the order information with ID "{ids}"?'
    },
      title: 'Charging Order',
      orderId: 'Order ID',
      orderNo: 'Order No',
      userId: 'User ID',
      userName: 'User Name',
      userPhone: 'User Phone',
      stationName: 'Station',
      pileName: 'Charging Pile',
      gunNumber: 'Gun Number',
      startTime: 'Start Time',
      endTime: 'End Time',
      chargeDuration: 'Duration',
      chargeAmount: 'Charged Amount',
      chargePower: 'Power',
      totalAmount: 'Total Amount',
      electricityFee: 'Electricity Fee',
      serviceFee: 'Service Fee',
      discountAmount: 'Discount',
      payAmount: 'Pay Amount',
      payStatus: 'Payment Status',
      payTime: 'Payment Time',
      payMethod: 'Payment Method',
      orderStatus: 'Order Status',
      plateNumber: 'License Plate',
      vinCode: 'VIN Code',
      socStart: 'Start SOC',
      socEnd: 'End SOC',
      startMeterReading: 'Start Meter',
      endMeterReading: 'End Meter',
      createTime: 'Create Time',
      placeholder: {
        searchOrder: 'Please input order number',
        searchUser: 'Please input user name/phone',
        selectStation: 'Please select station',
        selectStatus: 'Please select order status',
        selectPayStatus: 'Please select payment status'
      }
    },
    // Pricing Strategy
    price: {
      title: 'Pricing Strategy',
      strategyId: 'Strategy ID',
      strategyName: 'Strategy Name',
      strategyType: 'Pricing Type',
      timeType: 'Time Type',
      startTime: 'Start Time',
      endTime: 'End Time',
      electricityPrice: 'Electricity Price',
      servicePrice: 'Service Price',
      parkPrice: 'Parking Price',
      maxPrice: 'Max Price',
      minPrice: 'Min Price',
      minDuration: 'Min Duration',
      maxDuration: 'Max Duration',
      effectiveDate: 'Effective Date',
      expiryDate: 'Expiry Date',
      applicableStations: 'Applicable Stations',
      status: 'Status',
      remark: 'Remark',
      placeholder: {
        searchStrategy: 'Please input strategy name',
        selectType: 'Please select pricing type',
        selectStation: 'Please select station'
      }
    },
    // Merchant Management
    merchant: {
      title: 'Merchant Management',
      merchantId: 'Merchant ID',
      merchantName: 'Merchant Name',
      merchantCode: 'Merchant Code',
      contactPerson: 'Contact Person',
      contactPhone: 'Contact Phone',
      email: 'Email',
      address: 'Address',
      businessLicense: 'Business License',
      accountName: 'Account Name',
      accountBank: 'Bank',
      accountNo: 'Account No',
      settlementCycle: 'Settlement Cycle',
      commissionRate: 'Commission Rate',
      status: 'Status',
      createTime: 'Create Time',
      remark: 'Remark',
      placeholder: {
        searchMerchant: 'Please input merchant name'
      }
    },
    // Brand Management
    brand: {
      title: 'Brand Management',
      brandId: 'Brand ID',
      brandName: 'Brand Name',
      brandCode: 'Brand Code',
      brandLogo: 'Brand Logo',
      manufacturer: 'Manufacturer',
      country: 'Country',
      website: 'Website',
      sort: 'Sort',
      status: 'Status',
      createTime: 'Create Time',
      remark: 'Remark',
      placeholder: {
        searchBrand: 'Please input brand name'
      }
    },
    // Occupancy Order
    occupancyOrder: {
      title: 'Occupancy Order',
      occupancyNo: 'Occupancy Order No',
      orderNo: 'Charge Order No',
      settleStatus: 'Settlement Status',
      orderStatus: 'Order Status',
      duration: 'Occupancy Duration',
      fee: 'Occupancy Fee',
      isFee: 'Is Fee Generated',
      payTime: 'Payment Time',
      endReason: 'End Reason',
      orderInfo: 'Related Order Info',
      userName: 'User Name',
      phone: 'Phone',
      merchantName: 'Merchant Name',
      stationName: 'Station Name',
      pileName: 'Pile Name',
      yes: 'Yes',
      no: 'No',
      addTitle: 'Add Occupancy Order',
      editTitle: 'Edit Occupancy Order',
      deleteConfirm: 'Are you sure to delete the occupancy order with ID "{ids}"?',
      validate: {
        idRequired: 'Occupancy order ID is required',
        occupancyNoRequired: 'Occupancy order number is required',
        orderNoRequired: 'Charge order number is required',
        durationRequired: 'Occupancy duration is required',
        feeRequired: 'Occupancy fee is required',
        isFeeRequired: 'Is fee generated is required',
        payTimeRequired: 'Payment time is required',
        settleStatusRequired: 'Settlement status is required',
        orderStatusRequired: 'Order status is required',
        endReasonRequired: 'End reason is required'
      }
    },
    // Order Detail
    orderDetail: {
      title: 'Order Detail',
      orderInfo: 'Order Information',
      pileInfo: 'Charging Pile Info',
      terminalCode: 'Terminal Code',
      terminalName: 'Terminal Name',
      brand: 'Brand',
      model: 'Model',
      station: 'Station',
      merchant: 'Merchant',
      contactPhone: 'Contact Phone',
      stationAddress: 'Station Address',
      demandVoltage: 'Demand/Actual Voltage',
      demandCurrent: 'Demand/Actual Current',
      soc: 'SOC',
      batteryTemp: 'Battery Temperature',
      currentUnit: 'Current/A',
      socUnit: 'SOC/%',
      tempUnit: 'Temperature/℃',
      voltageUnit: 'Voltage/V',
      yuan: 'CNY',
      kwh: 'kW·h',
      hour: 'h'
    },

  // Inspection Management Module
  inspectionModule: {
    title: 'Inspection Management',
    // Inspection Task
    task: {
      title: 'Inspection Task',
      taskId: 'Task ID',
      taskName: 'Task Name',
      taskCode: 'Task Code',
      planId: 'Plan',
      planName: 'Inspection Plan',
      pathId: 'Path',
      pathName: 'Path Name',
      userId: 'Inspector',
      userName: 'Inspector',
      inspector: 'Inspector',
      inspectDate: 'Inspection Date',
      startTime: 'Start Time',
      endTime: 'End Time',
      deadline: 'Deadline',
      taskStatus: 'Task Status',
      pointCount: 'Point Count',
      checkedCount: 'Checked Count',
      abnormalCount: 'Abnormal Count',
      completionRate: 'Completion Rate',
      checkResult: 'Check Result',
      remark: 'Remark',
      placeholder: {
        searchTask: 'Please input task name',
        selectPlan: 'Please select plan',
        selectPath: 'Please select path',
        selectUser: 'Please select inspector',
        selectStatus: 'Please select status'
      }
    },
    // Inspection Plan
    plan: {
      title: 'Inspection Plan',
      planId: 'Plan ID',
      planName: 'Plan Name',
      planCode: 'Plan Code',
      frequency: 'Frequency',
      cycleType: 'Cycle Type',
      cycleDay: 'Cycle Days',
      startTime: 'Start Time',
      endTime: 'End Time',
      pathId: 'Path',
      pathName: 'Path Name',
      executorType: 'Executor Type',
      executor: 'Executor',
      remindTime: 'Remind Time',
      status: 'Status',
      createTime: 'Create Time',
      remark: 'Remark',
      placeholder: {
        searchPlan: 'Please input plan name',
        selectPath: 'Please select path',
        selectFrequency: 'Please select frequency',
        selectStatus: 'Please select status'
      }
    },
    // Inspection Path
    path: {
      title: 'Inspection Path',
      pathId: 'Path ID',
      pathName: 'Path Name',
      pathCode: 'Path Code',
      pointCount: 'Point Count',
      estimatedTime: 'Estimated Time',
      estimatedDistance: 'Estimated Distance',
      region: 'Region',
      status: 'Status',
      createTime: 'Create Time',
      remark: 'Remark',
      placeholder: {
        searchPath: 'Please input path name',
        selectRegion: 'Please select region'
      }
    },
    // Inspection Point
    point: {
      title: 'Inspection Point',
      pointId: 'Point ID',
      pointName: 'Point Name',
      pointCode: 'Point Code',
      pointType: 'Point Type',
      location: 'Location',
      region: 'Region',
      longitude: 'Longitude',
      latitude: 'Latitude',
      rfidCode: 'RFID Code',
      qrCode: 'QR Code',
      checkItem: 'Check Item',
      checkStandard: 'Check Standard',
      normalValue: 'Normal Value',
      deviceId: 'Device',
      deviceName: 'Device Name',
      photoRequired: 'Photo Required',
      remarkRequired: 'Remark Required',
      sort: 'Sort',
      status: 'Status',
      createTime: 'Create Time',
      remark: 'Remark',
      placeholder: {
        searchPoint: 'Please input point name',
        selectType: 'Please select point type',
        selectRegion: 'Please select region'
      }
    },
    // Inspection Record
    record: {
      title: 'Inspection Record',
      recordId: 'Record ID',
      taskId: 'Task',
      taskName: 'Task Name',
      pointId: 'Point',
      pointName: 'Point Name',
      checkTime: 'Check Time',
      checkResult: 'Check Result',
      checkValue: 'Check Value',
      isNormal: 'Is Normal',
      normal: 'Normal',
      abnormal: 'Abnormal',
      abnormalDesc: 'Abnormal Description',
      abnormalType: 'Abnormal Type',
      abnormalLevel: 'Abnormal Level',
      handler: 'Handler',
      handleTime: 'Handle Time',
      handleResult: 'Handle Result',
      handleStatus: 'Handle Status',
      photo: 'Photo',
      signPhoto: 'Sign Photo',
      remark: 'Remark',
      placeholder: {
        searchRecord: 'Please input point name',
        selectResult: 'Please select result',
        selectStatus: 'Please select status'
      }
    },
    // Inspection Alarm
    alarm: {
      title: 'Inspection Alarm',
      alarmId: 'Alarm ID',
      alarmTitle: 'Alarm Title',
      alarmType: 'Alarm Type',
      alarmLevel: 'Alarm Level',
      alarmContent: 'Alarm Content',
      alarmTime: 'Alarm Time',
      pointId: 'Point',
      pointName: 'Point Name',
      pathName: 'Path Name',
      taskName: 'Task Name',
      handler: 'Handler',
      handleTime: 'Handle Time',
      handleResult: 'Handle Result',
      handleStatus: 'Handle Status',
      status: 'Status',
      createTime: 'Create Time',
      placeholder: {
        searchAlarm: 'Please input alarm title',
        selectType: 'Please select alarm type',
        selectLevel: 'Please select alarm level',
        selectStatus: 'Please select status'
      }
    }
  },

  // Hazardous Materials Management Module
  hazardousModule: {
    title: 'Hazardous Materials Management',
    // Hazardous Goods Info
    goods: {
      title: 'Hazardous Goods Info',
      goodsId: 'Goods ID',
      goodsName: 'Goods Name',
      goodsCode: 'Goods Code',
      goodsType: 'Goods Type',
      goodsCategory: 'Category',
      status: 'Status',
      specification: 'Specification',
      casNo: 'CAS No',
      unCode: 'UN Code',
      formula: 'Formula',
      molecularWeight: 'Molecular Weight',
      appearance: 'Appearance',
      density: 'Density',
      meltingPoint: 'Melting Point',
      boilingPoint: 'Boiling Point',
      flashPoint: 'Flash Point',
      storageTemp: 'Storage Temp',
      hazardClass: 'Hazard Class',
      hazardProperties: 'Hazard Properties',
      storageRequirements: 'Storage Requirements',
      handlingPrecautions: 'Precautions',
      emergencyMeasures: 'Emergency Measures',
      fireFightingMeasures: 'Fire Fighting',
      firstAidMeasures: 'First Aid',
      supplier: 'Supplier',
      manufacturer: 'Manufacturer',
      storageLocation: 'Storage Location',
      safetyStock: 'Safety Stock',
      maxStock: 'Max Stock',
      unit: 'Unit',
      validityPeriod: 'Validity Period',
      msdsFile: 'MSDS File',
      labelFile: 'Label File',
      warningSign: 'Warning Sign',
      createTime: 'Create Time',
      remark: 'Remark',
      placeholder: {
        searchGoods: 'Please input goods name/CAS',
        selectType: 'Please select goods type',
        selectCategory: 'Please select category',
        selectStatus: 'Please select status'
      }
    },
    // Inventory Management
    inventory: {
      title: 'Inventory Management',
      inventoryId: 'Inventory ID',
      goodsId: 'Goods',
      goodsName: 'Goods Name',
      goodsCode: 'Goods Code',
      goodsType: 'Goods Type',
      batchNo: 'Batch No',
      quantity: 'Quantity',
      availableQuantity: 'Available',
      lockedQuantity: 'Locked',
      unit: 'Unit',
      storageLocation: 'Location',
      storageArea: 'Area',
      storageCabinet: 'Cabinet',
      safetyStock: 'Safety Stock',
      maxStock: 'Max Stock',
      expiryDate: 'Expiry Date',
      productionDate: 'Production Date',
      supplier: 'Supplier',
      status: 'Status',
      createTime: 'Create Time',
      remark: 'Remark',
      placeholder: {
        searchInventory: 'Please input goods name',
        selectLocation: 'Please select location',
        selectStatus: 'Please select status'
      }
    },
    // Stock In Management
    stockIn: {
      title: 'Stock In',
      inId: 'In ID',
      inNo: 'In Order No',
      inType: 'In Type',
      inTime: 'In Time',
      goodsName: 'Goods Name',
      goodsCode: 'Goods Code',
      batchNo: 'Batch No',
      inQuantity: 'In Quantity',
      unit: 'Unit',
      storageLocation: 'Location',
      supplier: 'Supplier',
      productionDate: 'Production Date',
      expiryDate: 'Expiry Date',
      operator: 'Operator',
      operatorName: 'Operator Name',
      approver: 'Approver',
      approveTime: 'Approve Time',
      approveStatus: 'Approve Status',
      remark: 'Remark',
      placeholder: {
        searchInNo: 'Please input order number',
        selectType: 'Please select in type',
        selectStatus: 'Please select approve status'
      }
    },
    // Stock Out Management
    stockOut: {
      title: 'Stock Out',
      outId: 'Out ID',
      outNo: 'Out Order No',
      outType: 'Out Type',
      outTime: 'Out Time',
      goodsName: 'Goods Name',
      goodsCode: 'Goods Code',
      batchNo: 'Batch No',
      outQuantity: 'Out Quantity',
      unit: 'Unit',
      recipient: 'Recipient',
      recipientDept: 'Department',
      recipientPhone: 'Phone',
      usePurpose: 'Purpose',
      returnTime: 'Return Time',
      operator: 'Operator',
      approver: 'Approver',
      approveTime: 'Approve Time',
      approveStatus: 'Approve Status',
      remark: 'Remark',
      placeholder: {
        searchOutNo: 'Please input order number',
        selectType: 'Please select out type',
        selectStatus: 'Please select approve status'
      }
    },
    // Inventory Warning
    warning: {
      title: 'Inventory Warning',
      warningId: 'Warning ID',
      warningType: 'Warning Type',
      warningLevel: 'Warning Level',
      goodsName: 'Goods Name',
      currentQuantity: 'Current Quantity',
      safetyStock: 'Safety Stock',
      maxStock: 'Max Stock',
      warningTime: 'Warning Time',
      handler: 'Handler',
      handleTime: 'Handle Time',
      handleStatus: 'Handle Status',
      placeholder: {
        searchGoods: 'Please input goods name',
        selectType: 'Please select warning type',
        selectLevel: 'Please select warning level',
        selectStatus: 'Please select handle status'
      }
    }
  },
  // Profile Module
  profileModule: {
    title: 'Profile',
    personalInfo: 'Personal Info',
    myOrder: 'My Orders',
    myInspection: 'My Inspections',
    orderCountMonthly: 'Order Count (Monthly)',
    inspectionCountMonthly: 'Inspection Count (Monthly)',
    userName: 'Username',
    nickName: 'Nickname',
    sex: 'Gender',
    phone: 'Phone',
    email: 'Email',
    dept: 'Department',
    post: 'Post',
    role: 'Role',
    createTime: 'Create Time',
    editInfo: 'Edit Info',
    resetPwd: 'Reset Password',
    pending: 'Pending',
    completed: 'Completed',
    male: 'Male',
    female: 'Female',
    unknown: 'Unknown',
    oldPassword: 'Old Password',
    newPassword: 'New Password',
    confirmPassword: 'Confirm Password',
    enterOldPwd: 'Please enter old password',
    enterNewPwd: 'Please enter new password',
    confirmNewPwd: 'Please confirm new password',
    pwdNotEmpty: 'Old password cannot be empty',
    pwdLength: 'Length should be between 6 and 20 characters',
    confirmPwdNotEmpty: 'Confirm password cannot be empty',
    pwdMismatch: 'Two passwords do not match',
    modifySuccess: 'Modified successfully',
    chart: {
      quantity: 'Quantity',
      month: 'Month',
      finished: 'Finished',
      unfinished: 'Unfinished'
    },
    placeholder: {
      inputOldPwd: 'Please enter old password',
      inputNewPwd: 'Please enter new password',
      confirmNewPwd: 'Please confirm new password'
    }
  },
  // Control Module
  controlModule: {
    title: 'Centralized Control',
    deviceManagement: 'Device Management',
    areaControl: 'Area Control',
    remoteOperation: 'Remote Operation',
    controlLog: 'Control Log',
    deviceControl: 'Device Control',
    searchDevice: 'Search device',
    deviceId: 'Device ID',
    deviceName: 'Device Name',
    deviceType: 'Device Type',
    location: 'Location',
    status: 'Status',
    lastOnlineTime: 'Last Online Time',
    action: 'Action',
    control: 'Control',
    online: 'Online',
    offline: 'Offline',
    waterSourceArea: 'Water Source Area',
    middleStationArea: 'Middle Station Area',
    highPoolArea: 'High Pool Area',
    wellArea: 'Well Area',
    open: 'Open',
    close: 'Close',
    areaControlSuffix: 'Area Control',
    operationType: 'Operation Type',
    targetDevice: 'Target Device',
    operationParams: 'Operation Params',
    securityVerify: 'Security Verify',
    inputSecurityCode: 'Input security code',
    execute: 'Execute',
    reset: 'Reset',
    logId: 'Log ID',
    operationTypeCol: 'Operation Type',
    target: 'Target',
    operator: 'Operator',
    operationTime: 'Operation Time',
    result: 'Result',
    remark: 'Remark',
    success: 'Success',
    fail: 'Fail',
    start: 'Start',
    stop: 'Stop',
    restart: 'Restart',
    adjust: 'Adjust',
    selectOperationType: 'Select operation type',
    selectDevice: 'Select device',
    inputParams: 'Input operation params',
    inputSecurityPwd: 'Input security password',
    deviceNameLabel: 'Device Name',
    controlCommand: 'Control Command',
    selectCommand: 'Select command',
    params: 'Parameters',
    inputParamsLabel: 'Input parameters',
    securityPwd: 'Security Password',
    cancel: 'Cancel',
    confirm: 'Confirm',
    placeholder: {
      searchDevice: 'Search device',
      selectOperationType: 'Select operation type',
      selectDevice: 'Select device',
      inputParams: 'Input operation params',
      inputSecurityCode: 'Input security code',
      selectCommand: 'Select command',
      inputParams: 'Input parameters',
      inputSecurityPwd: 'Input security password'
    }
  },
  // Metering Module
  meteringModule: {
    title: 'Metering Management',
    meterManagement: 'Meter Management',
    calibrationPlan: 'Calibration Plan',
    searchMeter: 'Search meter',
    meterId: 'Meter ID',
    meterName: 'Meter Name',
    type: 'Type',
    specification: 'Specification',
    installationLocation: 'Installation Location',
    status: 'Status',
    lastCalibrationDate: 'Last Calibration Date',
    nextCalibrationDate: 'Next Calibration Date',
    action: 'Action',
    detail: 'Detail',
    calibrate: 'Calibrate',
    planId: 'Plan ID',
    meter: 'Meter',
    planCalibrationDate: 'Plan Calibration Date',
    executor: 'Executor',
    addPlan: 'Add Plan',
    calibrationRecord: 'Calibration Record',
    startDate: 'Start Date',
    endDate: 'End Date',
    selectStatus: 'Select Status',
    all: 'All',
    pending: 'Pending',
    executing: 'Executing',
    completed: 'Completed',
    cancelled: 'Cancelled',
    pass: 'Pass',
    fail: 'Fail',
    analysis: 'Calibration Data Analysis',
    qualificationTrend: 'Qualification Rate Trend',
    deviationDistribution: 'Deviation Distribution',
    meterStatusStatistics: 'Meter Status Statistics',
    export: 'Export Records',
    execute: 'Execute',
    viewMeterDetail: 'View meter details of',
    viewPlanDetail: 'View calibration plan',
    viewRecordDetail: 'View calibration record',
    fillCompleteInfo: 'Please fill in complete plan information',
    meterNotExist: 'Selected meter does not exist',
    planAdded: 'Calibration plan added',
    startExecute: 'Start executing calibration plan',
    planCompleted: 'Calibration plan completed',
    exportSuccess: 'Calibration records exported',
    placeholder: {
      searchMeter: 'Search meter',
      selectStatus: 'Select status',
      startDate: 'Start date',
      endDate: 'End date',
      selectMeter: 'Select meter',
      selectDate: 'Select date',
      inputExecutor: 'Input executor',
      inputRemark: 'Input remark'
    }
  },
  dataBoardModule: {
    title: 'Data Dashboard',
    totalEnergy: 'Total Energy Consumption',
    electricity: 'Electricity',
    water: 'Water',
    projectOverview: 'Project Overview',
    projectTotal: 'Total Projects',
    meterTotal: 'Total Meters',
    alarmRecord: 'Alarm Records',
    equipmentStatus: 'Equipment Status',
    alarmInfo: 'Alarm Information',
    todayEnergyTrend: 'Today\'s Energy Trend',
    todayEnergyStatistics: 'Today\'s Energy Statistics',
    electricityKwh: 'Electricity(kW·h)',
    comprehensiveEnergy: 'Comprehensive Energy(kgce)',
    todayWaterUsage: 'Today\'s Water Usage(t)',
    dailyPowerCurve: 'Daily Power Curve',
    todayWaterInfo: 'Today\'s Water Information',
    normal: 'Normal',
    alarm: 'Alarm',
    offline: 'Offline',
    energyPlatform: 'Energy Platform',
    carbonDataVisualizationPlatform: 'Carbon Data Visualization Platform'
  },
  dispatchModule: {
    title: 'Dispatch Management',
    loadForecast: 'Load Forecast',
    priceForecast: 'Price Forecast',
    weatherForecast: 'Weather Forecast',
    modelManagement: 'Model Management',
    costSaving: 'Cost Saving',
    efficiencyImprovement: 'Efficiency Improvement',
    emissionReduction: 'Carbon Emission Reduction',
    trendAnalysis: 'Trend Analysis'
  },
  toolModule: {
    title: 'System Tools',
    basicInfo: 'Basic Information',
    columnInfo: 'Column Information',
    genInfo: 'Generation Information',
    fieldProperties: 'Field Properties',
    formProperties: 'Form Properties'
  },
  equipmentModule: {
    title: 'Equipment Management',
    equipmentInfo: 'Equipment Information',
    equipmentAlarm: 'Equipment Alarm',
    equipmentData: 'Equipment Data',
    equipmentDocuments: 'Equipment Documents'
  },
  dataQueryModule: {
    title: 'Data Query',
    dailyRawData: 'Daily Raw Data',
    dailyPeakData: 'Daily Peak Data'
  },
  componentsModule: {
    title: 'Components',
    icons: 'Icons',
    elementIcons: 'Element-UI Icons'
  },
  autoeeModule: {
    title: 'Automation',
    stockInRecord: 'Stock In Record',
    stockOutRecord: 'Stock Out Record'
  },
  alarmModule: {
    title: 'Alarm Management',
    alarmAnalysis: 'Alarm Analysis',
    alarmHistory: 'Alarm History',
    alarmRule: 'Alarm Rule',
    realtimeAlarm: 'Realtime Alarm',
    paramName: 'Parameter Name',
    alarmTime: 'Alarm Time',
    alarmInfo: 'Alarm Info',
    alarmLevel: 'Alarm Level',
    alarmArea: 'Alarm Area',
    alarmEquipment: 'Alarm Equipment',
    alarmVal: 'Alarm Value',
    endTime: 'End Time',
    alarmParam: 'Alarm Parameter',
    alarmCount: 'Alarm Count',
    alarmType: 'Alarm Type',
    eventType: 'Event Type',
    condition1: 'Condition 1',
    thresholdValue1: 'Threshold 1',
    condition2: 'Condition 2',
    thresholdValue2: 'Threshold 2',
    userId: 'Reminder',
    createOrderSwitch: 'Auto Create Order',
    alarmSwitch: 'Alarm Switch',
    alarmDesc: 'Alarm Description',
    addAlarm: 'Add Realtime Alarm',
    editAlarm: 'Edit Realtime Alarm',
    addAlarmRule: 'Add Alarm Rule',
    editAlarmRule: 'Edit Alarm Rule'
  },
  analysisReportModule: {
    title: 'Analysis Report',
    generateReport: 'Generate Analysis Report',
    print: 'Print',
    reportName: 'Analysis Report',
    energyStatistics: '1. Energy Statistics',
    electricityUsage: '2. Electricity Usage',
    electricityFee: '3. Electricity Fee',
    waterUsage: '4. Water Usage',
    waterFee: '5. Water Fee',
    recurringRate: '6. Recurring Rate',
    electricityTotal: 'During this period, total electricity consumption is {value}kW·h, maximum electricity consumption is {max}kW·h, maximum load occurred at {date}.',
    electricityFeeTotal: 'During this period, total electricity fee is ￥{value}',
    waterTotal: 'During this period, total water consumption is {value}t, maximum water consumption is {max}t, maximum flow occurred at {date}.',
    waterFeeTotal: 'During this period, total water fee is ￥{value}',
    suggestion: 'Suggestion: Increase monitoring circuits at the lower end of the transformer to collect end data, determine the energy consumption of each monitoring point, and check for electricity waste. Or replace lighting and other equipment in the building with energy-saving products to reduce energy consumption and achieve energy conservation and emission reduction.'
  },
  cameraModule: {
    title: 'Camera Management',
    realtimeVideo: 'Realtime Video',
    cameraConfig: 'Camera Configuration',
    cameraName: 'Camera Name',
    cameraBrand: 'Brand',
    cameraSn: 'Serial Number',
    cameraIp: 'Camera IP',
    cameraPort: 'Port',
    cameraUser: 'Username',
    cameraPassword: 'Password',
    cameraType: 'Camera Type',
    cameraStatus: 'Status',
    cameraLocation: 'Installation Location',
    addCamera: 'Add Camera',
    editCamera: 'Edit Camera'
  },
  carbonAssetsModule: {
    title: 'Carbon Assets Management',
    carbonAnalysis: 'Carbon Emission Analysis',
    monthlyCarbon: 'Monthly Carbon Emission',
    yearlyCarbon: 'Yearly Carbon Emission',
    currentMonth: 'Current Month',
    lastMonth: 'Last Month',
    currentYear: 'Current Year',
    lastYear: 'Last Year',
    trend: 'Trend',
    energyType: 'Energy Type',
    year: 'Year'
  },
  energyAnalysisModule: {
    title: 'Energy Analysis',
    energyOverview: 'Energy Overview',
    energyFlow: 'Energy Flow',
    energyTrend: 'Energy Trend',
    yoyAnalysis: 'YoY Analysis',
    monAnalysis: 'MoM Analysis',
    lossAnalysis: 'Loss Analysis',
    expenseBoard: 'Expense Dashboard',
    expenseReport: 'Expense Report',
    monthOnMonth: 'MoM',
    todayEnergy: 'Today Energy',
    yesterdayEnergy: 'Yesterday',
    currentMonthEnergy: 'Current Month',
    lastMonthEnergy: 'Last Month',
    currentYearEnergy: 'Current Year',
    lastYearEnergy: 'Last Year',
    trend: 'Trend',
    area: 'Area',
    energyType: 'Energy Type'
  },
  itemizedAnalysisModule: {
    title: 'Itemized Analysis',
    itemizedOverview: 'Itemized Overview',
    energyType: 'Energy Type',
    date: 'Date',
    search: 'Search'
  },
  maintenanceModule: {
    title: 'Maintenance Management',
    duty: 'Duty Management',
    inspectionPlan: 'Inspection Plan',
    inspectionRecord: 'Inspection Record',
    repairOrder: 'Repair Order',
    schedule: 'Schedule Management'
  },
  quotaModule: {
    title: 'Quota Management',
    analysis: 'Quota Analysis',
    config: 'Quota Configuration',
    monitor: 'Quota Monitor'
  },
  systemModule: {
    title: 'System Management',
    config: 'Parameter Configuration',
    dept: 'Department Management',
    dict: 'Dictionary Management',
    menu: 'Menu Management',
    notice: 'Notice Management',
    post: 'Position Management',
    role: 'Role Management',
    user: 'User Management'
  },
  digitaltwinModule: {
    title: 'Digital Twin'
  },
  inventoryModule: {
    title: 'Inventory Management',
    attachment: 'Attachment Management',
    purveyor: 'Supplier Management'
  },
  managementSystemModule: {
    title: 'Management System',
    prePlan: 'PrePlan Management',
    process: 'Process Management',
    regulation: 'Regulation Management',
    standard: 'Standard Management'
  },
  monitorModule: {
    title: 'System Monitor',
    admin: 'System Admin',
    cache: 'Cache Monitor',
    logininfor: 'Login Log',
    online: 'Online Users',
    operlog: 'Operation Log',
    xxljob: 'Scheduled Task'
  },
  newenergyModule: {
    title: 'New Energy',
    energyStorage: 'Energy Storage',
    microGrid: 'Micro Grid',
    pvStation: 'PV Station',
    storageBattery: 'Storage Battery'
  },
  reportModule: {
    title: 'Report Management',
    generate: 'Report Generate',
    template: 'Report Template'
  },
  patrolModule: {
    title: 'Patrol Management',
    patrolPoint: 'Patrol Point',
    patrolPath: 'Patrol Route',
    patrolPlan: 'Patrol Plan',
    patrolRecord: 'Patrol Record',
    patrolAlarm: 'Patrol Alarm',
    patrolTask: 'Patrol Task',
    pointName: 'Point Name',
    pointLocation: 'Point Location',
    pathName: 'Route Name',
    pathCode: 'Route Code',
    pathDesc: 'Route Description',
    pathPoints: 'Route Points',
    pathStatus: 'Route Status',
    planName: 'Plan Name',
    planDesc: 'Plan Description',
    planType: 'Plan Type',
    planCycle: 'Plan Cycle',
    planStartTime: 'Plan Start Time',
    planEndTime: 'Plan End Time',
    planStatus: 'Plan Status',
    planStartDate: 'Plan Start Date',
    planEndDate: 'Plan End Date',
    planCycleValue: 'Plan Cycle Value',
    taskName: 'Task Name',
    taskDesc: 'Task Description',
    taskStatus: 'Task Status',
    taskAssignee: 'Assignee',
    taskStartTime: 'Task Start Time',
    taskEndTime: 'Task End Time',
    recordTime: 'Patrol Time',
    recordResult: 'Patrol Result',
    recordDesc: 'Patrol Description',
    alarmName: 'Alarm Name',
    alarmDesc: 'Alarm Description',
    alarmLevel: 'Alarm Level',
    alarmStatus: 'Alarm Status',
    alarmTime: 'Alarm Time',
    addPoint: 'Add Patrol Point',
    editPoint: 'Edit Patrol Point',
    addPath: 'Add Patrol Route',
    editPath: 'Edit Patrol Route',
    addPlan: 'Add Patrol Plan',
    editPlan: 'Edit Patrol Plan',
    addTask: 'Add Patrol Task',
    editTask: 'Edit Patrol Task',
    recordAlarm: 'Record Alarm',
    viewRecord: 'View Patrol Record',
    pointNameRequired: 'Point name is required',
    pointLocationRequired: 'Point location is required',
    pathNameRequired: 'Route name is required',
    pathCodeRequired: 'Route code is required',
    planNameRequired: 'Plan name is required',
    taskNameRequired: 'Task name is required',
    alarmNameRequired: 'Alarm name is required',
    pointOrder: 'Point Order'
  },
  repairModule: {
    title: 'Repair Management',
    maintainOrder: 'Repair Order',
    orderNo: 'Order No.',
    orderType: 'Order Type',
    orderStatus: 'Order Status',
    orderDesc: 'Order Description',
    orderPriority: 'Priority',
    orderAssignee: 'Assignee',
    orderStartTime: 'Start Time',
    orderEndTime: 'Completion Time',
    orderSource: 'Order Source',
    equipmentName: 'Equipment Name',
    faultDesc: 'Fault Description',
    repairDesc: 'Repair Description',
    repairResult: 'Repair Result',
    addOrder: 'Add Order',
    editOrder: 'Edit Order',
    orderNoRequired: 'Order number is required',
    equipmentNameRequired: 'Equipment name is required',
    faultDescRequired: 'Fault description is required'
  },
  goodsModule: {
    title: 'Goods Management',
    goodsInfo: 'Goods Information',
    goodsInventory: 'Goods Inventory',
    goodsStockIn: 'Goods Stock In',
    goodsStockOut: 'Goods Stock Out',
    goodsName: 'Goods Name',
    goodsCode: 'Goods Code',
    goodsCategory: 'Goods Category',
    goodsUnit: 'Unit',
    goodsSpec: 'Specification',
    goodsPrice: 'Unit Price',
    goodsStock: 'Stock Quantity',
    goodsMinStock: 'Min Stock',
    goodsMaxStock: 'Max Stock',
    goodsDesc: 'Goods Description',
    stockInNo: 'Stock In No.',
    stockOutNo: 'Stock Out No.',
    stockInTime: 'Stock In Time',
    stockOutTime: 'Stock Out Time',
    stockInQty: 'Stock In Quantity',
    stockOutQty: 'Stock Out Quantity',
    stockInType: 'Stock In Type',
    stockOutType: 'Stock Out Type',
    stockInPerson: 'Stock In Person',
    stockOutPerson: 'Stock Out Person',
    stockInReason: 'Stock In Reason',
    stockOutReason: 'Stock Out Reason',
    addGoods: 'Add Goods',
    editGoods: 'Edit Goods',
    stockIn: 'Stock In',
    stockOut: 'Stock Out',
    goodsNameRequired: 'Goods name is required',
    goodsCodeRequired: 'Goods code is required',
    goodsUnitRequired: 'Unit is required'
  },
  dangerGoodsModule: {
    title: 'Dangerous Goods Management',
    dangerGoodsInfo: 'Dangerous Goods Information',
    dangerGoodsInventory: 'Dangerous Goods Inventory',
    dangerGoodsStockIn: 'Dangerous Goods Stock In',
    dangerGoodsStockOut: 'Dangerous Goods Stock Out',
    dangerGoodsName: 'Dangerous Goods Name',
    dangerGoodsStatus: 'Dangerous Goods Status',
    dangerType: 'Dangerous Goods Type',
    specification: 'Specification',
    storageLocation: 'Storage Location',
    currentStock: 'Current Stock',
    dangerStockInNo: 'Stock In No.',
    dangerStockInQty: 'Stock In Quantity',
    dangerStockOutNo: 'Stock Out No.',
    dangerStockOutQty: 'Stock Out Quantity',
    dangerStockOutReason: 'Stock Out Reason',
    dangerLevel: 'Danger Level',
    safetyDesc: 'Safety Instructions',
    storageRequirement: 'Storage Requirements',
    addDangerGoods: 'Add Dangerous Goods',
    editDangerGoods: 'Edit Dangerous Goods',
    dangerGoodsNameRequired: 'Dangerous goods name is required',
    dangerLevelRequired: 'Danger level is required'
  },
  gatewayModule: {
    title: 'Gateway Management',
    gatewayName: 'Gateway Name',
    gatewayCode: 'Gateway Code',
    gatewayIp: 'Gateway IP',
    gatewayPort: 'Gateway Port',
    gatewayStatus: 'Gateway Status',
    gatewayDesc: 'Gateway Description',
    onlineStatus: 'Online Status',
    lastOnlineTime: 'Last Online Time',
    addGateway: 'Add Gateway',
    editGateway: 'Edit Gateway',
    gatewayNameRequired: 'Gateway name is required',
    gatewayCodeRequired: 'Gateway code is required'
  },
  hospital: {
    homeTitle: 'Hospital Smart Energy Decision System',
    homeDesc: 'Collect operation and energy consumption data of hospital examination and testing equipment via IOT platform callback, providing device ledger, metric definition and data monitoring capabilities to support energy analysis and decision making.',
    deviceManage: 'Device Ledger',
    deviceManageDesc: 'Manage CT/MRI/DR/Ultrasound/LAB/DSA device profiles with IOT device binding',
    metricManage: 'Metric Definition',
    metricManageDesc: 'Define metric code, name, unit and data type to standardize callback data parsing',
    callbackLogManage: 'Callback Logs',
    callbackLogDesc: 'View IOT platform callback records, processing status and failure reasons to ensure reliable data ingestion',
    deviceName: 'Device Name',
    deviceCode: 'Device Code',
    deviceType: 'Device Type',
    model: 'Model',
    manufacturer: 'Manufacturer',
    iotDeviceId: 'IOT Device ID',
    iotDeviceIdPlaceholder: 'Enter IOT platform device ID (leave blank to unbind)',
    unbound: 'Unbound',
    bindIot: 'IOT Bind',
    viewData: 'Data Points',
    statusNormal: 'Normal',
    statusDisabled: 'Disabled',
    statusOffline: 'Offline',
    deviceNameRequired: 'Device name is required',
    deviceCodeRequired: 'Device code is required',
    deviceTypeRequired: 'Device type is required',
    addDevice: 'Add Device',
    editDevice: 'Edit Device',
    metricCode: 'Metric Code',
    metricName: 'Metric Name',
    unit: 'Unit',
    dataType: 'Data Type',
    highFreq: 'High Freq',
    yes: 'Yes',
    no: 'No',
    dataTypeNumber: 'Number',
    dataTypeStatus: 'Status',
    dataTypeString: 'String',
    metricCodeRequired: 'Metric code is required',
    metricNameRequired: 'Metric name is required',
    addMetric: 'Add Metric',
    editMetric: 'Edit Metric',
    requestId: 'Request ID',
    sourceIp: 'Source IP',
    deviceCount: 'Devices',
    pointCount: 'Data Points',
    callbackStatus: 'Status',
    callbackSuccess: 'Success',
    callbackAuthFail: 'Auth Failed',
    callbackParseFail: 'Parse Failed',
    callbackFail: 'Failed',
    errorMsg: 'Error',
    costTime: 'Cost',
    receiveTime: 'Receive Time',
    metricValue: 'Value',
    ts: 'Collect Time',
    quality: 'Quality',
    qualityGood: 'Good',
    qualityBad: 'Bad',
    monitorManage: 'Device Monitor',
    monitorManageDesc: 'Real-time online status, running status and energy data of inspection devices',
    onlineStatus: 'Online Status',
    online: 'Online',
    offline: 'Offline',
    noData: 'No Data',
    runStatus: 'Running Status',
    running: 'Running',
    standby: 'Standby',
    lastPower: 'Power(kW)',
    lastElectricity: 'Energy(kWh)',
    lastUpdate: 'Last Data Time',
    openAlarm: 'Open Alarms',
    viewTrend: 'Trend',
    trendData: 'Trend Data',
    autoRefresh: 'Auto Refresh',
    alarmRuleManage: 'Alarm Rules',
    alarmRuleManageDesc: 'Configure overload/offline alarm rules and notifications',
    ruleName: 'Rule Name',
    ruleType: 'Rule Type',
    thresholdRule: 'Threshold Alarm',
    offlineRule: 'Offline Alarm',
    condition: 'Condition',
    conditionG: 'Greater(>)',
    conditionGE: 'GreaterEqual(>=)',
    conditionL: 'Less(<)',
    conditionLE: 'LessEqual(<=)',
    conditionE: 'Equal(=)',
    thresholdValue: 'Threshold',
    offlineTimeoutMin: 'Offline Timeout(min)',
    level: 'Level',
    levelNormal: 'Normal',
    levelSerious: 'Serious',
    levelUrgent: 'Urgent',
    enabled: 'Enabled',
    disabled: 'Disabled',
    notifyEmail: 'Notify Email',
    notifyEmailPlaceholder: 'Comma-separated emails, empty means record only',
    addRule: 'Add Rule',
    editRule: 'Edit Rule',
    allDevices: 'All Devices',
    ruleNameRequired: 'Rule name is required',
    ruleTypeRequired: 'Rule type is required',
    alarmRecordManage: 'Alarm Records',
    alarmRecordManageDesc: 'View and handle device alarm records',
    alarmType: 'Alarm Type',
    overload: 'Overload',
    offlineAlarm: 'Offline',
    handleStatus: 'Handle Status',
    pending: 'Pending',
    closed: 'Closed',
    alarmContent: 'Content',
    alarmVal: 'Trigger Value',
    startTime: 'Start Time',
    endTime: 'End Time',
    handle: 'Handle',
    handleRemark: 'Handle Remark',
    handleBy: 'Handled By',
    energyManage: 'Energy Analysis',
    energyManageDesc: 'Hospital energy overview, trends, rankings and comparisons',
    efficiencyManage: 'Efficiency Evaluation',
    efficiencyManageDesc: 'Device efficiency evaluation, saving suggestions and report export',
    timeRange: 'Time Range',
    drillLevel: 'Drill Level',
    levelArea: 'By Area',
    levelDept: 'By Dept',
    levelDevice: 'By Device',
    energyOverview: 'Energy Overview',
    dimension: 'Dimension',
    kwh: 'Consumption(kWh)',
    avgPower: 'Avg Power(kW)',
    totalKwh: 'Total Consumption',
    chainRatio: 'Chain Ratio(%)',
    energyTrend: 'Energy Trend',
    energyRank: 'Device Consumption Ranking',
    rankNo: 'No.',
    efficiencyTitle: 'Device Efficiency Evaluation',
    standbyRatio: 'Standby Ratio(%)',
    effScore: 'Score',
    effLevel: 'Level',
    suggestionTitle: 'Energy Saving Suggestions',
    noSuggestion: 'No suggestions for this period',
    exportEfficiency: 'Export Efficiency',
    exportSuggestions: 'Export Suggestions',
    areaManage: 'Area Management',
    areaManageDesc: 'Multi-campus data isolation & authorization',
    areaName: 'Campus Name',
    areaCode: 'Campus Code',
    areaType: 'Campus Type',
    areaTypeCampus: 'Campus',
    areaTypeBuilding: 'Building',
    areaNameRequired: 'Please enter campus name',
    areaCodeRequired: 'Please enter campus code',
    addArea: 'Add Campus',
    editArea: 'Edit Campus',
    workloadManage: 'Workload Management',
    workloadManageDesc: 'Device workload & unit energy consumption',
    workloadCount: 'Workload',
    workloadDate: 'Stat Date',
    addWorkload: 'Add Workload',
    editWorkload: 'Edit Workload',
    deviceRequired: 'Please select device',
    workloadDateRequired: 'Please select stat date',
    unitEnergy: 'Unit Workload Energy',
    screenManage: 'Big Screen',
    screenManageDesc: 'Energy visualization dashboard',
    screenTitle: 'Hospital Smart Energy Screen',
    recentAlarm: 'Recent Alarms',
    escalated: 'escalated',
    energyCategory: 'Energy by Category',
    energyCategoryTrend: 'Category Trend',
    percent: 'Percent',
    hsNone: 'Pending',
    hsConfirmed: 'Confirmed',
    hsProcessing: 'Processing',
    hsDone: 'Handled',
    confirm: 'Confirm',
    process: 'Process',
    action_confirm: 'Confirm Alarm',
    action_process: 'Process Alarm',
    action_done: 'Complete',
    escalation: 'Escalation',
    escalateMin: 'Escalate After (min)',
    escalateLevel: 'Escalate Level',
    min: 'min',
    dashTitle: 'Role Dashboard',
    dashSub: 'Dynamically show data modules based on the current user\'s role and permissions',
    currentRole: 'Current Role',
    roleAdmin: 'Super Admin',
    roleAreaAdmin: 'Campus Admin',
    roleEnergyAdmin: 'Energy Admin',
    roleDeviceAdmin: 'Device Admin',
    roleCommon: 'Common User',
    dashOverview: 'Campus Energy Overview',
    dashAreaCompare: 'Campus Comparison',
    dashDeviceStatus: 'Device Status',
    dashCategory: 'Energy Category',
    dashTrend: 'Energy Trend',
    dashRank: 'Device Rank',
    dashEfficiency: 'Unit Energy Efficiency',
    dashAlarm: 'Recent Alarms',
    deviceOnline: 'Online',
    deviceFault: 'Fault',
    viewMore: 'View More'
  }
}
