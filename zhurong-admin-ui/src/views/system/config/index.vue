<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item :label="$t('form.configName')" prop="configName">
        <el-input
      ...
      <el-form-item :label="$t('form.configKey')" prop="configKey">
        <el-input
      ...
      <el-form-item :label="$t('common.systemBuiltIn')" prop="configType">
        <el-select v-model="queryParams.configKey" :placeholder="$t('common.systemBuiltIn')" clearable>
          <el-option
    ...
    <el-table v-loading="loading" :data="configList" @selection-change="handleSelectionChange">
      ...
      <el-table-column :label="$t('table.configId')" align="center" prop="configId" />
      <el-table-column :label="$t('table.configName')" align="center" prop="configName" :show-overflow-tooltip="true" />
      <el-table-column :label="$t('table.configKey')" align="center" prop="configKey" :show-overflow-tooltip="true" />
      <el-table-column :label="$t('table.configValue')" align="center" prop="configValue" :show-overflow-tooltip="true" />
      <el-table-column :label="$t('common.systemBuiltIn')" align="center" prop="configType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_yes_no" :value="scope.row.configType"/>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.remark')" align="center" prop="remark" :show-overflow-tooltip="true" />
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item v-if="form.configId == 2" label="参数键值" prop="configValue">
          <el-radio-group v-model="form.configValue">
            <el-radio style="margin-bottom: 8px;" v-for="(logo, index) in sysLogos" :key="index" :label="logo.ossUrl">
              <ImagePreview
                :width=100 :height=100
                :src="logo.ossUrl"
                :preview-src-list="[logo.ossUrl]"/>
            </el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="form.configId == 3" label="参数键值" prop="configValue">
          <el-radio-group v-model="form.configValue">
            <el-radio style="margin-bottom: 8px;" v-for="(logo, index) in browerLogos" :key="index" :label="logo.ossUrl">
              <ImagePreview
                :width=100 :height=100
                :src="logo.ossUrl"
                :preview-src-list="[logo.ossUrl]"/>
...
import {listLogo} from "@/api/system/logoInfo"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        configName: [
          { required: true, message: this.$t('rules.configNameRequired'), trigger: "blur" }
        ],
        configKey: [
          { required: true, message: this.$t('rules.configKeyRequired'), trigger: "blur" }
        ],
        configValue: [
          { required: true, message: this.$t('rules.configValueRequired'), trigger: "blur" }
        ]
      },
      sysLogos: [],
      browerLogos: []
  ...
  methods: {
    ...
    getLogoList() {
      listLogo().then(res => {
        this.sysLogos = res.rows.filter(item => item.type == '0')
        this.browerLogos = res.rows.filter(item => item.type == '1')
      })
    }
  }
};
