<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
      ...
      <el-form-item :label="$t('form.roleKey')" prop="roleKey">
        <el-input
          v-model="queryParams.roleKey"
          :placeholder="$t('placeholder.roleKey')"
          clearable
          style="width: 240px"
          @keyup.enter.native="handleQuery"
        />
          ...
          <el-dropdown size="mini" @command="(command) => handleCommand(command, scope.row)" v-hasPermi="['system:role:edit']">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="handleDataScope" icon="el-icon-circle-check"
                v-hasPermi="['system:role:edit']">{{ $t('form.dataScope') }}</el-dropdown-item>
              <el-dropdown-item command="handleAuthUser" icon="el-icon-user"
                v-hasPermi="['system:role:edit']">{{ $t('system.role.authUser') }}</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item :label="$t('form.roleName')" prop="roleName">
          <el-input v-model="form.roleName" :placeholder="$t('placeholder.roleName')" />
        </el-form-item>
        <el-form-item prop="roleKey">
          ...
          <el-input v-model="form.roleKey" :placeholder="$t('placeholder.roleKey')" />
        </el-form-item>
        <el-form-item :label="$t('table.roleSort')" prop="roleSort">
        ...
        <el-form-item :label="$t('form.menuPermissions')">
          <el-checkbox v-model="menuExpand" @change="handleCheckedTreeExpand($event, 'menu')">展开/折叠</el-checkbox>
          <el-checkbox v-model="menuNodeAll" @change="handleCheckedTreeNodeAll($event, 'menu')">{{ $t('common.selectAll') }}/{{ $t('common.empty') }}</el-checkbox>
    ...
    <el-dialog :title="title" :visible.sync="openDataScope" width="500px" append-to-body>
      <el-form :model="form" label-width="80px">
        <el-form-item :label="$t('form.roleName')">
          <el-input v-model="form.roleName" :disabled="true" />
        </el-form-item>
        <el-form-item :label="$t('form.roleKey')">
          <el-input v-model="form.roleKey" :disabled="true" />
        </el-form-item>
        <el-form-item :label="$t('form.dataScope')">
        ...
        <el-form-item :label="$t('form.dataScope')" v-show="form.dataScope == 2">
          <el-checkbox v-model="deptExpand" @change="handleCheckedTreeExpand($event, 'dept')">展开/折叠</el-checkbox>
          <el-checkbox v-model="deptNodeAll" @change="handleCheckedTreeNodeAll($event, 'dept')">{{ $t('common.selectAll') }}/{{ $t('common.empty') }}</el-checkbox>
...
export default {
  ...
  data() {
    return {
      ...
      dataScopeOptions: [
        {
          value: "1",
          label: this.$t('system.role.dataScope1')
        },
        {
          value: "2",
          label: this.$t('system.role.dataScope2')
        },
        {
          value: "3",
          label: this.$t('system.role.dataScope3')
        },
        {
          value: "4",
          label: this.$t('system.role.dataScope4')
        },
        {
          value: "5",
          label: this.$t('system.role.dataScope5')
        }
      ],
      // 菜单列表
      menuOptions: [],
      ...
      rules: {
        roleName: [
          { required: true, message: this.$t('rules.roleNameRequired'), trigger: "blur" }
        ],
        roleKey: [
          { required: true, message: this.$t('rules.roleKeyRequired'), trigger: "blur" }
        ],
        roleSort: [
          { required: true, message: this.$t('rules.roleSortRequired'), trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm(this.$t('confirm.delete') + ' "' + text + '""' + row.roleName + '"?').then(function() {
    ...
    handleDataScope(row) {
      ...
      getRole(row.roleId).then(response => {
        ...
        this.title = this.$t('form.dataScope');
      });
    },
    /** 分配用户操作 */
