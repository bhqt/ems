  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body @close="cancel">
      <el-form ref="form" :model="form" :rules="rules" label-width="110px">
        ...
        <el-form-item label="计划巡检时间">
          <el-date-picker clearable v-model="form.planTime" type="datetime" format="yyyy-MM-dd HH:mm"
            value-format="yyyy-MM-dd HH:mm" :placeholder="$t('common.pleaseSelect')">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="巡检周期">
          <el-select v-model="form.inspectionCycle" :placeholder="$t('common.pleaseSelect')">
            <el-option v-for="item in inspectionCycleList" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="巡检人">
          <el-select v-model="form.userIds" multiple :placeholder="$t('common.pleaseSelect')" @change="personChange">
            <el-option v-for="item in userList" :key="item.userId" :label="item.nickName"
              :value="item.userId"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="巡检状态" prop="inspectionStatus" v-if="form.id != null">
          <el-select v-model="form.inspectionStatus" :placeholder="$t('common.pleaseSelect')" clearable
        ...
        <el-form-item label="实际巡检时间">
          <el-date-picker clearable v-model="form.inspectionTime" type="datetime" format="yyyy-MM-dd HH:mm"
            value-format="yyyy-MM-dd HH:mm" :placeholder="$t('common.pleaseSelect')">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="巡检内容">
          <el-input type="textarea" v-model="form.inspectionRemark" :placeholder="$t('common.pleaseInput')" />
        </el-form-item>
      </el-form>
    ...
    <el-drawer
      ...
      <div style="height:calc(100% - 72px);padding: 0px 20px;overflow:auto;">
        ...
        <el-form ref="form" :model="form" :rules="rules" label-width="110px">
          ...
          <div style="font-size:14px;">
            <el-form-item label="计划名称" prop="planName">
              <el-input v-model="form.planName" placeholder="暂无内容" :disabled="disabled"></el-input>
            </el-form-item>
            <el-form-item label="项目名称" prop="projectName">
              <el-input v-model="form.projectName" placeholder="暂无内容" :disabled="disabled"></el-input>
            </el-form-item>
            <el-form-item label="巡检内容" prop="planContent">
              <el-input type="textarea" rows="3" v-model="form.planContent" placeholder="暂无内容" :disabled="disabled"></el-input>
            </el-form-item>
            <el-form-item label="计划巡检时间" prop="planTime">
              <el-date-picker :disabled="disabled" clearable v-model="form.planTime" type="datetime" format="yyyy-MM-dd HH:mm"
          ...
          <div style="font-size:14px;">
            <el-form-item label="巡检人">
              <el-select v-model="form.userIds" multiple placeholder="暂无内容" @change="personChange" :disabled="disabled" style="width:100%;">
                <el-option v-for="item in userList" :key="item.userId" :label="item.nickName"
                  :value="item.userId"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="实际巡检时间" :prop="dealWith ? 'inspectionTime' : ''">
              <el-date-picker :disabled="disabled&&!dealWith" clearable v-model="form.inspectionTime" type="datetime" format="yyyy-MM-dd HH:mm"
                value-format="yyyy-MM-dd HH:mm" placeholder="暂无内容">
              </el-date-picker>
            </el-form-item>
            <el-form-item label="巡检记录">
              <el-input type="textarea" rows="3" v-model="form.inspectionRemark" placeholder="暂无内容" :disabled="disabled&&!dealWith"></el-input>
            </el-form-item>
          </div>
        </el-form>
      </div>
      <div class="drawer-footer" style="padding: 20px;">
        ...
        <el-button v-if="!disabled" type="primary" size="small" @click="saveForm" icon="el-icon-edit-outline" :loading="buttonLoading">{{ $t('common.save') }}</el-button>
        <el-button v-if="dealWith" type="primary" size="small" @click="submitForm" icon="el-icon-finished" :loading="buttonLoading">完 成</el-button>
      </div>
    </el-drawer>
