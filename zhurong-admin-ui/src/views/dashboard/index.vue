    ...
    <div class="app-container">
      <div class="container-left">
            ...
            <div class="status-item flex-column-around">
              <div class="status-normal flex-column-evenly">
                <div class="flex-between">
                  <span class="text-icon">{{ $t('dashboard.normalDevice') }}</span>
                  <span>{{ $t('dashboard.proportion') }}</span>
                </div>
                <div class="flex-between">
              ...
              <div class="status-alarm flex-column-evenly">
                <div class="flex-between">
                  <span class="text-icon">{{ $t('dashboard.alarmDevice') }}</span>
                  <span>{{ $t('dashboard.proportion') }}</span>
                </div>
                <div class="flex-between">
              ...
              <div class="status-offLine flex-column-evenly">
                <div class="flex-between">
                  <span class="text-icon">{{ $t('dashboard.offlineDevice') }}</span>
                  <span>{{ $t('dashboard.proportion') }}</span>
                </div>
                <div class="flex-between">
        ...
        <el-card class="alarm-info" shadow="hover">
          <div slot="header">
            <span>{{ $t('dashboard.alarmInfo') }}</span>
            <el-button style="padding: 3px 0;position:absolute;right:12px;" type="text" @click="toAlarm">{{ $t('dashboard.more') }}</el-button>
          </div>
          <alarmInfo/>
        </el-card>
      </div>
      <div class="container-right">
        <el-card class="energy-statistic" shadow="hover">
          <div slot="header">
            <span>{{ $t('dashboard.energyStatistic') }}</span>
          </div>
          <div class="statistic-content">
            <div class="column-one">
              <lightning theme="outline" size="35" class="park-icon"/>
              <div>{{ electricityTotal }}</div>
              <div>{{ $t('dashboard.electricity') }}</div>
            </div>
            <div class="column-two">
              <dashboard theme="filled" size="35" class="park-icon"/>
              <div>{{ powerTotal}}</div>
              <div>{{ $t('dashboard.comprehensiveEnergy') }}</div>
            </div>
            <div class="column-three">
              <cycle theme="filled" size="35" class="park-icon"/>
              <div>{{ carbonTotal}}</div>
              <div>{{ $t('dashboard.carbonEmission') }}</div>
            </div>
          </div>
        </el-card>
        <el-card class="energy-trend" shadow="hover">
          <div slot="header">
            <span>{{ $t('dashboard.energyTrend') }}</span>
          </div>
          <div class="trend-content">
            <el-tabs v-model="activeName" @tab-click="handleClick">
              <el-tab-pane :label="$t('dashboard.totalEnergy')" name="total">
                <barChart v-if="activeName==='total'" :height="'100%'" :xName="$t('dashboard.time')" :yName="'kgce'" :xData="xData" :yData="energyData"/>
              </el-tab-pane>
              <el-tab-pane :label="$t('dashboard.electricity')" name="electricity">
                <barChart v-if="activeName==='electricity'" :height="'100%'" :xName="$t('dashboard.time')" :yName="'kM·h'" :xData="xData" :yData="electricityData"/>
              </el-tab-pane>
              <el-tab-pane :label="$t('dashboard.water')" name="water">
                <barChart v-if="activeName==='water'" :height="'100%'" :xName="$t('dashboard.time')" :yName="'t'" :xData="xData" :yData="waterData"/>
              </el-tab-pane>
            </el-tabs>
          </div>
        </el-card>
        <el-card class="carbon-emissions" shadow="hover">
          <div slot="header">
            <span>{{ $t('dashboard.carbonEmissions') }}</span>
          </div>
          <barChart :height="'100%'" :xName="$t('dashboard.time')" :yName="'kg'" :xData="xData" :yData="carbonData"/>
        </el-card>
      </div>
    </div>
  </div>
  ...
  methods: {
    getEquipmentData() {
      getAllStatus().then(res => {
        ...
        this.pieData = [
          {value: res.data[0].count, name: this.$t('dashboard.normalDevice'), itemStyle: {color: '#1890ff'}},
          {value: res.data[1].count, name: this.$t('dashboard.alarmDevice'), itemStyle: {color: '#ff4949'}},
          {value: res.data[2].count, name: this.$t('dashboard.offlineDevice'), itemStyle: {color: '#ffba00'}},
        ]
      })
    },
    /** 查询历史报警列表 */
    ...
    initMap() {
      ...
      // var infoWindow = new BMapGL.InfoWindow('智慧软件', opts)
