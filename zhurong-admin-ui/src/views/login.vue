  ...
  <div class="login-mobile" v-if="isMobile">
    ...
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form">
      <h3 class="title">
        ...
        <!-- <h2>{{ sysTitle }}</h2> -->
        <h2 style="font-weight: 700; margin: 0;">{{ $t('login.title') }}</h2>
      </h3>
      <el-form-item prop="username">
        <el-input v-model="loginForm.username" type="text" auto-complete="off" :placeholder="$t('placeholder.username')" class="unchanged">
          <svg-icon slot="prefix" icon-class="user" class="el-input__icon input-icon" />
        </el-input>
      </el-form-item>
      <el-form-item prop="password">
        <el-input v-model="loginForm.password" type="password" auto-complete="off" :placeholder="$t('placeholder.password')"
          @keyup.enter.native="handleLogin" show-password class="unchanged">
          <svg-icon slot="prefix" icon-class="password" class="el-input__icon input-icon" />
        </el-input>
      </el-form-item>
      <el-form-item prop="code" v-if="captchaEnabled">
        <el-input v-model="loginForm.code" auto-complete="off" :placeholder="$t('placeholder.captcha')" style="width: 63%"
      ...
      <div class="select-click">
        <el-checkbox v-model="loginForm.rememberMe" style="margin:0px 0px 25px 0px;" class="unchanged">{{ $t('login.remember') }}</el-checkbox>
      </div>
      <el-form-item style="width:100%;">
        ...
        <div style="float: right;" v-if="register">
          <router-link class="link-type" :to="'/register'">{{ $t('login.register') || '立即注册' }}</router-link>
        </div>
        <router-link class="link-type" :to="'/applyAccount'" style="font-weight:bold"
          v-if="experienceShow">{{ $t('login.experience') || '获取体验账号' }}</router-link>
      </el-form-item>
    </el-form>
    <!--  底部  -->
    <div class="el-login-footer">
      <!-- <span>苏州智慧软件技术有限公司 © 版权所有</span> -->
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getCode() {
      getCodeImg().then(res => {
        console.log("获取验证码：res=", JSON.stringify(res))
