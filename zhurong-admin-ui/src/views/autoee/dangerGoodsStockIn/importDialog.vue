...
export default {
    ...
    return {
      pageData: {
        ...
        importData: {
          ...
          url: process.env.VUE_APP_BASE_API + "/autoee/dangerGoodsStockIn/importDangerGoodsStockInData",
          actionUrlExtend: "",
          fileSelected: false
        }
      }
  ...
  methods: {
    /** 导入按钮操作 */
    openDangerGoodsStockInImportDialog(parentQueryParams) {

      if (dangerGoodsStockInExtend.openImportDialogExtend(this, parentQueryParams)) {
    ...
    importTemplate() {
      ...
      this.download('autoee/dangerGoodsStockIn/downLoadImportTemplateDangerGoodsStockIn', {},
        `危化品入库记录_导入模版_${fileExt}.xlsx`)
    },
  ...
  mounted() {
    // mounted扩展方法
    dangerGoodsStockInExtend.importMountedStartExtend(this);
  }
};
</script>
