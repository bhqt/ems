<template>
  <div class="app-container">
    <!-- 新增或修改危化品入库记录对话框  去掉高度设置  style="height: 800px"-->
    <el-dialog class="ruoyi_dialog" :title="title" :visible.sync="open" width="1000px" append-to-body>
      ...
      <el-form ref="addUpdateFormRef"  :model="addUpdateForm" :rules="addUpdateFormRules" label-width="150px" style="padding-right: 30px" :scroll-to-error="true">
                ...
                <el-form-item v-show='true' label="存放地点" prop="supplier" style="display: inline-block;width: 45%;">
                      <el-input v-model="addUpdateForm.supplier" :placeholder="$t('common.pleaseInput')" maxlength="50" show-word-limit clearable/>
                </el-form-item>
...
export default {
  ...
  methods: {
    ...
    /** 打开新增窗口 */
    openDangerGoodsStockInAddDialog(parentQueryParams, dicts) {
      this.dicts = dicts;
      this.reset();
      this.title = "添加危化品入库记录";
      this.addOrUpdate = "add";
      dangerGoodsStockInExtend.openAddDialogExtend(this, parentQueryParams);
      this.open = true;
    },
    /** 打开修改窗口 */
    openDangerGoodsStockInUpdateDialog(id, dicts) {
      this.reset();
      dangerGoodsStockIn.selectDataByPkDangerGoodsStockIn(id).then(response => {
        this.addUpdateForm = response.data;
        this.title = "修改危化品入库记录";
      ...
      this.$refs.addUpdateFormRef.validate(valid => {
        if (valid) {
          if (this.addUpdateForm.id != null) {
            dangerGoodsStockIn.updateNullValueByDangerGoodsStockIn(this.addUpdateForm).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.$emit("addUpdateSubmitCallback");
            });
          } else {
            dangerGoodsStockIn.addDangerGoodsStockIn(this.addUpdateForm).then(response => {
  ...
  mounted() {
    // mounted扩展方法
    dangerGoodsStockInExtend.addUpdateMountedStartExtend(this);
  }
}
</script>
