<template>
  <div class="app-container">
    ...
    >
      <!-- 循环生成查询表单项 -->
      <el-form-item class="a_query_form_item" label="入库编号" prop="id">
        <el-input
          clearable
          placeholder="请输入入库编号"
      ...
      <el-form-item class="a_query_form_item" label="存放地点" prop="supplier">
        <el-input
          clearable
          placeholder="请输入存放地点"
          v-model="pageData.queryParams.supplier"
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item class="a_query_form_item_datetime_between" label="更新时间">
        <el-date-picker
    ...
    <el-row :gutter="10" class="mb8">
      <el-col v-if="true" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleAdd"
          v-hasPermi="['autoee:dangerGoodsStockIn:add']"
        >新增</el-button>
      </el-col>
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          @click="handleUpdate"
          v-hasPermi="['autoee:dangerGoodsStockIn:edit']"
        >编辑</el-button>
      </el-col>
      <el-col v-if="false" :span="1.5">
        <el-button
      ...
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          v-hasPermi="['autoee:dangerGoodsStockIn:remove']"
        >删除</el-button>
      </el-col>
      <el-col v-if="false" :span="1.5">
        <el-button
      ...
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleImport"
          v-hasPermi="['autoee:dangerGoodsStockIn:import']"
        >导入</el-button>
      </el-col>
      <el-col v-if="true" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleExport"
          v-hasPermi="['autoee:dangerGoodsStockIn:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="0.5">
    ...
    <el-table
      ...
      :data="pageData.dangerGoodsStockInList"
    ...
    >
      ...
      <el-table-column label="入库编号" align="center" prop="id" />
      ...
      <el-table-column v-if="true" label="入库数量" align="center" prop="quantity" sortable="custom" :sort-orders="['descending', 'ascending']"/>
      <el-table-column v-if="true" label="存放地点" align="center" prop="supplier" sortable="custom" :sort-orders="['descending', 'ascending']"/>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-if="true" size="small" link icon="el-icon-edit" type="text"
            @click="handleUpdate(scope.row)" v-hasPermi="['autoee:dangerGoodsStockIn:edit']"
          >编辑
          </el-button>
          <el-button v-if="true" size="small" link icon="el-icon-zoom-in" type="text"
            @click="handleShowDetail(scope.row)" v-hasPermi="['autoee:dangerGoodsStockIn:list']"
          >查看
          </el-button>
          <el-button v-if="true" size="small" link icon="el-icon-delete" type="text"
            @click="handleDelete(scope.row)" v-hasPermi="['autoee:dangerGoodsStockIn:remove']"
          >删除
          </el-button>
        </template>
      </el-table-column>
...
import dangerGoodsStockInExtend from "@/api/autoee/dangerGoodsStockInExtend";
...
export default {
  name: 'DangerGoodsStockIn',
  dicts: ['sys_user', 'a_danger_goods_info', 'sys_dept'],
  components: {
    addUpdateDialog,
    detailDialog,
    importDialog,
  ...
  methods: {
    ...
    getList(operateFlag) {
      ...
      dangerGoodsStockInExtend.indexOperateSuccessExtend(this, operateFlag)
      ...
      dangerGoodsStockIn.selectPageListDangerGoodsStockIn(this.pageData.queryParams).then(response => {
        this.pageData.dangerGoodsStockInList = response.rows;
    ...
    /** 新增按钮操作 */
    async handleAdd() {
        this.$refs.addUpdateDialogRef.openDangerGoodsStockInAddDialog(this.pageData.queryParams);
    },

    /** 修改按钮操作 */
    async handleUpdate(row) {
        const id = row ? row.id : this.pageData.selectedIds;
        this.$refs.addUpdateDialogRef.openDangerGoodsStockInUpdateDialog(id);
    },
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('确认删除当前选中的记录吗？').then(() => {
        return dangerGoodsStockIn.deleteDangerGoodsStockInByIds(idsArr);
      }).then(() => {
        this.getList("delete");
    ...
    handleDeleteAllData() {
      ...
      this.$modal.confirm('警告：此操作将清空全部数据！\n清空后不可恢复！\n确定要继续吗？')
        ...
        .then(({ value }) => {
          ...
          if (value.trim() === '确认清空') {
            // 执行删除操作
            return dangerGoodsStockIn.deleteDangerGoodsStockInAllData();
          } else {
    ...
    handleShowDetail(row) {
      this.$refs.detailDialogRef.openDangerGoodsStockInDetailDialog(row);
    },

    /** 导入按钮操作 */
    handleImport() {
      this.$refs.importDialogRef.openDangerGoodsStockInImportDialog(this.pageData.queryParams);
    },
    ...
    handleExport() {
      this.$modal.confirm('确认导出当前查询条件下的所有记录吗？').then(() => {
        ...
        return this.download('autoee/dangerGoodsStockIn/exportDangerGoodsStockIn', {...this.pageData.queryParams},
          `危化品入库记录_导出_${fileExt}.xlsx`);
      }).catch((error) => {
    ...
    tableSummaryMethod(param) {
      return dangerGoodsStockInExtend.tableSummaryMethodExtend(this, param);
    },
  ...
  mounted() {
    ...
    // mounted扩展方法
    //dangerGoodsStockInExtend.indexMountedStartExtend(this);
  }
}
</script>
