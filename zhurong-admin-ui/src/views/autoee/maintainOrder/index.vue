<template>
  <div class="app-container">
    ...
    <el-table
      ...
      :data="pageData.maintainOrderList"
...
export default {
  ...
  data() {
    ...
    dateRanges.dateRangeReportTime = [];
  ...
  methods: {
    ...
    getList(operateFlag) {
      ...
      maintainOrderExtend.indexOperateSuccessExtend(this, operateFlag)
      this.pageData.loadingListData = true;

      if (this.dateRangeReportTime !== null && this.dateRangeReportTime !== '') {
        this.pageData.queryParams.params["beginReportTime"] = this.dateRangeReportTime[0];
        this.pageData.queryParams.params["endReportTime"] = this.dateRangeReportTime[1];
      } else {
        this.pageData.queryParams.params["beginReportTime"] = null;
        this.pageData.queryParams.params["endReportTime"] = null;
      }

      maintainOrder.selectPageListMaintainOrder(this.pageData.queryParams).then(response => {
        this.pageData.maintainOrderList = response.rows;
    ...
    /** 重置按钮操作 */
    resetQuery() {
      this.dateRangeReportTime = [this.baseBeginTime, this.baseEndTime];
    ...
    /** 新增按钮操作 */
    async handleAdd() {
        this.$refs.addUpdateDialogRef.openMaintainOrderAddDialog(this.pageData.queryParams);
    },

    /** 修改按钮操作 */
    async handleUpdate(row) {
        const id = row ? row.id : this.pageData.selectedIds;
        this.$refs.addUpdateDialogRef.openMaintainOrderUpdateDialog(id);
    },
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('确认删除当前选中的记录吗？').then(() => {
        return maintainOrder.deleteMaintainOrderByIds(idsArr);
      }).then(() => {
        this.getList("delete");
    ...
    handleDeleteAllData() {
      ...
      this.$modal.confirm('警告：此操作将清空全部数据！\n清空后不可恢复！\n确定要继续吗？')
        ...
        .then(({ value }) => {
          ...
          if (value.trim() === '确认清空') {
            // 执行删除操作
            return maintainOrder.deleteMaintainOrderAllData();
          } else {
    ...
    handleShowDetail(row) {
      this.$refs.detailDialogRef.openMaintainOrderDetailDialog(row);
    },

    /** 导入按钮操作 */
    handleImport() {
      this.$refs.importDialogRef.openMaintainOrderImportDialog(this.pageData.queryParams);
    },
    ...
    handleExport() {
      this.$modal.confirm('确认导出当前查询条件下的所有记录吗？').then(() => {
        ...
        return this.download('autoee/maintainOrder/exportMaintainOrder', {...this.pageData.queryParams},
          `维修工单_导出_${fileExt}.xlsx`);
      }).catch((error) => {
    ...
    tableSummaryMethod(param) {
      return maintainOrderExtend.tableSummaryMethodExtend(this, param);
    },
  ...
  mounted() {
    ...
    // mounted扩展方法
    //maintainOrderExtend.indexMountedStartExtend(this);
  }
}
</script>
