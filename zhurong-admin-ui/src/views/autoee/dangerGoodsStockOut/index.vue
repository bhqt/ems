<template>
  <div class="app-container">
    ...
    >
      <!-- 循环生成查询表单项 -->
      <el-form-item class="a_query_form_item" label="出库编号" prop="id">
        <el-input
          clearable
          placeholder="请输入出库编号"
      ...
      <el-form-item class="a_query_form_item" label="入库编号" prop="dangerGoodsStockInId">
        <el-select
          class="a_query_form_select"
          v-model="pageData.queryParams.dangerGoodsStockInId"
          placeholder="请选择"
          clearable
        >
          <el-option
      ...
      <el-form-item class="a_query_form_item" label="操作人员" prop="userId">
        <el-select
          class="a_query_form_select"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col v-if="true" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleAdd"
          v-hasPermi="['autoee:dangerGoodsStockOut:add']"
        >新增</el-button>
      </el-col>
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          @click="handleUpdate"
          v-hasPermi="['autoee:dangerGoodsStockOut:edit']"
        >编辑</el-button>
      </el-col>
      <el-col v-if="false" :span="1.5">
        <el-button
      ...
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          v-hasPermi="['autoee:dangerGoodsStockOut:remove']"
        >删除</el-button>
      </el-col>
      <el-col v-if="false" :span="1.5">
        <el-button
      ...
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleImport"
          v-hasPermi="['autoee:dangerGoodsStockOut:import']"
        >导入</el-button>
      </el-col>
      <el-col v-if="true" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleExport"
          v-hasPermi="['autoee:dangerGoodsStockOut:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="0.5">
    ...
    <el-table
      ...
      :data="pageData.dangerGoodsStockOutList"
    ...
    >
      ...
      <el-table-column label="出库编号" align="center" prop="id" />
      <el-table-column v-if="true" label="入库编号" align="center" prop="dangerGoodsStockInId" sortable="custom" :sort-orders="['descending', 'ascending']">
      ...
      <el-table-column v-if="true" label="出库数量" align="center" prop="quantity" sortable="custom" :sort-orders="['descending', 'ascending']"/>
      <el-table-column v-if="true" label="出库原因" align="center" prop="reason" :show-overflow-tooltip="true" sortable="custom" :sort-orders="['descending', 'ascending']"/>
      ...
      <el-table-column v-if="true" label="操作人员" align="center" prop="userId" sortable="custom" :sort-orders="['descending', 'ascending']">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-if="true" size="small" link icon="el-icon-edit" type="text"
            @click="handleUpdate(scope.row)" v-hasPermi="['autoee:dangerGoodsStockOut:edit']"
          >编辑
          </el-button>
          <el-button v-if="true" size="small" link icon="el-icon-zoom-in" type="text"
            @click="handleShowDetail(scope.row)" v-hasPermi="['autoee:dangerGoodsStockOut:list']"
          >查看
          </el-button>
          <el-button v-if="true" size="small" link icon="el-icon-delete" type="text"
            @click="handleDelete(scope.row)" v-hasPermi="['autoee:dangerGoodsStockOut:remove']"
          >删除
          </el-button>
        </template>
      </el-table-column>
...
export default {
    ...
    return {
      ...
      pageData: {
        dicts: {
          sys_user: [],
          a_danger_goods_info: [],
          a_danger_goods_stock_in: [],
          sys_dept: [],
        },
        dangerGoodsStockOutList: [],
  ...
  methods: {
    ...
    getList(operateFlag) {
      ...
      dangerGoodsStockOutExtend.indexOperateSuccessExtend(this, operateFlag)
      this.pageData.loadingListData = true;


      dangerGoodsStockOut.selectPageListDangerGoodsStockOut(this.pageData.queryParams).then(response => {
        this.pageData.dangerGoodsStockOutList = response.rows;
    ...
    /** 新增按钮操作 */
    async handleAdd() {
        this.$refs.addUpdateDialogRef.openDangerGoodsStockOutAddDialog(this.pageData.queryParams);
    },

    /** 修改按钮操作 */
    async handleUpdate(row) {
        const id = row ? row.id : this.pageData.selectedIds;
        this.$refs.addUpdateDialogRef.openDangerGoodsStockOutUpdateDialog(id);
    },
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('确认删除当前选中的记录吗？').then(() => {
        return dangerGoodsStockOut.deleteDangerGoodsStockOutByIds(idsArr);
      }).then(() => {
        this.getList("delete");
    ...
    handleDeleteAllData() {
      ...
      this.$modal.confirm('警告：此操作将清空全部数据！\n清空后不可恢复！\n确定要继续吗？')
        ...
        .then(({ value }) => {
          ...
          if (value.trim() === '确认清空') {
            // 执行删除操作
            return dangerGoodsStockOut.deleteDangerGoodsStockOutAllData();
          } else {
    ...
    handleShowDetail(row) {
      this.$refs.detailDialogRef.openDangerGoodsStockOutDetailDialog(row);
    },

    /** 导入按钮操作 */
    handleImport() {
      this.$refs.importDialogRef.openDangerGoodsStockOutImportDialog(this.pageData.queryParams);
    },
    ...
    handleExport() {
      this.$modal.confirm('确认导出当前查询条件下的所有记录吗？').then(() => {
        ...
        return this.download('autoee/dangerGoodsStockOut/exportDangerGoodsStockOut', {...this.pageData.queryParams},
          `危化品出库记录_导出_${fileExt}.xlsx`);
      }).catch((error) => {
    ...
    tableSummaryMethod(param) {
      return dangerGoodsStockOutExtend.tableSummaryMethodExtend(this, param);
    },
  ...
  mounted() {
    ...
    // mounted扩展方法
    //dangerGoodsStockOutExtend.indexMountedStartExtend(this);
  }
}
</script>
