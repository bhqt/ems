...
export default {
    ...
    return {
      pageData: {
        ...
        importData: {
          ...
          url: process.env.VUE_APP_BASE_API + "/autoee/dangerGoodsStockOut/importDangerGoodsStockOutData",
          actionUrlExtend: "",
          fileSelected: false
        }
      }
  ...
  methods: {
    /** 导入按钮操作 */
    openDangerGoodsStockOutImportDialog(parentQueryParams) {

      if (dangerGoodsStockOutExtend.openImportDialogExtend(this, parentQueryParams)) {
    ...
    importTemplate() {
      ...
      this.download('autoee/dangerGoodsStockOut/downLoadImportTemplateDangerGoodsStockOut', {},
        `危化品出库记录_导入模版_${fileExt}.xlsx`)
    },
  ...
  mounted() {
    // mounted扩展方法
    dangerGoodsStockOutExtend.importMountedStartExtend(this);
  }
};
</script>
