<template>
  <div class="app-container">
    ...
    <el-table
      ...
      :data="pageData.dangerGoodsInventoryList"
    ...
    <dangerGoodsInOutHistoryDialog ref="dangerGoodsInOutHistoryDialogRef" />
  </div>
</template>
<script>
...
import dangerGoodsInventoryExtend from "@/api/autoee/dangerGoodsInventoryExtend";

// 导入组件
import addUpdateDialog from './addUpdateDialog.vue';
import detailDialog from './detailDialog.vue';
import importDialog from './importDialog.vue';
import dangerGoodsInOutHistoryDialog from './dangerGoodsInOutHistoryDialog.vue';

// 部门选择器相关（如果有）

export default {
  name: 'DangerGoodsInventory',
  dicts: ['danger_goods_type', 'goods_unit', 'sys_user', 'a_danger_goods_info', 'sys_dept'],
  components: {
    addUpdateDialog,
    detailDialog,
    importDialog,
    dangerGoodsInOutHistoryDialog
  },
  data() {
    // 获取当前日期
  ...
  methods: {
    ...
    getList(operateFlag) {
      ...
      dangerGoodsInventoryExtend.indexOperateSuccessExtend(this, operateFlag)
      this.pageData.loadingListData = true;


      dangerGoodsInventory.selectPageListDangerGoodsInventory(this.pageData.queryParams).then(response => {
        this.pageData.dangerGoodsInventoryList = response.rows;
    ...
    /** 新增按钮操作 */
    async handleAdd() {
        this.$refs.addUpdateDialogRef.openDangerGoodsInventoryAddDialog(this.pageData.queryParams);
    },

    /** 修改按钮操作 */
    async handleUpdate(row) {
        const id = row ? row.id : this.pageData.selectedIds;
        this.$refs.addUpdateDialogRef.openDangerGoodsInventoryUpdateDialog(id);
    },
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('确认删除当前选中的记录吗？').then(() => {
        return dangerGoodsInventory.deleteDangerGoodsInventoryByIds(idsArr);
      }).then(() => {
        this.getList("delete");
    ...
    handleDeleteAllData() {
      ...
      this.$modal.confirm('警告：此操作将清空全部数据！\n清空后不可恢复！\n确定要继续吗？')
        ...
        .then(({ value }) => {
          ...
          if (value.trim() === '确认清空') {
            // 执行删除操作
            return dangerGoodsInventory.deleteDangerGoodsInventoryAllData();
          } else {
    ...
    handleShowDetail(row) {
      this.$refs.detailDialogRef.openDangerGoodsInventoryDetailDialog(row);
    },

    /** 导入按钮操作 */
    handleImport() {
      this.$refs.importDialogRef.openDangerGoodsInventoryImportDialog(this.pageData.queryParams);
    },
    ...
    handleExport() {
      this.$modal.confirm('确认导出当前查询条件下的所有记录吗？').then(() => {
        ...
        return this.download('autoee/dangerGoodsInventory/exportDangerGoodsInventory', {...this.pageData.queryParams},
          `危化品库存_导出_${fileExt}.xlsx`);
      }).catch((error) => {
    ...
    /** 显示危化品出入库历史 */
    showDangerGoodsInOutHistory(row) {
      this.$refs.dangerGoodsInOutHistoryDialogRef.openDialog(row);
    },

    /** 表格合计方法 */
    tableSummaryMethod(param) {
      return dangerGoodsInventoryExtend.tableSummaryMethodExtend(this, param);
    },
  ...
  mounted() {
    ...
    // mounted扩展方法
    //dangerGoodsInventoryExtend.indexMountedStartExtend(this);
  }
}
</script>
