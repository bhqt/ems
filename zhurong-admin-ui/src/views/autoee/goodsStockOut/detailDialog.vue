<template>
  <div class="app-container">
    <!--查看物品出库记录详细对话框 去掉高度设置  style="height: 800px"-->
        ...
        <el-row :gutter="20">
          ...
          <el-form-item label="出库编号：" prop="id">
              <div>{{ pageData.detailForm.id }}</div>
		        ...
		        <el-col v-if="true" :span="12">
          <el-form-item label="入库编号：" prop="goodsStockInId">
            <div >
              <dict-tag :options="dict.type.a_goods_stock_in" :value="pageData.detailForm.goodsStockInId" :show-css="false"/>
            </div>
          </el-form-item>
        </el-col>
		        <el-col v-if="true" :span="12">
...
export default {
  ...
  methods: {
    /** 打开详细窗口 */
    openGoodsStockOutDetailDialog(row) {
      goodsStockOut.selectDetailByPkGoodsStockOut(row.id).then(response => {
        this.pageData.detailForm = response.data;
        this.pageData.open = true;
        this.pageData.title = "查看物品出库记录";
      });
    },
