...
export default {
    ...
    return {
      pageData: {
        ...
        importData: {
          ...
          url: process.env.VUE_APP_BASE_API + "/autoee/goodsStockOut/importGoodsStockOutData",
          actionUrlExtend: "",
          fileSelected: false
        }
      }
  ...
  methods: {
    /** 导入按钮操作 */
    openGoodsStockOutImportDialog(parentQueryParams) {

      if (goodsStockOutExtend.openImportDialogExtend(this, parentQueryParams)) {
    ...
    importTemplate() {
      ...
      this.download('autoee/goodsStockOut/downLoadImportTemplateGoodsStockOut', {},
        `物品出库记录_导入模版_${fileExt}.xlsx`)
    },
  ...
  mounted() {
    // mounted扩展方法
    goodsStockOutExtend.importMountedStartExtend(this);
  }
};
</script>
