...
export default {
    ...
    return {
      pageData: {
        ...
        importData: {
          ...
          url: process.env.VUE_APP_BASE_API + "/autoee/patrolRecord/importPatrolRecordData",
          actionUrlExtend: "",
          fileSelected: false
        }
      }
  ...
  methods: {
    /** 导入按钮操作 */
    openPatrolRecordImportDialog(parentQueryParams) {

      if (patrolRecordExtend.openImportDialogExtend(this, parentQueryParams)) {
    ...
    importTemplate() {
      ...
      this.download('autoee/patrolRecord/downLoadImportTemplatePatrolRecord', {},
        `巡更记录_导入模版_${fileExt}.xlsx`)
    },
  ...
  mounted() {
    // mounted扩展方法
    patrolRecordExtend.importMountedStartExtend(this);
  }
};
</script>
