<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item class="a_query_form_item" label="巡更路线" prop="patrolPathId">
        <el-select
          class="a_query_form_select"
      ...
      <el-form-item class="a_query_form_item" label="巡更结果" prop="patrolResult">
        <el-select
          class="a_query_form_select"
          v-model="pageData.queryParams.patrolResult"
          placeholder="请选择"
          clearable
        >
          <el-option
    ...
    <el-row :gutter="10" class="mb8">
      <el-col v-if="true" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleAdd"
          v-hasPermi="['autoee:patrolRecord:add']"
        >新增</el-button>
      </el-col>
      <el-col v-if="true" :span="1.5">
        <el-button
          ...
          @click="handleUpdate"
          v-hasPermi="['autoee:patrolRecord:edit']"
        >编辑</el-button>
      </el-col>
      ...
      <el-col v-if="true" :span="1.5">
        <el-button
          ...
          v-hasPermi="['autoee:patrolRecord:remove']"
        >删除</el-button>
      </el-col>
      <el-col v-if="false" :span="1.5">
        <el-button
      ...
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleImport"
          v-hasPermi="['autoee:patrolRecord:import']"
        >导入</el-button>
      </el-col>
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleExport"
          v-hasPermi="['autoee:patrolRecord:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="0.5">
    ...
    <el-table
      ...
      :data="pageData.patrolRecordList"
    ...
    >
      ...
      <el-table-column v-if="true" label="巡更路线" align="center" prop="patrolPathId" sortable="custom" :sort-orders="['descending', 'ascending']">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.a_patrol_path" :value="scope.row.patrolPathId" />
        </template>
      </el-table-column>
      <el-table-column v-if="true" label="巡更点位" align="center" prop="patrolPointId" sortable="custom" :sort-orders="['descending', 'ascending']">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.a_patrol_point" :value="scope.row.patrolPointId" />
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="true" label="点位顺序" align="center" prop="pointOrder" sortable="custom" :sort-orders="['descending', 'ascending']"/>
      <el-table-column v-if="true" label="点位巡更时间" align="center" prop="arriveTime" sortable="custom" :sort-orders="['descending', 'ascending']">
        <template slot-scope="scope">
          <span>{{ $common.formatDateTime(scope.row.arriveTime) }}</span>
        </template>
      </el-table-column>
      <el-table-column v-if="false" label="巡更结果" align="center" prop="patrolResult" sortable="custom" :sort-orders="['descending', 'ascending']">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.patrol_result" :value="scope.row.patrolResult"/>
        </template>
      </el-table-column>
      <el-table-column v-if="false" label="结果描述" align="center" prop="resultDesc" :show-overflow-tooltip="true" sortable="custom" :sort-orders="['descending', 'ascending']"/>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-if="true" size="small" link icon="el-icon-edit" type="text"
            @click="handleUpdate(scope.row)" v-hasPermi="['autoee:patrolRecord:edit']"
          >编辑
          </el-button>
          <el-button v-if="true" size="small" link icon="el-icon-zoom-in" type="text"
            @click="handleShowDetail(scope.row)" v-hasPermi="['autoee:patrolRecord:list']"
          >查看
          </el-button>
          <el-button v-if="true" size="small" link icon="el-icon-delete" type="text"
            @click="handleDelete(scope.row)" v-hasPermi="['autoee:patrolRecord:remove']"
          >删除
          </el-button>
        </template>
      </el-table-column>
...
export default {
    ...
    return {
      ...
      pageData: {
        ...
        rules: {
          ...
          patrolPointId: [
            { required: true, message: "巡更点位不能为空", trigger: "blur" }
          ],
          patrolUserId: [
  ...
  methods: {
    ...
    getList(operateFlag) {
      ...
      patrolRecordExtend.indexOperateSuccessExtend(this, operateFlag)
      this.pageData.loadingListData = true;


      patrolRecord.selectPageListPatrolRecord(this.pageData.queryParams).then(response => {
        this.pageData.patrolRecordList = response.rows;
    ...
    /** 新增按钮操作 */
    async handleAdd() {
        this.$refs.addUpdateDialogRef.openPatrolRecordAddDialog(this.pageData.queryParams);
    },

    /** 修改按钮操作 */
    async handleUpdate(row) {
        const id = row ? row.id : this.pageData.selectedIds;
        this.$refs.addUpdateDialogRef.openPatrolRecordUpdateDialog(id);
    },
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('确认删除当前选中的记录吗？').then(() => {
        return patrolRecord.deletePatrolRecordByIds(idsArr);
      }).then(() => {
        this.getList("delete");
    ...
    handleDeleteAllData() {
      ...
      this.$modal.confirm('警告：此操作将清空全部数据！\n清空后不可恢复！\n确定要继续吗？')
        ...
        .then(({ value }) => {
          ...
          if (value.trim() === '确认清空') {
            // 执行删除操作
            return patrolRecord.deletePatrolRecordAllData();
          } else {
    ...
    handleShowDetail(row) {
      this.$refs.detailDialogRef.openPatrolRecordDetailDialog(row);
    },

    /** 导入按钮操作 */
    handleImport() {
      this.$refs.importDialogRef.openPatrolRecordImportDialog(this.pageData.queryParams);
    },
    ...
    handleExport() {
      this.$modal.confirm('确认导出当前查询条件下的所有记录吗？').then(() => {
        ...
        return this.download('autoee/patrolRecord/exportPatrolRecord', {...this.pageData.queryParams},
          `巡更记录_导出_${fileExt}.xlsx`);
      }).catch((error) => {
    ...
    tableSummaryMethod(param) {
      return patrolRecordExtend.tableSummaryMethodExtend(this, param);
    },
  ...
  mounted() {
    ...
    // mounted扩展方法
    //patrolRecordExtend.indexMountedStartExtend(this);
  }
}
</script>
