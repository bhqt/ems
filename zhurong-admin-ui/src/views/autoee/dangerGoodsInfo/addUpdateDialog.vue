<template>
  <div class="app-container">
    <!-- 新增或修改危化品信息管理对话框  去掉高度设置  style="height: 800px"-->
    <el-dialog class="ruoyi_dialog" :title="title" :visible.sync="open" width="1000px" append-to-body>
      ...
      <el-form ref="addUpdateFormRef"  :model="addUpdateForm" :rules="addUpdateFormRules" label-width="150px" style="padding-right: 30px" :scroll-to-error="true">
                <el-form-item v-show='true' label="危化品名称" prop="dangerGoodsName" style="display: inline-block;width: 45%;">
                      <el-input v-model="addUpdateForm.dangerGoodsName" :placeholder="$t('common.pleaseInput')" maxlength="50" show-word-limit clearable/>
                </el-form-item>
                  <el-form-item v-show='true' label="危化品类型" prop="dangerGoodsType" style="display: inline-block;width: 45%;">
                  ...
                  <el-form-item v-show='true' label="危化品状态" prop="dangerGoodsStatus" style="display: inline-block;width: 45%;">
                    <el-select clearable v-model="addUpdateForm.dangerGoodsStatus" :placeholder="$t('common.pleaseSelect')" style="width: 100%">
                      <el-option
                ...
                <el-form-item v-show='true' label="存储位置" prop="storageLocation" style="display: inline-block;width: 45%;">
                      <el-input v-model="addUpdateForm.storageLocation" :placeholder="$t('common.pleaseInput')" maxlength="200" show-word-limit clearable/>
                </el-form-item>
...
export default {
  ...
  methods: {
    ...
    /** 打开新增窗口 */
    openDangerGoodsInfoAddDialog(parentQueryParams, dicts) {
      this.dicts = dicts;
      this.reset();
      this.title = "添加危化品信息管理";
      this.addOrUpdate = "add";
      dangerGoodsInfoExtend.openAddDialogExtend(this, parentQueryParams);
      this.open = true;
    },
    /** 打开修改窗口 */
    openDangerGoodsInfoUpdateDialog(id, dicts) {
      this.reset();
      dangerGoodsInfo.selectDataByPkDangerGoodsInfo(id).then(response => {
        this.addUpdateForm = response.data;
        this.title = "修改危化品信息管理";
      ...
      this.$refs.addUpdateFormRef.validate(valid => {
        if (valid) {
          if (this.addUpdateForm.id != null) {
            dangerGoodsInfo.updateNullValueByDangerGoodsInfo(this.addUpdateForm).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.$emit("addUpdateSubmitCallback");
            });
          } else {
            dangerGoodsInfo.addDangerGoodsInfo(this.addUpdateForm).then(response => {
  ...
  mounted() {
    // mounted扩展方法
    dangerGoodsInfoExtend.addUpdateMountedStartExtend(this);
  }
}
</script>
