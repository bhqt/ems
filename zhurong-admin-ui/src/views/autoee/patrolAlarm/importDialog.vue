...
export default {
    ...
    return {
      pageData: {
        ...
        importData: {
          ...
          url: process.env.VUE_APP_BASE_API + "/autoee/patrolAlarm/importPatrolAlarmData",
          actionUrlExtend: "",
          fileSelected: false
        }
      }
  ...
  methods: {
    /** 导入按钮操作 */
    openPatrolAlarmImportDialog(parentQueryParams) {

      if (patrolAlarmExtend.openImportDialogExtend(this, parentQueryParams)) {
    ...
    importTemplate() {
      ...
      this.download('autoee/patrolAlarm/downLoadImportTemplatePatrolAlarm', {},
        `巡更报警_导入模版_${fileExt}.xlsx`)
    },
  ...
  mounted() {
    // mounted扩展方法
    patrolAlarmExtend.importMountedStartExtend(this);
  }
};
</script>
