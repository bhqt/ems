...
export default {
    ...
    return {
      pageData: {
        ...
        importData: {
          ...
          url: process.env.VUE_APP_BASE_API + "/autoee/patrolTask/importPatrolTaskData",
          actionUrlExtend: "",
          fileSelected: false
        }
      }
  ...
  methods: {
    /** 导入按钮操作 */
    openPatrolTaskImportDialog(parentQueryParams) {

      if (patrolTaskExtend.openImportDialogExtend(this, parentQueryParams)) {
    ...
    importTemplate() {
      ...
      this.download('autoee/patrolTask/downLoadImportTemplatePatrolTask', {},
        `巡更任务_导入模版_${fileExt}.xlsx`)
    },
  ...
  mounted() {
    // mounted扩展方法
    patrolTaskExtend.importMountedStartExtend(this);
  }
};
</script>
