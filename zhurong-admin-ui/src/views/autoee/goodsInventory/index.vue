<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item class="a_query_form_item" :label="$t('goodsModule.goodsCategory')" prop="goodsType">
        <el-select
          class="a_query_form_select"
      ...
      <el-form-item class="a_query_form_item" :label="$t('goodsModule.goodsSpec')" prop="specification">
        <el-input
          clearable
          :placeholder="$t('goodsModule.goodsSpec')"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleAdd"
          v-hasPermi="['autoee:goodsInventory:add']"
      ...
      <el-col v-if="true" :span="1.5">
        <el-button
          ...
          @click="handleUpdate"
          v-hasPermi="['autoee:goodsInventory:edit']"
      ...
      <el-col v-if="true" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleExport"
          v-hasPermi="['autoee:goodsInventory:export']"
    ...
    <el-table
      ...
      :data="pageData.goodsInventoryList"
    ...
    >
      ...
      <el-table-column v-if="true" :label="$t('goodsModule.goodsName')" align="center" prop="goodsId" sortable="custom" :sort-orders="['descending', 'ascending']">
        <template slot-scope="scope">
          <el-button type="text" @click="showGoodsInOutHistory(scope.row)" style="padding: 0">
            <dict-tag :options="dict.type.a_goods_info" :value="scope.row.goodsId" />
          </el-button>
        </template>
      </el-table-column>
      <el-table-column v-if="true" :label="$t('goodsModule.goodsCategory')" align="center" prop="goodsType" sortable="custom" :sort-orders="['descending', 'ascending']">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.goods_type" :value="scope.row.goodsType"/>
        </template>
      </el-table-column>
      <el-table-column v-if="true" :label="$t('goodsModule.goodsSpec')" align="center" prop="specification" sortable="custom" :sort-orders="['descending', 'ascending']"/>
      <el-table-column v-if="true" :label="$t('goodsModule.goodsStock')" align="center" prop="currentStock" sortable="custom" :sort-orders="['descending', 'ascending']"/>
      ...
      <el-table-column :label="$t('common.operation')" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-if="true" size="small" link icon="el-icon-edit" type="text"
            @click="handleUpdate(scope.row)" v-hasPermi="['autoee:goodsInventory:edit']"
          >{{ $t('common.edit') }}
          </el-button>
          <el-button v-if="true" size="small" link icon="el-icon-zoom-in" type="text"
            @click="handleShowDetail(scope.row)" v-hasPermi="['autoee:goodsInventory:list']"
          >{{ $t('common.view') }}
          </el-button>
          <el-button v-if="true" size="small" link icon="el-icon-delete" type="text"
            @click="handleDelete(scope.row)" v-hasPermi="['autoee:goodsInventory:remove']"
          >{{ $t('common.delete') }}
          </el-button>
        </template>
      </el-table-column>
    ...
    <goodsInOutHistoryDialog ref="goodsInOutHistoryDialogRef" />
  </div>
</template>
<script>
...
import goodsInOutHistoryDialog from './goodsInOutHistoryDialog.vue';
...
export default {
  ...
  methods: {
    ...
    getList(operateFlag) {
      ...
      goodsInventoryExtend.indexOperateSuccessExtend(this, operateFlag)
      this.pageData.loadingListData = true;


      goodsInventory.selectPageListGoodsInventory(this.pageData.queryParams).then(response => {
        this.pageData.goodsInventoryList = response.rows;
    ...
    /** 新增按钮操作 */
    async handleAdd() {
        this.$refs.addUpdateDialogRef.openGoodsInventoryAddDialog(this.pageData.queryParams);
    },

    /** 修改按钮操作 */
    async handleUpdate(row) {
        const id = row ? row.id : this.pageData.selectedIds;
        this.$refs.addUpdateDialogRef.openGoodsInventoryUpdateDialog(id);
    },
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('确认删除当前选中的记录吗？').then(() => {
        return goodsInventory.deleteGoodsInventoryByIds(idsArr);
      }).then(() => {
        this.getList("delete");
    ...
    handleDeleteAllData() {
      ...
      this.$modal.confirm('警告：此操作将清空全部数据！\n清空后不可恢复！\n确定要继续吗？')
        ...
        .then(({ value }) => {
          ...
          if (value.trim() === '确认清空') {
            // 执行删除操作
            return goodsInventory.deleteGoodsInventoryAllData();
          } else {
    ...
    handleShowDetail(row) {
      this.$refs.detailDialogRef.openGoodsInventoryDetailDialog(row);
    },

    /** 显示物品出入库历史记录 */
    showGoodsInOutHistory(row) {
      this.$refs.goodsInOutHistoryDialogRef.openDialog(row);
    },

    /** 导入按钮操作 */
    handleImport() {
      this.$refs.importDialogRef.openGoodsInventoryImportDialog(this.pageData.queryParams);
    },
    ...
    handleExport() {
      this.$modal.confirm('确认导出当前查询条件下的所有记录吗？').then(() => {
        ...
        return this.download('autoee/goodsInventory/exportGoodsInventory', {...this.pageData.queryParams},
          `物品库存_导出_${fileExt}.xlsx`);
      }).catch((error) => {
    ...
    tableSummaryMethod(param) {
      return goodsInventoryExtend.tableSummaryMethodExtend(this, param);
    },
  ...
  mounted() {
    ...
    // mounted扩展方法
    //goodsInventoryExtend.indexMountedStartExtend(this);
  }
}
</script>
