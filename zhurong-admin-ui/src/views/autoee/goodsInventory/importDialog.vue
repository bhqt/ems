...
export default {
    ...
    return {
      pageData: {
        ...
        importData: {
          ...
          url: process.env.VUE_APP_BASE_API + "/autoee/goodsInventory/importGoodsInventoryData",
          actionUrlExtend: "",
          fileSelected: false
        }
      }
  ...
  methods: {
    /** 导入按钮操作 */
    openGoodsInventoryImportDialog(parentQueryParams) {

      if (goodsInventoryExtend.openImportDialogExtend(this, parentQueryParams)) {
    ...
    importTemplate() {
      ...
      this.download('autoee/goodsInventory/downLoadImportTemplateGoodsInventory', {},
        `物品库存_导入模版_${fileExt}.xlsx`)
    },
  ...
  mounted() {
    // mounted扩展方法
    goodsInventoryExtend.importMountedStartExtend(this);
  }
};
</script>
