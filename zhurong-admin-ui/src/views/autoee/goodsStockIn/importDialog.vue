...
export default {
    ...
    return {
      pageData: {
        ...
        importData: {
          ...
          url: process.env.VUE_APP_BASE_API + "/autoee/goodsStockIn/importGoodsStockInData",
          actionUrlExtend: "",
          fileSelected: false
        }
      }
  ...
  methods: {
    /** 导入按钮操作 */
    openGoodsStockInImportDialog(parentQueryParams) {

      if (goodsStockInExtend.openImportDialogExtend(this, parentQueryParams)) {
    ...
    importTemplate() {
      ...
      this.download('autoee/goodsStockIn/downLoadImportTemplateGoodsStockIn', {},
        `物品入库记录_导入模版_${fileExt}.xlsx`)
    },
  ...
  mounted() {
    // mounted扩展方法
    goodsStockInExtend.importMountedStartExtend(this);
  }
};
</script>
