<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item class="a_query_form_item" label="物品名称" prop="goodsId">
        <el-select
    ...
    <el-row :gutter="10" class="mb8">
      <el-col v-if="true" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleAdd"
          v-hasPermi="['autoee:goodsStockIn:add']"
        >新增</el-button>
      </el-col>
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          @click="handleUpdate"
          v-hasPermi="['autoee:goodsStockIn:edit']"
        >编辑</el-button>
      </el-col>
      <el-col v-if="false" :span="1.5">
        <el-button
      ...
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          v-hasPermi="['autoee:goodsStockIn:remove']"
        >删除</el-button>
      </el-col>
      <el-col v-if="false" :span="1.5">
        <el-button
      ...
      <el-col v-if="false" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleImport"
          v-hasPermi="['autoee:goodsStockIn:import']"
        >导入</el-button>
      </el-col>
      <el-col v-if="true" :span="1.5">
        <el-button
          ...
          size="small"
          @click="handleExport"
          v-hasPermi="['autoee:goodsStockIn:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="0.5">
    ...
    <el-table
      ...
      :data="pageData.goodsStockInList"
    ...
    >
      ...
      <el-table-column v-if="true" label="物品名称" align="center" prop="goodsId" sortable="custom" :sort-orders="['descending', 'ascending']">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-if="true" size="small" link icon="el-icon-edit" type="text"
            @click="handleUpdate(scope.row)" v-hasPermi="['autoee:goodsStockIn:edit']"
          >编辑
          </el-button>
          <el-button v-if="true" size="small" link icon="el-icon-zoom-in" type="text"
            @click="handleShowDetail(scope.row)" v-hasPermi="['autoee:goodsStockIn:list']"
          >查看
          </el-button>
          <el-button v-if="true" size="small" link icon="el-icon-delete" type="text"
            @click="handleDelete(scope.row)" v-hasPermi="['autoee:goodsStockIn:remove']"
          >删除
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList(operateFlag) {
      ...
      goodsStockInExtend.indexOperateSuccessExtend(this, operateFlag)
      ...
      goodsStockIn.selectPageListGoodsStockIn(this.pageData.queryParams).then(response => {
        this.pageData.goodsStockInList = response.rows;
    ...
    /** 新增按钮操作 */
    async handleAdd() {
        this.$refs.addUpdateDialogRef.openGoodsStockInAddDialog(this.pageData.queryParams);
    },

    /** 修改按钮操作 */
    async handleUpdate(row) {
        const id = row ? row.id : this.pageData.selectedIds;
        this.$refs.addUpdateDialogRef.openGoodsStockInUpdateDialog(id);
    },
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('确认删除当前选中的记录吗？').then(() => {
        return goodsStockIn.deleteGoodsStockInByIds(idsArr);
      }).then(() => {
        this.getList("delete");
    ...
    handleDeleteAllData() {
      ...
      this.$modal.confirm('警告：此操作将清空全部数据！\n清空后不可恢复！\n确定要继续吗？')
        ...
        .then(({ value }) => {
          ...
          if (value.trim() === '确认清空') {
            // 执行删除操作
            return goodsStockIn.deleteGoodsStockInAllData();
          } else {
    ...
    handleShowDetail(row) {
      this.$refs.detailDialogRef.openGoodsStockInDetailDialog(row);
    },

    /** 导入按钮操作 */
    handleImport() {
      this.$refs.importDialogRef.openGoodsStockInImportDialog(this.pageData.queryParams);
    },
    ...
    handleExport() {
      this.$modal.confirm('确认导出当前查询条件下的所有记录吗？').then(() => {
        ...
        return this.download('autoee/goodsStockIn/exportGoodsStockIn', {...this.pageData.queryParams},
          `物品入库记录_导出_${fileExt}.xlsx`);
      }).catch((error) => {
    ...
    tableSummaryMethod(param) {
      return goodsStockInExtend.tableSummaryMethodExtend(this, param);
    },
  ...
  mounted() {
    ...
    // mounted扩展方法
    //goodsStockInExtend.indexMountedStartExtend(this);
  }
}
</script>
