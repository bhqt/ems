...
export default {
    ...
    return {
      pageData: {
        ...
        importData: {
          ...
          url: process.env.VUE_APP_BASE_API + "/autoee/patrolPlan/importPatrolPlanData",
          actionUrlExtend: "",
          fileSelected: false
        }
      }
  ...
  methods: {
    /** 导入按钮操作 */
    openPatrolPlanImportDialog(parentQueryParams) {

      if (patrolPlanExtend.openImportDialogExtend(this, parentQueryParams)) {
    ...
    importTemplate() {
      ...
      this.download('autoee/patrolPlan/downLoadImportTemplatePatrolPlan', {},
        `巡更计划_导入模版_${fileExt}.xlsx`)
    },
  ...
  mounted() {
    // mounted扩展方法
    patrolPlanExtend.importMountedStartExtend(this);
  }
};
</script>
