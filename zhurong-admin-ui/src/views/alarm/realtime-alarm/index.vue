...
import { listItemTopology } from "@/api/system/itemTopology";
export default {
    ...
    watch: {
        // 精确监听query参数变化，避免deep监听带来的性能问题
        '$route.query.alarmLevel'(newVal) {
            if (newVal && newVal !== this.queryParams.alarmLevel) {
                this.queryParams.alarmLevel = newVal;
                this.handleQuery();
            }
        }
    },
    methods: {
        ...
        /* getAreaList() {
            this.loading = true;
            listItemTopology(this.queryAreaParams).then(response => {
