<template>
  <div id="tags-view-container" class="tags-view-container">
    ...
    <ul v-show="visible" :style="{left:left+'px',top:top+'px'}" class="contextmenu">
      <li @click="refreshSelectedTag(selectedTag)"><i class="el-icon-refresh-right"></i> {{ $t('tagsView.refreshPage') }}</li>
...
export default {
  ...
  methods: {
    ...
    isFirstView() {
      try {
        return this.selectedTag.fullPath === '/user/profile' || this.selectedTag.fullPath === this.visitedViews[1].fullPath
...
.tags-view-container {
  ...
  box-shadow: 0 1px 3px 0 var(--tag-shadow-color-1), 0 0 3px 0 var(--tag-shadow-color-2);
  .tags-view-wrapper {
    .tags-view-item {
      ...
      &.active {
        background-color: var(--tag-item-background-active);
        color: var(--tag-item-color-active);
        border-color: var(--tag-item-border-active);
        &::before {
          content: '';
          background: var(--tag-item-icon);
          display: inline-block;
          width: 8px;
          height: 8px;
          border-radius: 50%;
  ...
  .contextmenu {
    ...
    box-shadow: 2px 2px 3px 0 var(--context-menu-shadow);
    li {
      margin: 0;
      padding: 7px 16px;
      cursor: pointer;
      &:hover {
        background: var(--context-menu-background-active);
        color: var(--context-menu-color-active);
      }
    }
  }
}
