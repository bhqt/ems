...
CREATE TABLE IF NOT EXISTS `xxl_job_info` (
  ...
  `schedule_conf` varchar(128) DEFAULT NULL COMMENT 'Schedule configuration',
  `misfire_strategy` varchar(50) NOT NULL DEFAULT 'DO_NOTHING' COMMENT 'Misfire strategy',
  `executor_route_strategy` varchar(50) DEFAULT NULL COMMENT 'Executor route strategy',
  `executor_handler` varchar(255) DEFAULT NULL COMMENT 'Executor handler',
  `executor_param` varchar(512) DEFAULT NULL COMMENT 'Executor parameter',
  `executor_block_strategy` varchar(50) DEFAULT NULL COMMENT 'Block strategy',
  `executor_timeout` int(11) NOT NULL DEFAULT '0' COMMENT 'Executor timeout in seconds',
  ...
  `trigger_status` int(11) NOT NULL DEFAULT '0' COMMENT 'Trigger status: 0=stop, 1=running',
...
CREATE TABLE IF NOT EXISTS `xxl_job_log` (
  ...
  `executor_handler` varchar(255) DEFAULT NULL COMMENT '执行器任务，本次执行的 handler',
...
CREATE TABLE IF NOT EXISTS `xxl_job_user` (
  ...
  `permission` varchar(255) DEFAULT NULL COMMENT '权限：执行器 ID 列表，多个逗号分割',
  PRIMARY KEY (`id`),
