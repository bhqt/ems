package com.ruoyi.autoee.patrolTask.handler;
 ...
 */
@Component
public class PatrolTaskDictHandler implements DictDataHandler
{
	private static final Logger logger = LoggerFactory.getLogger(PatrolTaskDictHandler.class);

	// 要使用Mapper，不能使用Service，否则会出现循环依赖问题
	private final PatrolTaskMapper patrolTaskMapper;
	private static final String dictType = "a_patrol_task";


	// 构造函数注入
	public PatrolTaskDictHandler(PatrolTaskMapper patrolTaskMapper) {
	...
	public List<SysDictData> handleGetDictData(Object params) {
		...
		PatrolTask queryData = JSONUtil.toBean(JSONUtil.toJsonStr(params), PatrolTask.class);
		List<PatrolTask> queryList = patrolTaskMapper.selectDataListByEqPatrolTask(queryData);
		for (int i = 0; i < queryList.size(); i++) {
			...
			data.setDictLabel(queryList.get(i).getPatrolTaskName() + "");
	...
	public HashMap<String, String> handleGetLableValueMap(Object params) {
		...
		PatrolTask queryData = JSONUtil.toBean(JSONUtil.toJsonStr(params), PatrolTask.class);
		List<PatrolTask> list = patrolTaskMapper.selectDataListByEqPatrolTask(queryData);
		for (PatrolTask data : list) {
			map.put(data.getPatrolTaskName()+"", data.getId()+"");
		}
		return map;
	}
	...
	public String handleGetDictAllLableStr(Object params) {
		...
		if (!queryList.isEmpty()) {
			for (PatrolTask data : queryList) {
				res.append(data.getPatrolTaskName()).append(",");
			}
			// 去掉最后一个逗号（如果字符串非空）
			if (res.length() > 0) {
