package com.ruoyi.autoee.goodsInfo.domain;
...
/**
 * 物品信息管理对象 a_goods_info
 *
 * @author AutoEE亿达科技有限公司 版权所有 Spring全栈工程师
 *
 */
// 设置实体类对应的表名
@TableName("a_goods_info") // 如果使用了 MyBatis-Plus，表名默认是根据实体类名自动转换的，如果你的实体类名为 PatrolRecord，MyBatis-Plus 默认会将其转换为 patrol_record表名，而不是你期望的 a_patrol_record。
public class GoodsInfo extends BaseEntity
{
	...
	// 如果是下拉框、自定义下拉框，直接取对应的扩展字段进行前端显示或导出
	@Excel(name = "物品状态" , type = Excel.Type.ALL )
	@TableField(select = false) // 在扩展字段上添加注解，告诉 MyBatis-Plus 这些字段不是数据库表中的列，避免查询报错
    private String goodsStatusExtend;
    /** 规格型号 */
    @JsonFormat(shape = JsonFormat.Shape.STRING) // 其他数字等类型都序列化为字符串类型，避免前端转码问题和long型数字精度丢失问题
	...
	public void setGoodsStatusExtend(String goodsStatusExtend)
    {
        this.goodsStatusExtend = goodsStatusExtend;
    }

    public String getGoodsStatusExtend()
    {
        return goodsStatusExtend;
    }
