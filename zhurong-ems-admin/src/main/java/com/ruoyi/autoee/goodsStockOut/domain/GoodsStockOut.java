package com.ruoyi.autoee.goodsStockOut.domain;
...
/**
 * 物品出库记录对象 a_goods_stock_out
 *
 * @author AutoEE亿达科技有限公司 版权所有 Spring全栈工程师
 *
 */
// 设置实体类对应的表名
@TableName("a_goods_stock_out") // 如果使用了 MyBatis-Plus，表名默认是根据实体类名自动转换的，如果你的实体类名为 PatrolRecord，MyBatis-Plus 默认会将其转换为 patrol_record表名，而不是你期望的 a_patrol_record。
public class GoodsStockOut extends BaseEntity
{
    ...
    private String goodsStockInIdExtend;
    /** 物品名称 */
    @JsonFormat(shape = JsonFormat.Shape.STRING) // 其他数字等类型都序列化为字符串类型，避免前端转码问题和long型数字精度丢失问题
	// 下拉框，直接通过扩展字段进行导出，避免字段类型不一致，无法设置值
    ...
    public void setGoodsStockInId(Long goodsStockInId)
    {
        this.goodsStockInId = goodsStockInId;
    }

    public Long getGoodsStockInId()
    {
        return goodsStockInId;
    }

	public void setGoodsStockInIdExtend(String goodsStockInIdExtend)
    {
        this.goodsStockInIdExtend = goodsStockInIdExtend;
    }

    public String getGoodsStockInIdExtend()
    {
        return goodsStockInIdExtend;
    }
    ...
    public String toString() {
        return new ToStringBuilder(this,ToStringStyle.MULTI_LINE_STYLE)
            .append("id", getId())
            .append("goodsStockInId", getGoodsStockInId())
