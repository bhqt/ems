package com.ruoyi.autoee.patrolRecord.handler;
 ...
 */
@Component
public class PatrolRecordDictHandler implements DictDataHandler
{
	private static final Logger logger = LoggerFactory.getLogger(PatrolRecordDictHandler.class);

	// 要使用Mapper，不能使用Service，否则会出现循环依赖问题
	private final PatrolRecordMapper patrolRecordMapper;
	private static final String dictType = "a_patrol_record";


	// 构造函数注入
	public PatrolRecordDictHandler(PatrolRecordMapper patrolRecordMapper) {
	...
	public List<SysDictData> handleGetDictData(Object params) {
		...
		PatrolRecord queryData = JSONUtil.toBean(JSONUtil.toJsonStr(params), PatrolRecord.class);
		List<PatrolRecord> queryList = patrolRecordMapper.selectDataListByEqPatrolRecord(queryData);
		for (int i = 0; i < queryList.size(); i++) {
			...
			data.setDictLabel(queryList.get(i).getPatrolPlanId() + "");
	...
	public HashMap<String, String> handleGetLableValueMap(Object params) {
		...
		PatrolRecord queryData = JSONUtil.toBean(JSONUtil.toJsonStr(params), PatrolRecord.class);
		List<PatrolRecord> list = patrolRecordMapper.selectDataListByEqPatrolRecord(queryData);
		for (PatrolRecord data : list) {
			map.put(data.getPatrolPlanId()+"", data.getId()+"");
		}
		return map;
	}
	...
	public String handleGetDictAllLableStr(Object params) {
		...
		if (!queryList.isEmpty()) {
			for (PatrolRecord data : queryList) {
				res.append(data.getPatrolPlanId()).append(",");
			}
			// 去掉最后一个逗号（如果字符串非空）
			if (res.length() > 0) {
