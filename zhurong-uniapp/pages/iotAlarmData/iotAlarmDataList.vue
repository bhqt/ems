	...
	<view class="a_container">
		<uni-forms class="a_query_form" :model="pageData.queryParams" ref="queryFormRef"
			...
			<uni-forms-item class="a_query_form_item" label="报警值" name="value">
				<uni-easyinput v-model="pageData.queryParams.value" placeholder="请输入报警值" clearable/>
			</uni-forms-item>
	  ...
	  <view class="list">
			<!-- 基于 uni-list 的页面布局 -->
			<uni-list>
				<!-- 所有字段  deptId,   areaCodeDict,   deviceTypeDict,   deviceCodeDict,   alarmMonitorParam,   alarmName,   value,   alarmCondition,   userId,   updateTime,   -->
				...
				<!-- to 属性携带参数跳转详情页面，当前只为参考 :to="'/pages/news/newsDetail?id='+item.id" -->
			  	...
			  	<uni-list-item v-for="item in pageData.iotAlarmDataList" :key="item.id"
								   :title="item.alarmName.toString()"
								   :rightText="item.updateTime.toString()"
								   :note="formatNote(item)"
								   thumb="/static/biaoshi.svg" thumb-size="medium"
								   showArrow link="navigateTo" :to="'./iotAlarmDataDetail?id='+item.id">
				</uni-list-item>
			</uni-list>
	...
	methods: {
		...
		loadData(){
        	...
        	iotAlarmData.selectDetailListByLikeIotAlarmData(data).then(res => {
            	...
            	let list = res.rows;
            	this.pageData.iotAlarmDataList = list;
        	})
	    },
		//查询
		handleQuery()
		...
		formatNote(item) {
			try {
				return `${item.deviceCodeDictExtend || ''}\n${item.value || ''}\n${item.userIdExtend || ''}`
			} catch (e) {
				return ''
			}
		}
...
<style lang="scss">
//@import '@/static/styles/autoee/iotAlarmData/iotAlarmDataList.scss';
</style>
