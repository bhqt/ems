<template>
	<view class="a_container">
		<uni-forms class="a_add_form" :model="pageData.addUpdateForm" ref="addUpdateFormRef" :rules="pageData.addUpdateFormRules"
			...
			<uni-forms-item v-show="true" class="a_add_form_item" label="物品名称" name="goodsName">
				<uni-easyinput v-model="pageData.addUpdateForm.goodsName" placeholder="请输入物品名称" clearable/>
			</uni-forms-item>
			<uni-forms-item v-show="true" class="a_add_form_item" label="物品类型" name="goodsType">
			...
			<uni-forms-item v-show="true" class="a_add_form_item" label="物品状态" name="goodsStatus">
				<uni-data-picker class="a_add_form_select" v-model="pageData.addUpdateForm.goodsStatus" :localdata="pageData.dictData.goods_status" placeholder="请选择" clearable>
				</uni-data-picker>
			</uni-forms-item>
...
export default {
	...
	data() {
		return {
			...
			pageData: {
				...
				goodsName: {
					rules: [
					{ required: true, errorMessage: "物品名称不能为空", trigger: "blur" },
				]
				},
				// 物品类型
				goodsType: {
				...
				goodsStatus: {
					rules: [
					{ required: true, errorMessage: "物品状态不能为空", trigger: "change" },
				]
				},
				// 规格型号
				specification: {
	...
	methods: {
		...
		loadData(id){
        	...
        	goodsInfo.selectDetailByPkGoodsInfo(id).then(res => {
			...
			this.$refs.addUpdateFormRef.validate().then(res => {
				// this.$modal.confirm('确认提交当前操作？').then(() => {
					if (this.pageData.addUpdateForm.id != null) {
					  ...
					  goodsInfo.updateNullValueByGoodsInfo(this.pageData.addUpdateForm).then(response => {
							// this.$api.msg(`修改成功`);
					  ...
					  goodsInfo.addGoodsInfo(this.pageData.addUpdateForm).then(response => {
							// this.$api.msg(`新增成功`);
...
<style lang="scss">
//@import '@/static/styles/autoee/goodsInfo/goodsInfoAdd.scss';
</style>
