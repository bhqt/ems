<template>
	<view class="a_container">
		<uni-forms class="a_add_form" :model="pageData.addUpdateForm" ref="addUpdateFormRef" :rules="pageData.addUpdateFormRules"
			...
			<uni-forms-item v-show="true" class="a_add_form_item" label="入库编号" name="goodsStockInId">
				<uni-data-picker class="a_add_form_select" v-model="pageData.addUpdateForm.goodsStockInId" :localdata="pageData.dictData.a_goods_stock_in" placeholder="请选择" clearable>
				</uni-data-picker>
			</uni-forms-item>
			<uni-forms-item v-show="true" class="a_add_form_item" label="物品名称" name="goodsId">
			...
			this.$refs.addUpdateFormRef.validate().then(res => {
				// this.$modal.confirm('确认提交当前操作？').then(() => {
					if (this.pageData.addUpdateForm.id != null) {
					  ...
					  goodsStockOut.updateNullValueByGoodsStockOut(this.pageData.addUpdateForm).then(response => {
							// this.$api.msg(`修改成功`);
							this.$modal.alertCallback(`修改成功`, () => {
								uni.navigateBack()
								// this.$tab.reLaunch('/pages/index/index');
								// uni.navigateTo({
								  //     url: `./goodsStockOutList`
								// })
							});
					  });
					} else {
					  goodsStockOut.addGoodsStockOut(this.pageData.addUpdateForm).then(response => {
							// this.$api.msg(`新增成功`);
...
<style lang="scss">
//@import '@/static/styles/autoee/goodsStockOut/goodsStockOutAdd.scss';
</style>
