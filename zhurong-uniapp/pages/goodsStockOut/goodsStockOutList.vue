	...
	<view class="a_container">
		<uni-forms class="a_query_form" :model="pageData.queryParams" ref="queryFormRef"
			...
			<uni-forms-item class="a_query_form_item" label="入库编号" name="goodsStockInId">
				<uni-data-picker class="a_query_form_select" v-model="pageData.queryParams.goodsStockInId" :localdata="pageData.dictData.a_goods_stock_in" placeholder="请选择" clearable>
				</uni-data-picker>
			</uni-forms-item>
			<uni-forms-item class="a_query_form_item" label="物品名称" name="goodsId">
				<uni-data-picker class="a_query_form_select" v-model="pageData.queryParams.goodsId" :localdata="pageData.dictData.a_goods_info" placeholder="请选择" clearable>
				</uni-data-picker>
			</uni-forms-item>
	  ...
	  <view class="list">
			<!-- 基于 uni-list 的页面布局 -->
			<uni-list>
				<!-- 所有字段  id,   goodsStockInId,   goodsId,   quantity,   reason,   userId,   updateTime,   -->
				...
				<!-- to 属性携带参数跳转详情页面，当前只为参考 :to="'/pages/news/newsDetail?id='+item.id" -->
			  	...
			  	<uni-list-item v-for="item in pageData.goodsStockOutList" :key="item.id"
								   ...
								   showArrow link="navigateTo" :to="'./goodsStockOutDetail?id='+item.id">
				</uni-list-item>
			</uni-list>
			<!-- 点击加载更多按钮 -->
...
export default {
	...
	async onLoad(options) {
	    ...
	    // 允许传入参数进行查询列表时的过滤：目前支持下拉框字段
  		if (this.$stringUtil.isNotEmpty(options.goodsStockInId)) {
			this.pageData.queryParams.goodsStockInId = options.goodsStockInId
		}
  		if (this.$stringUtil.isNotEmpty(options.goodsId)) {
			this.pageData.queryParams.goodsId = options.goodsId
		}
	...
	methods: {
		async initData() {
		 	try {
				this.pageData.dictData.a_goods_stock_in = await getDictData('a_goods_stock_in')
		...
		loadData(){
			return new Promise((resolve, reject) => {
				...
				//data.limitTopN = 200
        		goodsStockOut.selectPageListGoodsStockOut(data).then(res => {
           		 	// console.log("getAddressByCurrentUser-res=", res)
            		let list = res.rows;
	                // 如果是第一页，则直接替换数据，否则追加数据
	                if (this.pageData.queryParams.pageNum === 1) {
					  this.pageData.goodsStockOutList = list;
					} else {
	              	  this.pageData.goodsStockOutList = [...this.pageData.goodsStockOutList, ...list];
					}
	            	// 设置加载状态
		...
		formatNote(item) {
			try {
				return ` ${item.goodsIdExtend.toString()} \n  ${item.quantity.toString()} \n  ${item.reason.toString()}`
			} catch (e) {
				return ''
			}
		}
...
<style lang="scss">
//@import '@/static/styles/autoee/goodsStockOut/goodsStockOutList.scss';
</style>
