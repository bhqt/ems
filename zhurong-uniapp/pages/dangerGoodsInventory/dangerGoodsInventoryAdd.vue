<template>
	<view class="a_container">
		<uni-forms class="a_add_form" :model="pageData.addUpdateForm" ref="addUpdateFormRef" :rules="pageData.addUpdateFormRules"
			...
			<uni-forms-item v-show="false" class="a_add_form_item" label="危化品名称" name="dangerGoodsId">
			...
			<uni-forms-item v-show="false" class="a_add_form_item" label="危化品类型" name="dangerGoodsType">
			...
			this.$refs.addUpdateFormRef.validate().then(res => {
				// this.$modal.confirm('确认提交当前操作？').then(() => {
					if (this.pageData.addUpdateForm.id != null) {
					  ...
					  dangerGoodsInventory.updateNullValueByDangerGoodsInventory(this.pageData.addUpdateForm).then(response => {
							// this.$api.msg(`修改成功`);
							this.$modal.alertCallback(`修改成功`, () => {
								uni.navigateBack()
								// this.$tab.reLaunch('/pages/index/index');
								// uni.navigateTo({
								  //     url: `./dangerGoodsInventoryList`
								// })
							});
					  });
					} else {
					  dangerGoodsInventory.addDangerGoodsInventory(this.pageData.addUpdateForm).then(response => {
							// this.$api.msg(`新增成功`);
...
<style lang="scss">
//@import '@/static/styles/autoee/dangerGoodsInventory/dangerGoodsInventoryAdd.scss';
</style>
