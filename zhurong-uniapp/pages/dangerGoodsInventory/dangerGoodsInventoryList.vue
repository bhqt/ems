	...
	<view class="a_container">
		<uni-forms class="a_query_form" :model="pageData.queryParams" ref="queryFormRef"
			...
			<uni-forms-item class="a_query_form_item" label="危化品类型" name="dangerGoodsType">
				<uni-data-picker class="a_query_form_select" v-model="pageData.queryParams.dangerGoodsType" :localdata="pageData.dictData.danger_goods_type" placeholder="请选择" clearable>
				</uni-data-picker>
			</uni-forms-item>
			...
			<uni-forms-item class="a_query_form_item" label="当前库存数量">
				<uni-number-box class="a_query_form_item_number_between" clearable placeholder=">="
				                v-model="pageData.queryParams.params.beginCurrentStock"/>
				&nbsp;~&nbsp;
				<uni-number-box class="a_query_form_item_number_between" clearable placeholder="<="
				                v-model="pageData.queryParams.params.endCurrentStock"/>
			</uni-forms-item>
			<uni-forms-item class="a_query_form_item" label="单位" name="goodsUnit">
				<uni-data-picker class="a_query_form_select" v-model="pageData.queryParams.goodsUnit" :localdata="pageData.dictData.goods_unit" placeholder="请选择" clearable>
				</uni-data-picker>
			</uni-forms-item>
		</uni-forms>
		<view class="a_button">
	  ...
	  <view class="list">
			<!-- 基于 uni-list 的页面布局 -->
			<uni-list>
				<!-- 所有字段  dangerGoodsId,   dangerGoodsType,   specification,   currentStock,   goodsUnit,   updateTime,   -->
				...
				<!-- to 属性携带参数跳转详情页面，当前只为参考 :to="'/pages/news/newsDetail?id='+item.id" -->
			  	...
			  	<uni-list-item v-for="item in pageData.dangerGoodsInventoryList" :key="item.id"
								   ...
								   showArrow link="navigateTo" :to="'./dangerGoodsInventoryDetail?id='+item.id">
				</uni-list-item>
			</uni-list>
			<!-- 点击加载更多按钮 -->
...
export default {
	...
	async onLoad(options) {
	    ...
	    // 允许传入参数进行查询列表时的过滤：目前支持下拉框字段
  		...
  		if (this.$stringUtil.isNotEmpty(options.dangerGoodsType)) {
			this.pageData.queryParams.dangerGoodsType = options.dangerGoodsType
		}
	...
	methods: {
		...
		loadData(){
			return new Promise((resolve, reject) => {
				...
				//data.limitTopN = 200
        		dangerGoodsInventory.selectPageListDangerGoodsInventory(data).then(res => {
           		 	// console.log("getAddressByCurrentUser-res=", res)
            		let list = res.rows;
	                // 如果是第一页，则直接替换数据，否则追加数据
	                if (this.pageData.queryParams.pageNum === 1) {
					  this.pageData.dangerGoodsInventoryList = list;
					} else {
	              	  this.pageData.dangerGoodsInventoryList = [...this.pageData.dangerGoodsInventoryList, ...list];
					}
	            	// 设置加载状态
		...
		formatNote(item) {
			try {
				return ` ${item.dangerGoodsIdExtend.toString()} \n  ${item.dangerGoodsTypeExtend.toString()} \n  ${item.specification.toString()} \n  ${item.currentStock.toString()}`
			} catch (e) {
				return ''
			}
		}
...
<style lang="scss">
//@import '@/static/styles/autoee/dangerGoodsInventory/dangerGoodsInventoryList.scss';
</style>
