<template>
	<view class="a_container">
		<uni-forms class="a_add_form" :model="pageData.addUpdateForm" ref="addUpdateFormRef" :rules="pageData.addUpdateFormRules"
			...
			<uni-forms-item v-show="true" class="a_add_form_item" label="入库编号" name="dangerGoodsStockInId">
				<uni-data-picker class="a_add_form_select" v-model="pageData.addUpdateForm.dangerGoodsStockInId" :localdata="pageData.dictData.a_danger_goods_stock_in" placeholder="请选择" clearable>
				</uni-data-picker>
			</uni-forms-item>
			...
			<uni-forms-item v-show="true" class="a_add_form_item" label="出库数量">
				<uni-easyinput type="number" v-model="pageData.addUpdateForm.quantity" clearable placeholder="请输入出库数量" />
			</uni-forms-item>
			<uni-forms-item v-show="true" class="a_add_form_item" label="出库原因" name="reason">
				<uni-easyinput type="textarea" v-model="pageData.addUpdateForm.reason" placeholder="请输入出库原因" clearable/>
			</uni-forms-item>
...
export default {
	...
	data() {
		return {
			...
			pageData: {
				...
				dangerGoodsStockInId: {
					rules: [
					{ required: true, errorMessage: "入库编号不能为空并且为整数", trigger: "change" },
				]
				},
				// 危化品名称
				dangerGoodsId: {
				...
				quantity: {
					rules: [
					{ required: true, errorMessage: "出库数量不能为空并且为整数", trigger: "blur" },
	...
	methods: {
		...
		loadData(id){
        	...
        	dangerGoodsStockOut.selectDetailByPkDangerGoodsStockOut(id).then(res => {
			...
			this.$refs.addUpdateFormRef.validate().then(res => {
				// this.$modal.confirm('确认提交当前操作？').then(() => {
					if (this.pageData.addUpdateForm.id != null) {
					  ...
					  dangerGoodsStockOut.updateNullValueByDangerGoodsStockOut(this.pageData.addUpdateForm).then(response => {
							// this.$api.msg(`修改成功`);
							this.$modal.alertCallback(`修改成功`, () => {
								uni.navigateBack()
								// this.$tab.reLaunch('/pages/index/index');
								// uni.navigateTo({
								  //     url: `./dangerGoodsStockOutList`
								// })
							});
					  });
					} else {
					  dangerGoodsStockOut.addDangerGoodsStockOut(this.pageData.addUpdateForm).then(response => {
							// this.$api.msg(`新增成功`);
...
<style lang="scss">
//@import '@/static/styles/autoee/dangerGoodsStockOut/dangerGoodsStockOutAdd.scss';
</style>
