	...
	<view class="a_container">
		<uni-forms class="a_query_form" :model="pageData.queryParams" ref="queryFormRef"
			...
			<uni-forms-item class="a_query_form_item" label="点位名称" name="pointName">
				<uni-easyinput v-model="pageData.queryParams.pointName" placeholder="请输入点位名称" clearable/>
			</uni-forms-item>
			<uni-forms-item class="a_query_form_item" label="点位地点" name="pointLocation">
				<uni-easyinput v-model="pageData.queryParams.pointLocation" placeholder="请输入点位地点" clearable/>
			</uni-forms-item>
		</uni-forms>
		<view class="a_button">
	  ...
	  <view class="list">
			<!-- 基于 uni-list 的页面布局 -->
			<uni-list>
				<!-- 所有字段  pointName,   pointLocation,   updateTime,   -->
				...
				<!-- to 属性携带参数跳转详情页面，当前只为参考 :to="'/pages/news/newsDetail?id='+item.id" -->
			  	...
			  	<uni-list-item v-for="item in pageData.patrolPointList" :key="item.id"
								   ...
								   showArrow link="navigateTo" :to="'./patrolPointDetail?id='+item.id">
				</uni-list-item>
			</uni-list>
			<!-- 点击加载更多按钮 -->
	...
	methods: {
		...
		loadData(){
			return new Promise((resolve, reject) => {
				...
				//data.limitTopN = 200
        		patrolPoint.selectPageListPatrolPoint(data).then(res => {
           		 	// console.log("getAddressByCurrentUser-res=", res)
            		let list = res.rows;
	                // 如果是第一页，则直接替换数据，否则追加数据
	                if (this.pageData.queryParams.pageNum === 1) {
					  this.pageData.patrolPointList = list;
					} else {
	              	  this.pageData.patrolPointList = [...this.pageData.patrolPointList, ...list];
					}
	            	// 设置加载状态
...
<style lang="scss">
//@import '@/static/styles/autoee/patrolPoint/patrolPointList.scss';
</style>
