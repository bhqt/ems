	...
	<view class="a_container">
		<uni-forms class="a_query_form" :model="pageData.queryParams" ref="queryFormRef"
			...
			<uni-forms-item class="a_query_form_item" label="路线编号" name="routeCode">
				<uni-easyinput v-model="pageData.queryParams.routeCode" placeholder="请输入路线编号" clearable/>
			</uni-forms-item>
			<uni-forms-item class="a_query_form_item" label="路线名称" name="routeName">
				<uni-easyinput v-model="pageData.queryParams.routeName" placeholder="请输入路线名称" clearable/>
			</uni-forms-item>
			<uni-forms-item class="a_query_form_item" label="状态" name="patrolRouteStatus">
				<uni-data-picker class="a_query_form_select" v-model="pageData.queryParams.patrolRouteStatus" :localdata="pageData.dictData.patrol_route_status" placeholder="请选择" clearable>
				</uni-data-picker>
			</uni-forms-item>
		</uni-forms>
		<view class="a_button">
	  ...
	  <view class="list">
			<!-- 基于 uni-list 的页面布局 -->
			<uni-list>
				<!-- 所有字段  routeCode,   routeName,   patrolRouteStatus,   pointList,   updateTime,   -->
				...
				<!-- to 属性携带参数跳转详情页面，当前只为参考 :to="'/pages/news/newsDetail?id='+item.id" -->
			  	...
			  	<uni-list-item v-for="item in pageData.patrolPathList" :key="item.id"
								   ...
								   showArrow link="navigateTo" :to="'./patrolPathDetail?id='+item.id">
				</uni-list-item>
			</uni-list>
			<!-- 点击加载更多按钮 -->
...
export default {
	...
	async onLoad(options) {
	    ...
	    // 允许传入参数进行查询列表时的过滤：目前支持下拉框字段
  		if (this.$stringUtil.isNotEmpty(options.patrolRouteStatus)) {
			this.pageData.queryParams.patrolRouteStatus = options.patrolRouteStatus
		}
  		if (this.$stringUtil.isNotEmpty(options.pointList)) {
			this.pageData.queryParams.pointList = options.pointList
		}
	...
	methods: {
		...
		loadData(){
			return new Promise((resolve, reject) => {
				...
				//data.limitTopN = 200
        		patrolPath.selectPageListPatrolPath(data).then(res => {
           		 	// console.log("getAddressByCurrentUser-res=", res)
            		let list = res.rows;
	                // 如果是第一页，则直接替换数据，否则追加数据
	                if (this.pageData.queryParams.pageNum === 1) {
					  this.pageData.patrolPathList = list;
					} else {
	              	  this.pageData.patrolPathList = [...this.pageData.patrolPathList, ...list];
					}
	            	// 设置加载状态
		...
		formatNote(item) {
			try {
				return ` ${item.routeCode.toString()} \n  ${item.routeName.toString()} \n  ${item.patrolRouteStatusExtend.toString()} \n  ${item.pointListExtend.toString()}`
			} catch (e) {
				return ''
			}
		}
...
<style lang="scss">
//@import '@/static/styles/autoee/patrolPath/patrolPathList.scss';
</style>
