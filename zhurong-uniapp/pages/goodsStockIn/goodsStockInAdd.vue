...
export default {
	...
	methods: {
		...
		loadData(id){
        	...
        	goodsStockIn.selectDetailByPkGoodsStockIn(id).then(res => {
			...
			this.$refs.addUpdateFormRef.validate().then(res => {
				// this.$modal.confirm('确认提交当前操作？').then(() => {
					if (this.pageData.addUpdateForm.id != null) {
					  ...
					  goodsStockIn.updateNullValueByGoodsStockIn(this.pageData.addUpdateForm).then(response => {
							// this.$api.msg(`修改成功`);
							this.$modal.alertCallback(`修改成功`, () => {
								uni.navigateBack()
								// this.$tab.reLaunch('/pages/index/index');
								// uni.navigateTo({
								  //     url: `./goodsStockInList`
								// })
							});
					  });
					} else {
					  goodsStockIn.addGoodsStockIn(this.pageData.addUpdateForm).then(response => {
							// this.$api.msg(`新增成功`);
...
<style lang="scss">
//@import '@/static/styles/autoee/goodsStockIn/goodsStockInAdd.scss';
</style>
