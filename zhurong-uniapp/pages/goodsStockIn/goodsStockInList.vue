<template>
	<view class="a_container">
	  ...
	  <view class="list">
			<!-- 基于 uni-list 的页面布局 -->
			<uni-list>
				<!-- 所有字段  id,   goodsId,   quantity,   supplier,   userId,   updateTime,   -->
				...
				<!-- to 属性携带参数跳转详情页面，当前只为参考 :to="'/pages/news/newsDetail?id='+item.id" -->
			  	...
			  	<uni-list-item v-for="item in pageData.goodsStockInList" :key="item.id"
								   ...
								   showArrow link="navigateTo" :to="'./goodsStockInDetail?id='+item.id">
				</uni-list-item>
			</uni-list>
			<!-- 点击加载更多按钮 -->
	...
	methods: {
		...
		loadData(){
			return new Promise((resolve, reject) => {
				...
				//data.limitTopN = 200
        		goodsStockIn.selectPageListGoodsStockIn(data).then(res => {
           		 	// console.log("getAddressByCurrentUser-res=", res)
            		let list = res.rows;
	                // 如果是第一页，则直接替换数据，否则追加数据
	                if (this.pageData.queryParams.pageNum === 1) {
					  this.pageData.goodsStockInList = list;
					} else {
	              	  this.pageData.goodsStockInList = [...this.pageData.goodsStockInList, ...list];
					}
	            	// 设置加载状态
...
<style lang="scss">
//@import '@/static/styles/autoee/goodsStockIn/goodsStockInList.scss';
</style>
