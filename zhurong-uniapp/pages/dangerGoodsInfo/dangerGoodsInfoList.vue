	...
	<view class="a_container">
		<uni-forms class="a_query_form" :model="pageData.queryParams" ref="queryFormRef"
			...
			<uni-forms-item class="a_query_form_item" label="危化品名称" name="dangerGoodsName">
				<uni-easyinput v-model="pageData.queryParams.dangerGoodsName" placeholder="请输入危化品名称" clearable/>
			</uni-forms-item>
			...
			<uni-forms-item class="a_query_form_item" label="危化品状态" name="dangerGoodsStatus">
				<uni-data-picker class="a_query_form_select" v-model="pageData.queryParams.dangerGoodsStatus" :localdata="pageData.dictData.danger_goods_status" placeholder="请选择" clearable>
				</uni-data-picker>
			</uni-forms-item>
	  ...
	  <view class="list">
			<!-- 基于 uni-list 的页面布局 -->
			<uni-list>
				<!-- 所有字段  dangerGoodsName,   dangerGoodsType,   dangerGoodsStatus,   specification,   goodsUnit,   updateTime,   -->
				...
				<!-- to 属性携带参数跳转详情页面，当前只为参考 :to="'/pages/news/newsDetail?id='+item.id" -->
			  	...
			  	<uni-list-item v-for="item in pageData.dangerGoodsInfoList" :key="item.id"
								   ...
								   showArrow link="navigateTo" :to="'./dangerGoodsInfoDetail?id='+item.id">
				</uni-list-item>
			</uni-list>
			<!-- 点击加载更多按钮 -->
...
export default {
	...
	async onLoad(options) {
	    ...
	    // 允许传入参数进行查询列表时的过滤：目前支持下拉框字段
  		...
  		if (this.$stringUtil.isNotEmpty(options.dangerGoodsStatus)) {
			this.pageData.queryParams.dangerGoodsStatus = options.dangerGoodsStatus
		}
	...
	methods: {
		...
		loadData(){
			return new Promise((resolve, reject) => {
				...
				//data.limitTopN = 200
        		dangerGoodsInfo.selectPageListDangerGoodsInfo(data).then(res => {
           		 	// console.log("getAddressByCurrentUser-res=", res)
            		let list = res.rows;
	                // 如果是第一页，则直接替换数据，否则追加数据
	                if (this.pageData.queryParams.pageNum === 1) {
					  this.pageData.dangerGoodsInfoList = list;
					} else {
	              	  this.pageData.dangerGoodsInfoList = [...this.pageData.dangerGoodsInfoList, ...list];
					}
	            	// 设置加载状态
...
<style lang="scss">
//@import '@/static/styles/autoee/dangerGoodsInfo/dangerGoodsInfoList.scss';
</style>
