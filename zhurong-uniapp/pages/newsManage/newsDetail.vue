	...
	<view class="container">
		...
		<view class="">
			<view style="padding: 0 30upx;">
				<text class=""
				      style="font-size:  large;font-weight: bold;color: black">{{ newsManageDetail.newsTitle }}
				</text>
			</view>
			<view style="padding: 0 30upx;">
				<uni-icons type="contact-filled" size="16" color="#999"></uni-icons>
				<text class="uni-footer-text" style="font-weight: bold; font-size: 14">{{ newsManageDetail.author }}</text>
				<uni-icons style="padding-left: 10px" type="cloud-upload-filled" size="16" color="#999"></uni-icons>
				<text class="uni-footer-text" style="font-weight: bold; font-size: 14"> {{ newsManageDetail.createTime }}</text>
			</view>
		</view>
		<view style="padding: 10upx 30upx;">

			<!--#ifdef APP-PLUS || H5-->
			<u-parse :content="newsManageDetail.newsContent" @preview="preview" @navigate="navigate"></u-parse>
			<!--#endif-->
			<!--#ifdef MP-->
			<rich-text :nodes="newsManageDetail.newsContent"></rich-text>
			<!--#endif-->
		</view>
...
export default {
	...
	async onLoad(options) {
		...
		newsManage.selectDetailByPkNewsManage(id).then(res => {
			this.newsManageDetail = res.data;
			let productImagesArr = this.newsManageDetail.newsImage.split(",")
