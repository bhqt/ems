	...
	<view class="a_container">
		<uni-forms class="a_query_form" :model="pageData.queryParams" ref="queryFormRef"
			...
			<uni-forms-item class="a_query_form_item" label="存放地点" name="supplier">
				<uni-easyinput v-model="pageData.queryParams.supplier" placeholder="请输入存放地点" clearable/>
			</uni-forms-item>
		</uni-forms>
		<view class="a_button">
	  ...
	  <view class="list">
			<!-- 基于 uni-list 的页面布局 -->
			<uni-list>
				<!-- 所有字段  id,   dangerGoodsId,   quantity,   supplier,   userId,   updateTime,   -->
				...
				<!-- to 属性携带参数跳转详情页面，当前只为参考 :to="'/pages/news/newsDetail?id='+item.id" -->
			  	...
			  	<uni-list-item v-for="item in pageData.dangerGoodsStockInList" :key="item.id"
								   ...
								   showArrow link="navigateTo" :to="'./dangerGoodsStockInDetail?id='+item.id">
				</uni-list-item>
			</uni-list>
			<!-- 点击加载更多按钮 -->
	...
	methods: {
		...
		loadData(){
			return new Promise((resolve, reject) => {
				...
				//data.limitTopN = 200
        		dangerGoodsStockIn.selectPageListDangerGoodsStockIn(data).then(res => {
           		 	// console.log("getAddressByCurrentUser-res=", res)
            		let list = res.rows;
	                // 如果是第一页，则直接替换数据，否则追加数据
	                if (this.pageData.queryParams.pageNum === 1) {
					  this.pageData.dangerGoodsStockInList = list;
					} else {
	              	  this.pageData.dangerGoodsStockInList = [...this.pageData.dangerGoodsStockInList, ...list];
					}
	            	// 设置加载状态
...
<style lang="scss">
//@import '@/static/styles/autoee/dangerGoodsStockIn/dangerGoodsStockInList.scss';
</style>
