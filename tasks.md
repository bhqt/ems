...
- [x] T4.2 角色定制看板：按角色配置与展示 — views/hospital/dashboard（FR-17）已交付：按当前用户 roles/权限动态渲染模块（能耗概览/院区对比/设备状态/分项/趋势/排名/单位工作量能效/最近报警），admin 全量、非 admin 按角色预设展示，已入库菜单 hospital:dashboard + 看板查询按钮
